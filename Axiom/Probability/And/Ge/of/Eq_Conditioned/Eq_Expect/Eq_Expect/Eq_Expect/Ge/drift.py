...
@prove
def prove(Eq):
    from Axiom import Algebra, Neuro, Probability
    ...
    *Eq[-5:], (Eq.ge_VF, Eq.ge_reward) = apply(Equal(r[t] | s[:t] & a[:t], r[t]), # history-irrelevant conditional independence assumption for rewards based on states and actions
    ...
    Eq.VQ, Eq.VV, Eq.QV = Neuro.And.Eq.Expect.of.Eq_Conditioned.Eq_Expect.Eq_Expect.Bellman.apply(*Eq[:3])

    Eq.Q_quote = Eq[1].subs(π, π_quote)

    Eq.V_quote = Eq[2].subs(π, π_quote)

    Eq.VQ_quote, Eq.VV_quote, Eq.QV_quote = Neuro.And.Eq.Expect.of.Eq_Conditioned.Eq_Expect.Eq_Expect.Bellman.apply(Eq[0], Eq.Q_quote, Eq.V_quote)
