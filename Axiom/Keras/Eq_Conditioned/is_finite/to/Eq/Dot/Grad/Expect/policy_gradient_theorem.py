...
def prove(Eq):
    ...
    *Eq[-2:], Eq.hypothesis = apply(
    ...
    Eq << Keras.Eq_Conditioned.Eq_Expect.Eq_Expect.is_finite.to.Eq.Dot.Grad.Expect.Q_Function.apply(Eq[0], Eq.Q_Function, Eq.V_Function, Eq[-1])

    Eq << Eq[-1].this.rhs.apply(Discrete.Dot.eq.Sum)

    Eq << Eq[-1].this.find(Mul[Expectation]).apply(Stats.Mul.eq.Expect)

    Eq.eq_expect = Eq[-1].this.rhs.apply(Stats.Sum.Expect.eq.Expect.Sum)

    Eq << Keras.Eq_Conditioned.to.Eq.Expect.Grad.Log.Prob.Q_Function.discounted.apply(Eq[0], γ, t, π)
    ...
    Eq << Eq[-1].this.lhs.apply(Stats.Mul.eq.Expect)

    Eq << Eq[-1].this.rhs.apply(Stats.Mul.eq.Expect)

    Eq << Algebra.Eq.to.Eq.Sum.apply(Eq[-1], (t, 0, oo))

    Eq << Eq[-1].this.rhs.apply(Stats.Sum.Expect.eq.Expect.Sum)

    Eq << Eq[-1].this.lhs.apply(Stats.Sum.Expect.eq.Expect.Sum)

    Eq << Algebra.Eq.Eq.to.Eq.trans.apply(Eq.eq_expect, Eq[-1])

    Eq << Eq[-1].this.find(Sum).apply(Discrete.Sum.eq.Dot, 1)

    Eq << Eq[-1].this.rhs.apply(Stats.Expect.eq.Dot)
