...
def prove(Eq):
    ...
    Eq << Eq[-1].this.rhs.find(Expectation[Conditioned]).apply(Stats.Expect.eq.Mul)

    Eq << Eq[-1].this.rhs.find(Expectation[Conditioned[ReducedSum]]).apply(Stats.Expect.ReducedSum.eq.ReducedSum.Expect)

    Eq << Stats.Eq_Conditioned.to.Eq.Conditioned.independence_assumption.bidirectional.forget_histories.apply(Eq[0])

    Eq << Eq[-2].subs(Eq[-1])

    Eq << Eq[-1].this.find(ReducedSum[Expectation]).apply(Stats.ReducedSum.Expect.eq.Expect.ReducedSum)
