...
def prove(Eq):
    ...
    *Eq[-4:], Eq.hypothesis = apply(
    ...
    Eq.expect = Keras.Eq_Conditioned.Eq_Expect.Eq_Expect.to.Eq.Grad.Expect.policy_gradient.apply(*Eq[:3], n)

    Eq << Calculus.Eq.to.Eq.Limit.apply(Eq.expect, (n, oo))

    Eq.limit = Eq[-1].this.find(Limit).apply(Calculus.Limit.eq.Add)

    Eq << Stats.AbsExpect.le.ExpectAbs.apply(Eq.limit.find(Expectation[Derivative]).subs(n, t))

    Eq << Stats.Expect.le.Sup.apply(Eq[-1].rhs)

    Eq.le_sup = Algebra.Le.Le.to.Le.trans.apply(Eq[-2], Eq[-1])
    ...
    Eq << Algebra.Le.Le.to.Le.trans.apply(Eq[-1], Eq.le_sup)

    Eq << Algebra.Cond.to.All.apply(Eq[-1], t, simplify=None)
    ...
    Eq << Eq[-1].this.rhs.find(Expectation[Sum]).apply(Stats.Expect.Sum.eq.Sum.Expect)
