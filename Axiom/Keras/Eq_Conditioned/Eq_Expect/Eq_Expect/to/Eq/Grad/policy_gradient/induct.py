...
def prove(Eq):
    ...
    *Eq[-3:], Eq.hypothesis = apply(
    ...
    Eq.recursion = Keras.Eq_Conditioned.Eq_Expect.Eq_Expect.to.Eq.Grad.policy_gradient.recursion.apply(*Eq[:3])
    ...
    Eq << Eq[-1].this.find(Integral[Mul[Probability]] * Product).apply(Calculus.Mul.eq.Integral)
