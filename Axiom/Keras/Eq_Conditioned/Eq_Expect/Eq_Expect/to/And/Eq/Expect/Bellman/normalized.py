...
def prove(Eq):
    ...
    Eq << Keras.Eq_Expect.to.Eq.Expect.V_Function.normalized.apply(Eq[1])

    Eq << Algebra.Eq.Eq.to.Eq.trans.apply(Eq[2], Eq[-1])

    Eq << Stats.Eq_Conditioned.to.Eq.Conditioned.getitem.apply(Eq[0], 1)

    Eq << Keras.Eq_Conditioned.to.Eq.Expect.Bellman.V_Function.normalized.apply(Eq[-1], γ, t)
    ...
    Eq << Keras.Eq_Conditioned.to.Eq.Expect.Bellman.Q_Function.normalized.apply(Eq[0], γ, t)
