...
def prove(Eq):
    ...
    Eq << Algebra.Cond.to.Cond.domain_defined.apply(Eq.x_independence)

    Eq << Stats.Ne_0.to.And.Ne_0.apply(Eq[-1])

    Eq << Stats.Ne_0.to.Ne_0.Conditioned.apply(Eq[-3], y[:k])

    Eq << Stats.Ne_0.to.Prob.eq.Mul.Prob.bayes.apply(Eq[-2], x[:k + 1], y[k])

    Eq << Eq[-1].this.lhs.arg.apply(Algebra.And.concat, i=2, j=0)

    Eq << Stats.Ne_0.to.Eq.bayes.Conditioned.apply(Eq[-3], x[k], y[k])

    Eq << Eq[-1].this.lhs.find(And).apply(Algebra.And.concat, i=2, j=0)

    Eq << Eq[-3].subs(Eq[-1])

    Eq.xy_joint_probability = Stats.Ne_0.to.Prob.eq.Mul.Prob.bayes.apply(Eq[2], x[:k])

    Eq << Eq[-1].subs(Eq.xy_joint_probability.reversed)

    Eq.recursion = Algebra.Ne_0.Eq.to.Eq.scalar.apply(Eq[0], Eq[-1])

    Eq << Stats.Ne_0.to.Ne_0.joint_slice.apply(Eq.xy_nonzero_assumption, [k, k])

    Eq << Stats.Eq_Conditioned.to.Eq.Conditioned.getitem.apply(Eq.x_independence)

    Eq << Stats.Eq_Conditioned.Eq_Conditioned.to.Eq.Prob.joint.apply(Eq[-1], Eq.xy_independence)

    Eq << Stats.Ne_0.Eq_Conditioned.to.Eq.Conditioned.joint.apply(Eq[-1], Eq[0])

    Eq.recursion = Eq.recursion.subs(Eq[-1])

    Eq << Algebra.Cond.to.Or.subs.apply(Eq[2], k, k + 1)

    Eq << Eq[-1].this.find(NotElement).simplify()

    Eq << Algebra.Or.to.All.apply(Eq[-1])

    _, Eq.y_nonzero_assumption = Stats.Ne_0.to.And.Ne_0.apply(Eq.xy_nonzero_assumption)

    Eq <<= Eq[-1] & Eq.y_nonzero_assumption

    Eq.y_joint_y_historic = Eq[-1].this.lhs.arg.apply(Algebra.Eq.to.And.Eq.split)

    Eq << Stats.Ne_0.to.Ne_0.Conditioned.apply(Eq.y_joint_y_historic, y[:k])

    Eq << Stats.Ne_0.to.Eq.bayes.Conditioned.apply(Eq[-1], Eq.x_independence.lhs.lhs)

    Eq.recursion = Eq.recursion.subs(Eq[-1])

    Eq.recursion = Eq.recursion.subs(Eq.first_order_markov_assumption)

    Eq << Stats.Eq_Conditioned.to.Eq.Conditioned.getitem.apply(Eq.x_independence, wrt=y[:k])

    Eq << Stats.Ne_0.Eq_Conditioned.to.Eq.Conditioned.joint.apply(Eq.y_joint_y_historic, Eq[-1])

    Eq.recursion = Eq.recursion.subs(Eq[-1])

    Eq << Algebra.Eq.to.Eq.Prod.apply(Eq.recursion, (k, 1, k + 1))
    ...
    Eq << Algebra.Cond.to.Or.subs.apply(Eq[-1], k, t)
