...
def prove(Eq):
    ...
    Eq << Algebra.Ceiling.lt.Add_1.apply(Eq.eq_V_dquote.find(Ceiling).arg) - 1

    Eq << Eq[-1] * d + β[i]

    Eq << Eq[-1].this.rhs.subs(Eq.zeta[i])

    Eq << Algebra.Lt.to.Lt.relax.apply(Eq[-1], upper=n)

    Eq.le_ceiling = Algebra.Lt.to.Le.strengthen.apply(Eq[-1])

    Eq <<= Eq.eq_K_dquote.this.find(Min).args[1].apply(Algebra.Expr.eq.Piece, upper=Eq.le_ceiling.lhs), Eq.eq_V_dquote.this.find(Min).args[1].apply(Algebra.Expr.eq.Piece, upper=Eq.le_ceiling.lhs)

    Eq <<= Eq[-2].this.rhs().find(GreaterEqual).simplify(), Eq[-1].this.rhs().find(GreaterEqual).simplify()

    Eq <<= Eq[-2].this.find(Min).args[0].apply(Algebra.Expr.eq.Piece, lower=Eq.le_ceiling.lhs), Eq[-1].this.find(Min).args[0].apply(Algebra.Expr.eq.Piece, lower=Eq.le_ceiling.lhs)

    Eq <<= Algebra.Cond.Cond.to.Cond.subs.apply(Eq.le_ceiling, Eq[-2]), Algebra.Cond.Cond.to.Cond.subs.apply(Eq.le_ceiling, Eq[-1])
    ...
    Eq << Algebra.Le.to.Le.Ceiling.apply(Eq.le_min / d)

    Eq <<= Algebra.Le.Eq.to.Eq.Slice.apply(Eq[-1], Eq[-3]), Algebra.Le.Eq.to.Eq.Slice.apply(Eq[-1], Eq[-2])

    Eq <<= Algebra.Eq.Eq.to.Eq.trans.apply(Eq.slice_K_dquote, Eq[-2]), Algebra.Eq.Eq.to.Eq.trans.apply(Eq.slice_V_dquote, Eq[-1])

    Eq << Keras.Eq_Max.Eq_Min.Eq.Eq.to.Eq.Softmax.bert.position_representation.relative.band_part_mask.dilated.apply(Eq.beta, Eq.zeta, Eq[0], Eq[1], Q, K, V)
