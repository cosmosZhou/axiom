...
def prove(Eq):
    ...
    Eq << Eq[-1].this.rhs.args[0].apply(Keras.Softmax.eq.Mul.ReducedSum)

    Eq.zi_definition = Eq[-1].this.rhs.subs(Eq[-3])

    Eq << Eq.Xi_definition.this.rhs.defun()[i]

    Eq.Xi_definition = Eq[-1].this.rhs.expr.apply(Algebra.Bool.eq.Piece)
    ...
    Eq.le_zeta_i = Algebra.Eq.to.Le.relax.apply(Eq.zeta_i, upper=n)

    Eq << Algebra.Le.Eq.to.Eq.Slice.apply(Eq.le_zeta_i, Eq[-1], start=β[i], step=d)

    Eq.zi_definition = Eq[-3].subs(Eq[-1])

    Eq.le = Algebra.Eq_Max.Eq_Min.to.Le.apply(Eq.beta, Eq.zeta)

    Eq <<= Algebra.Le.Eq.to.Eq.Slice.apply(Eq.le, Eq.K_dquote[i], step=d), Algebra.Le.Eq.to.Eq.Slice.apply(Eq.le, Eq.V_dquote[i], step=d)
