...
def prove(Eq):
    ...
    Eq <<= Eq[-1].lhs.find(MatMul).this.apply(Discrete.Dot.eq.Lamda),\
        Eq[-1].lhs.find(MatMul[2]).this.apply(Discrete.Dot.eq.Lamda),\
        Eq[-1].lhs.find((~MatMul) - MatMul).this.apply(Discrete.Dot.eq.Lamda),\
        Eq[-1].lhs.find(MatMul - ~MatMul).this.apply(Discrete.Dot.eq.Lamda)
    ...
    Eq <<= Eq[-1].find(Lamda + Lamda).this.apply(Algebra.Add.eq.Lamda, deep=True),\
        Eq[-1].find(Lamda - Lamda).this.apply(Algebra.Add.eq.Lamda, deep=True),\
        Eq[-1].lhs.args[1].find(Lamda - Lamda).this.apply(Algebra.Add.eq.Lamda, deep=True)
    ...
    Eq << Eq[-3].subs(*Eq[-2:])
    Eq <<= Eq[-1].lhs.find(Sin * Sin + Cos * Cos).this.apply(Geometry.Add.eq.Cos), \
        Eq[-1].lhs.find(Sin * Cos - Sin * Cos).this.apply(Geometry.Sub.eq.Sin)
    ...
    Eq <<= Eq[-1].find(Lamda).this.apply(Algebra.Lamda.eq.Mul),\
        Eq[-1].find(-~Lamda).this.apply(Algebra.Lamda.eq.Mul)

    Eq << Eq[-1].find(Lamda[KroneckerDelta]).this.apply(Algebra.Lamda.eq.Identity)
