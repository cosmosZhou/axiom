...
def apply(eq_theta_r, eq_theta_c, eq_theta_z, eq_R, Q, K, V, r, c, z, t):
    from Axiom.Keras.Eq_Mul.Eq_Mul.Eq_Mul.Eq_Block.to.Eq.Dot.position_representation.space import extract
...
def prove(Eq):
    ...
    from Axiom.Keras.Eq_Mul.Eq_Mul.Eq_Mul.Eq_Block.to.Eq.Dot.position_representation.space import rotary_matrix
    ...
    Eq << Keras.Eq_Mul.Eq_Mul.Eq_Mul.Eq_Block.to.Eq.Dot.position_representation.space.apply(*Eq[:4], i_quote, j_quote, k_quote)

    Eq << Eq[-1].subs(i, r[t]).subs(j, c[t]).subs(k, z[t]).subs(i_quote, r[i]).subs(j_quote, c[i]).subs(k_quote, z[i])

    Eq.final = Eq[-3].subs(Eq[-1])

    Eq << Keras.Eq_Mul.Eq_Mul.Eq_Mul.Eq_Block.to.Dot.eq.Add.position_representation.space.apply(*Eq[:4], K[t], r, c, z)
    ...
    Eq <<= Eq[-1].find(Mul[Add]).this.apply(Algebra.Mul.eq.Add),\
        ...
        Eq[-1].find(Sum[3], Mul[Add]).this.apply(Algebra.Mul.eq.Add), \
        Eq[-1].find(Sum[3], Mul[Add][2]).this.apply(Algebra.Mul.eq.Add)
