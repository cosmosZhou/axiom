...
def prove(Eq):
    ...
    Eq << Eq[-1].this.find(Lamda ** 2).apply(Algebra.Pow.eq.Lamda)

    Eq << Eq[-1].this.find(Lamda ** 2).apply(Algebra.Pow.eq.Lamda)

    Eq << Eq[-1].this.find(Add).apply(Algebra.Add.eq.Lamda)

    Eq << Eq[-1].this.find(Add).apply(Algebra.Add.Piece.eq.Piece)

    Eq << Eq[-1].this.find(Cos ** 2).apply(Geometry.Square.Cos.eq.Sub.Square.Sin, simplify=None)
