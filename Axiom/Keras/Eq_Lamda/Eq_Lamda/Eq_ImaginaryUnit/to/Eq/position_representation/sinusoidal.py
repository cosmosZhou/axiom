...
def prove(Eq):
    ...
    Eq <<= Eq.f_def.this.find(Floor).apply(Algebra.Floor.eq.Piece), Eq.f_quote_def.this.find(Floor).apply(Algebra.Floor.eq.Piece)

    Eq <<= Eq[-2].this.find(Mul[Piecewise]).apply(Algebra.Mul.eq.Piece, simplify=None), Eq[-1].this.find(Mul[Piecewise]).apply(Algebra.Mul.eq.Piece, simplify=None)

    Eq <<= Eq[-2].this.find(Mul[Add]).apply(Algebra.Mul.eq.Add, simplify=None), Eq[-1].this.find(Mul[Add]).apply(Algebra.Mul.eq.Add, simplify=None)

    Eq <<= Eq[-2].this.find(Pow[Piecewise]).apply(Algebra.Pow.eq.Piece.exponent, simplify=None), Eq[-1].this.find(Pow[Piecewise]).apply(Algebra.Pow.eq.Piece.exponent, simplify=None)

    Eq <<= Eq[-2].this.find(Mul[Piecewise]).apply(Algebra.Mul.eq.Piece, simplify=None), Eq[-1].this.find(Mul[Piecewise]).apply(Algebra.Mul.eq.Piece, simplify=None)

    Eq <<= Eq[-2].this.find(Cos[Piecewise]).apply(Geometry.Cos.eq.Piece), Eq[-1].this.find(Sin[Piecewise]).apply(Geometry.Sin.eq.Piece)
    ...
    Eq << Eq.PE_definition.find(sin).this.arg.apply(Algebra.Mul.eq.Add)

    Eq << Eq.PE_definition.find(cos).this.arg.apply(Algebra.Mul.eq.Add)
    ...
    Eq << Eq[-1].this.rhs.apply(Algebra.Mul.Piece.eq.Piece, simplify=None)
    ...
    Eq << Eq[-1].this.rhs.apply(Algebra.Add.Piece.eq.Piece, simplify=None)

    Eq << Algebra.Eq.Eq.to.Eq.trans.apply(Eq.cossin, Eq[-1])

    Eq << Algebra.Eq.to.Eq.Lamda.apply(Eq[-1], (j, 0, d))

    Eq.PE_equality = Eq[-1].this.rhs.apply(Algebra.Lamda.eq.Add)

    Eq << Eq.PE_quote_definition.find(cos).this.arg.apply(Algebra.Mul.eq.Add)

    Eq << Eq.PE_quote_definition.find(sin).this.arg.apply(Algebra.Mul.eq.Add)

    Eq <<= Eq[-2].this.rhs.apply(Geometry.Cos.eq.Sub), Eq[-1].this.rhs.apply(Geometry.Sin.eq.Add)

    Eq <<= Algebra.Eq.Eq.to.Eq.trans.apply(Eq[-4], Eq[-2])
    ...
    Eq << Algebra.Eq.Eq.to.Eq.trans.apply(Eq.coscos, Eq[-1])
    ...
    Eq.geometric_progression = Algebra.Eq.to.Eq.geometric_progression.apply(Eq[-1], n=i)
    ...
    Eq << Eq[3].this.find(Mul).apply(Algebra.Mul.eq.Add)

    Eq << Eq[-1].this.rhs.apply(Algebra.Exp.eq.Mul)

    Eq.eq_euler = Eq[-1].this.find(exp).apply(Geometry.ExpI.eq.Add.Euler)

    Eq << Algebra.Mod.eq.Piece.apply(Eq.eq_euler.find(Mod))

    Eq << Eq[-1] / -2

    Eq << Eq[-1].this.rhs.apply(Algebra.Mul.eq.Piece, simplify=None)

    Eq << Eq[-1] + 1

    Eq << Eq[-1].this.rhs.apply(Algebra.Add.eq.Piece, simplify=None)

    Eq << Eq[-1] * S.Pi

    Eq << Eq[-1].this.rhs.apply(Algebra.Mul.eq.Piece, simplify=None)

    Eq <<= Geometry.Eq.to.Eq.Cos.apply(Eq[-1]), Geometry.Eq.to.Eq.Sin.apply(Eq[-1])

    Eq << Eq.eq_euler.subs(Eq[-1], Eq[-2])

    Eq << Eq[-1].this.find(Mul[Piecewise]).apply(Algebra.Mul.eq.Piece, simplify=None)

    Eq << Eq[-1].this.find(Add).apply(Algebra.Add.Piece.eq.Piece, simplify=None)
