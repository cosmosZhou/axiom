...
def prove(Eq):
    ...
    Eq <<= Algebra.Eq.to.Eq.Lamda.apply(Eq[-2], (j, 0, m)), Algebra.Eq.to.Eq.Lamda.apply(Eq[-1], (j, 0, m))

    Eq <<= Algebra.Eq.to.Eq.Transpose.apply(Eq[-2], (0, 1)), Algebra.Eq.to.Eq.Transpose.apply(Eq[-1], (0, 1))

    Eq <<= Algebra.Eq.Eq.to.Eq.trans.apply(Eq[-2], Eq[4]), Algebra.Eq.Eq.to.Eq.trans.apply(Eq[-1], Eq[5])

    Eq << Algebra.Eq.to.Eq.Transpose.apply(Eq[-2], (1, 1))

    Eq << Keras.Eq_Card.Subset_Cup.to.Eq.Dot.Softmax.position_representation.relative.gather.apply(Eq[0], Eq[1], Q, K, V, K_quote, V_quote)
