...
def prove(Eq):
    ...
    Eq << Keras.Eq_Block.to.Eq.mask.cross_attention.apply(Eq.ksi_def, a)
    ...
    Eq.divisor_definition = Eq[-1].this.rhs.apply(Algebra.Sum.eq.Piece)
    ...
    Eq <<= Algebra.Eq.to.Eq.Exp.apply(Eq[-2]), Algebra.Eq.to.Eq.Exp.apply(Eq[-1])

    Eq.lower_part, Eq.upper_part = Algebra.Eq.Eq.to.Eq.trans.apply(Eq[-2], Eq[0][i]), \
        Algebra.Eq.Eq.to.Eq.trans.apply(Eq[-1], Eq[1][i - h])
    ...
    Eq << Eq[-1].this.rhs.apply(Algebra.Lamda.eq.Piece)
    ...
    Eq << Eq[-1].this.find(Lamda[Mul[OneMatrix]]).apply(Algebra.Lamda.eq.Mul)
