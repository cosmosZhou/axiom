...
def prove(Eq):
    ...
    Eq << Keras.Eq_Add_Block.to.Eq.mask.cross_attention.apply(Eq.ksi_def, a)
    ...
    Eq << Eq.zi_definition.find(ReducedSum).this.apply(Discrete.ReducedSum.eq.Dot)

    Eq << Eq[-1].this.rhs.apply(Discrete.Dot.eq.Sum)

    Eq << Eq[-1].this.rhs.subs(Eq.ksi_def)

    Eq << Eq[-1].this.rhs.apply(Algebra.Sum.eq.Piece)

    Eq << Eq[-1].this.find(ExprCondPair)().expr.args[0].simplify()

    Eq << Eq[-1].this.find(ExprCondPair[2]).expr.apply(Algebra.Add.eq.Piece)
    ...
    Eq.divisor_definition = Eq[-1].this.rhs.apply(Algebra.Piece.eq.Add)
    ...
    Eq << Eq[-1].this.find(ReducedSum).apply(Discrete.ReducedSum.eq.Dot)
    ...
    Eq <<= Algebra.Eq.to.Eq.Exp.apply(Eq[-3]), Algebra.Eq.to.Eq.Exp.apply(Eq[-2]), Algebra.Eq.to.Eq.Exp.apply(Eq[-1])

    Eq << Eq[-1] * OneMatrix(d_z)

    Eq.lower_part, Eq.upper_part, Eq.diagonal_part = Algebra.Eq.Eq.to.Eq.trans.apply(Eq[-4], Eq[1][i]), \
        Algebra.Eq.Eq.to.Eq.trans.apply(Eq[-3], Eq[2][i - h]), \
        Algebra.Eq.Eq.to.Eq.trans.apply(Eq[-1], Eq.M_definition)

    Eq << Eq.divisor_definition * OneMatrix(d_z)

    Eq << Eq[-1].this.rhs.apply(Algebra.Mul.eq.Add, simplify=None)

    Eq << Eq[-1].this.rhs.subs(Eq.lower_part, Eq.upper_part, Eq.diagonal_part)

    Eq << Eq[-1].this.rhs.simplify()

    Eq.zi_definition = Algebra.Eq.Cond.to.Cond.subs_with_expand_dims.apply(Eq[-1], Eq.zi_definition)
    ...
    Eq << Eq[-1].this.rhs.expr.apply(Algebra.Piece.eq.Add)
    ...
    Eq << Eq[-1].this.find(Lamda).apply(Algebra.Lamda.eq.Piece)
    ...
    Eq << Algebra.Eq.Cond.to.Cond.subs_with_expand_dims.apply(Eq.diagonal_part, Eq[-1])
    ...
    Eq << Eq[-1].this.find(Lamda[Pow]).apply(Algebra.Lamda.eq.Pow)
    ...
    Eq << Eq[-1].this.find(Lamda).apply(Algebra.Lamda.eq.ReducedSum)
