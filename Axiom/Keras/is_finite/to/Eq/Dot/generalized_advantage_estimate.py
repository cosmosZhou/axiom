...
def prove(Eq):
    ...
    Eq << Algebra.Ne.to.Eq.Sum.geometric_series.apply(Eq[-1], Eq[-2].rhs.find(Mul[~Sum]))

    Eq << Eq[-3].subs(Eq[-1]) * (1 - λ)

    Eq << Eq[-1].this.find(Pow * Pow).apply(Algebra.Mul.eq.Add, 3)
    ...
    Eq << Calculus.Eq.to.Eq.Limit.apply(Eq[-1], (n, oo))
    ...
    Eq << Sets.LtAbs.is_finite.to.is_real.Sum.apply(Eq.lt, Eq[-1], simplify=None)
    ...
    Eq << Calculus.LtAbs.to.Eq_0.Limit.geometric_series.apply(Eq.lt, n)

    Eq << Calculus.Eq_0.is_limited.to.Eq_0.Limit.algebraic_limit_theorem.apply(Eq[-1], Eq[-2])
