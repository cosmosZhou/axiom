...
def prove(Eq):
    ...
    Eq << Keras.Eq_Block.Eq_Block.to.All.Eq.relative_distance.lower_triangle.upper_part.apply(Eq[1], Eq[2])

    Eq << Algebra.All_Eq.to.Eq.Lamda.apply(Eq[-1].this.expr.reversed, Eq[3].find(Lamda).expr)

    Eq << Keras.Eq_Block.Eq_Block.to.All.Eq.relative_distance.lower_triangle.lower_part.apply(Eq[1], Eq[2])
    ...
    Eq << Eq[-1].this.find(-~Min).apply(Keras.Min.eq.Add.Relu)
