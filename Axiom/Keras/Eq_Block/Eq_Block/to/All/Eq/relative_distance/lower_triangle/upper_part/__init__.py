...
def prove(Eq):
    ...
    Eq <<= Algebra.Cond.to.All.restrict.apply(Eq[-1], (i, 0, l), simplify=None)

    Eq << Algebra.All_Eq.to.All.Eq.Slice.apply(Eq[-1], slice(0, i + 1))
    ...
    Eq << Eq[-1].this.find(relu).apply(Keras.Relu.eq.Add.Min)
