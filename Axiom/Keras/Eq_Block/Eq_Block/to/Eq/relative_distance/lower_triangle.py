...
def prove(Eq):
    ...
    Eq <<= Algebra.Cond.to.All.restrict.apply(Eq[-1], (i, 0, n), simplify=None)

    Eq << Algebra.All_Eq.to.All.Eq.Slice.apply(Eq[-1], slice(i + 1 - Min(i + 1, l), i + 1))

    Eq << Eq[-1].this.rhs.find(~Indexed - Indexed).args[1].apply(Algebra.Expr.eq.Piece, upper=n - 1)

    Eq << Eq[-1].this(i).rhs(j).find(GreaterEqual).simplify()

    Eq.V_relu = Eq[-1].this.rhs.find(-~Min).apply(Keras.Min.eq.Add.Relu, index=1)

    Eq << Eq[1][i]

    Eq << Algebra.Cond.to.All.restrict.apply(Eq[-1], (i, 0, n), simplify=None)

    Eq << Algebra.All_Eq.to.All.Eq.Slice.apply(Eq[-1], slice(0, Min(i + 1, l)))

    Eq << Algebra.Eq.Eq.to.Eq.trans.apply(Eq.V_relu, Eq[-1])
