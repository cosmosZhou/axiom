...
def prove(Eq):
    ...
    Eq << Eq[-1].this.rhs.find(Add).apply(Algebra.Add.Piece.eq.Piece, swap=True)

    Eq << Eq[-1].this.rhs.find(Add[Piecewise]).apply(Algebra.Add.eq.Piece)

    Eq << Eq[-1].this.rhs.find(Add[Piecewise]).apply(Algebra.Add.eq.Piece)

    Eq.upper_part = Eq[-1].this.rhs.apply(Algebra.Lamda.Piece.eq.Lamda.Block)
    ...
    Eq << Algebra.Eq.to.Eq.Lamda.apply(Eq[-1], (i, 0, n + 1 - Min(l, n)), simplify=None)
