...
def prove(Eq):
    ...
    Eq << Algebra.Eq.Cond.to.Cond.subs.apply(Eq.x_definition.reversed, Eq[-1])

    Eq << Algebra.Eq.Cond.to.Cond.subs.apply(Eq.G_definition.reversed, Eq[-1])
    ...
    Eq << Algebra.Or.to.Imply.apply(Eq[-1], 0)

    Eq << Eq[-1].this.rhs.apply(Algebra.Eq.Eq.to.Eq.Sub, Eq[-4])
