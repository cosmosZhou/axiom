...
def prove(Eq):
    ...
    Eq << Eq[0].this.rhs.apply(Keras.Dot.Softmax.eq.Lamda.Dot.gpt.batched, i)

    Eq << Eq[-1].subs(n, n + 1)

    Eq << Eq[-1].this.find(Lamda).apply(Algebra.Lamda.eq.Block.pop)

    Eq << Algebra.Eq.to.Eq.Transpose.apply(Eq[2], axis=(0, 1))

    Eq << Eq[-2].subs(Eq[-1].reversed)

    Eq << Algebra.Eq.to.Eq.Transpose.apply(Eq[-1], axis=(0, 1))
