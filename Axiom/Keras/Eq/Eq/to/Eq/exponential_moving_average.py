...
def apply(initial_condition, recurrence, n=None):
    from Axiom.Keras.Eq.Eq.to.Eq.adam import extract
...
def prove(Eq):
    ...
    Eq << Keras.Eq.Eq.to.Eq.adam.apply(Eq[0], Eq[1], n)

    Eq << Eq[-1].subs(Eq[-2])

    Eq << Eq[-1].this.find(Sum[~Mul]).apply(Algebra.Mul.eq.Add)

    Eq << Eq[-1].this.find(Sum).apply(Algebra.Sum.eq.Add)

    Eq << Eq[-1].this.rhs.apply(Algebra.Mul.eq.Add, i=-1)
    ...
    Eq << Eq[-1].this.find(Sum).expr.args[:2].apply(Algebra.Mul.eq.Add)

    Eq << Eq[-1].this.find(Mul - Pow * Pow).apply(Algebra.Add.eq.Mul)

    Eq << Eq[-1].this.find(Mul[Sum]).apply(Algebra.Mul.eq.Sum)

    Eq << Eq[-1].this.find(Sum).expr.args[:3].apply(Algebra.Mul.eq.Add)
