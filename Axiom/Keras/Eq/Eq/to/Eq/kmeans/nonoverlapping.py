...
def prove(Eq):
    ...
    Eq << Sets.Eq.of.And.Imply.apply(Eq[4], wrt=j)
    ...
    Eq << Sets.Eq.Eq.to.Eq.Intersect.apply(Eq.omega_i_definition0, Eq[-1])

    Eq << Eq[-1].this.find(And).apply(Sets.Eq.Eq.to.In.FiniteSet)

    Eq << Eq.nonoverlapping.subs(Eq[-1])

    Eq << Eq[-1].this().find(Intersection).simplify()

    Eq << Sets.All_Eq_EmptySet.to.Eq.nonoverlapping.intlimit.utility.apply(Eq.nonoverlapping, k)
