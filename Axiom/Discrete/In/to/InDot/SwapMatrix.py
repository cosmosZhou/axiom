...
def prove(Eq):
    ...
    Eq << Sets.In.In.to.Subset.FiniteSet.apply(Eq[-1], Eq[-2], simplify=None)

    Eq << Sets.In.Subset.to.Subset.apply(Eq[0], Eq[-1], simplify=None)

    Eq << Sets.Subset.to.In.Piece.apply(Eq[-1], piecewise=Eq.element_piecewise.lhs)
