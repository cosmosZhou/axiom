...
def prove(Eq):
    ...
    Eq.x_slice_last, Eq.x_slice_domain = Algebra.And.All.of.All_And.apply(Eq[-1])

    Eq << Eq.x_slice_domain.this.expr.apply(Discrete.And.of.Eq.index, v)

    Eq.h_domain, Eq.x_h_equality = Algebra.And.All.of.All_And.apply(Eq[-1])

    hv = Eq.x_h_equality.expr.lhs.indices[0]
    Eq << Discrete.All_MemDot.permutation.apply(n + 1, w=w)
    ...
    Eq << Algebra.All.of.All_Eq.Cond.subs.apply(Eq.x_h_equality, Eq[-1])
    ...
    Eq << Eq.x_slice_domain.this.expr.apply(Discrete.And.of.Eq.index, u)
    ...
    Eq << Algebra.All.of.All_Eq.Cond.subs.apply(Eq[-1], Eq.indexu_eq_indexu)
