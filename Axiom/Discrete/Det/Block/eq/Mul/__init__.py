...
def prove(Eq):
    ...
    Eq << (BlockMatrix([[ZeroMatrix(2 * n, n), Identity(2 * n), ZeroMatrix(2 * n, n)], [Identity(n), ZeroMatrix(n, 2 * n), ZeroMatrix(n, n)], [ZeroMatrix(n, n), ZeroMatrix(n, 2 * n), Identity(n)]]) @ Eq[0].lhs.arg).this.apply(Discrete.Dot.eq.Block, True)
    ...
    Eq << Eq[-1].this.rhs.apply(Discrete.Dot.eq.Block, True)
