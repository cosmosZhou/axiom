...
def prove(Eq):
    ...
    Eq << Algebra.Eq.to.All.Eq.apply(Eq[-1])

    Eq << Eq[-1].this.expr.apply(Algebra.Eq.to.All.Eq)

    Eq << Eq[-1].this.rhs.apply(Algebra.Add.eq.Mul)

    Eq.recursion = Algebra.All_Eq.Cond.to.All.subs.apply(Eq[-1], Eq.recursion)
    ...
    Eq << Algebra.Cond.to.Cond.subs.apply(Eq[0], a[:n], a[1:n + 1])
    ...
    Eq << Algebra.Cond.to.All.apply(Eq[-1], _i)

    Eq << Algebra.All_Eq.Cond.to.All.subs.apply(Eq[-1], Eq.recursion)

    Eq << Eq.expand.apply(Discrete.Eq.to.Eq.Det)
