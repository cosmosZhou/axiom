...
def prove(Eq):
    ...
    x = x_slice.base
    Eq.x_abs_positive_s1 = Algebra.All.of.All_And.apply(Eq[-1])

    Eq.x_abs_sum_s1 = Algebra.All.of.All_And.apply(Eq[-1], 1)

    Eq.x_union_s1 = Algebra.All.of.All_And.apply(Eq[-1], 2)
    ...
    Eq << Set.Subset.given.All_Mem.apply(Eq.x_quote_set_in_s2)

    Eq << Eq[-1].subs(Eq.s2_definition)

    Eq << Eq[-1].this.expr.apply(Set.Mem.given.Mem.split.Imageset)

    Eq << Eq[-1].this.expr.rhs.definition

    Eq << Eq[-1].this.expr.args[0].simplify()

    Eq << Set.EqCup.of.Eq.apply(Eq[1], (i, 0, k + 1))
    ...
    Eq << Eq[-2].this.expr.apply(Algebra.Ge.of.Eq.Le.subs, Eq[-1])
    ...
    Eq << Eq[-2].apply(Algebra.All.of.Cond.restrict, (i, Unequal(i, j)))
    ...
    Eq << Algebra.All.And.of.All.All.apply(Eq.x_abs_positive_s1, Eq[-3].reversed)
    ...
    Eq << Eq.xi_all_is_positive.this.expr.apply(Algebra.Cond.given.And.All, cond=Equal(i, j))

    Eq << Algebra.All_And.given.And.All.apply(Eq[-1])

    Eq << Eq[-1].apply(Algebra.All.given.All.Or.limits.delete, simplify=None)

    Eq << Eq[-1].this.find(NotElement).apply(Set.NotMem_SDiff.given.Or, simplify=None)
    ...
    Eq << Eq[-1].this.find(NotElement).apply(Set.Or.of.NotMem_SDiff, simplify=None)

    Eq <<= Eq.x_quote_union & Eq.SqueezeTheorem & Eq.xi_all_is_positive

    Eq.x_quote_definition = Eq[1].apply(Algebra.EqLamda.of.Eq, (i, 0, k + 1), simplify=False)
    ...
    Eq << Set.Supset.given.All_Mem.apply(Eq[-1])
    ...
    Eq << Algebra.Any.of.Any_Eq.Cond.subs.apply(Eq[-2], Eq[-1])

    Eq.set_size_inequality = Eq[-1].this.expr.apply(Algebra.Lt.of.Eq.Lt.subs, Less(Eq[-1].expr.rhs, Eq[-1].expr.rhs + 1, plausible=True))

    Eq << Eq.x_quote_union.this.expr.lhs.apply(Set.Cup.eq.Union.split, cond={i, j})

    Eq << Set.CardUnion.le.AddCardS.apply(*Eq[-1].lhs.args)

    Eq << Set.CardCup.le.Sum_Card.apply(*Eq[-2].lhs.args[0].args)

    Eq << Algebra.Le.of.Le.Le.subs.apply(Eq[-2], Eq[-1])

    Eq << Algebra.Any.And.of.Cond.Any.apply(Eq[-1], Eq.set_size_inequality)
    ...
    Eq << Set.all.of.all.conditionset.split.baseset.apply(Eq[-1], simplify=False, s=s2_hat_n)
    Eq.s2_hat_n_assertion = Eq[-1].this.expr.apply(Set.element.of.any_contains.split.cup)
    Eq << s2_hat_n.this.definition
    Eq << Eq[-1].this.rhs.apply(Set.conditionset.to.imageset)
    ...
    Eq << Set.All_CupFinset.eq.Range.apply(s2_quote_n)
    Eq << Eq[-1].this.expr.apply(Set.Eq.Eq.All_is_positive.notcontains.then.eq.Stirling2, s1=s1_quote)
    Eq << Algebra.All_any.then.All_any.limits_swap.apply(Eq[-1])
    Eq << Eq.s2_hat_n_hypothesis.this.expr.expr.apply(Set.eq.of.eq.cup_finiteset)
    Eq << Eq[-1].subs(Eq.x_quote_definition)
    Eq.supset_A = Set.supset.then.supset.cup.lhs.apply(Eq.supset_A, (j,), simplify=False)
