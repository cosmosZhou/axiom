...
def prove(Eq):
    ...
    Eq << Discrete.Eq.to.And.index.apply(Eq[0], j=j)
    ...
    Eq << Discrete.Eq.to.And.index.apply(Eq[0], j=i)

    Eq.di_domain, Eq.x_di_eqaulity = Eq[-2].subs(Eq.di_definition.reversed), Eq[-1].subs(Eq.di_definition.reversed)

    Eq << Algebra.Cond.Cond.to.Cond.subs.apply(Eq.di_domain, Eq.expand)

    Eq.expand = Algebra.Cond.Cond.to.Cond.subs.apply(Eq.dj_domain, Eq[-1])

    Eq << Sets.In.In.to.Subset.FiniteSet.apply(Eq.dj_domain, Eq.di_domain, simplify=False)

    Eq.eq_intersection = Sets.Subset.to.Eq.Intersect.apply(Eq[-1])

    Eq << Eq.expand.subs(Eq.x_di_eqaulity)

    Eq.union_equality, Eq.piecewise_equality = Sets.Subset.to.Eq.Union.apply(Eq[-2]), Eq.definition.subs(Eq[-1])

    Eq.piecewise_equality = Eq.piecewise_equality.this.lhs.apply(Discrete.Dot.eq.Sum)

    Eq << Eq.piecewise_equality.lhs.args[-1].this.apply(Algebra.Sum.Complement.eq.Add)
    ...
    Eq.dj_eq = Sets.In.to.Eq.Piece.expr_swap.apply(Eq.dj_domain, Eq.piecewise_equality.lhs.args[2])

    Eq << Sets.In.to.Eq.Piece.expr_swap.apply(Eq.di_domain, Eq.piecewise_equality.lhs.args[-1])

    Eq << Sets.In.to.Eq.Intersect.apply(Eq.dj_domain)
    ...
    Eq << Discrete.Eq.to.Eq.index.Delta.indexOf.apply(Eq[0], i, j)
