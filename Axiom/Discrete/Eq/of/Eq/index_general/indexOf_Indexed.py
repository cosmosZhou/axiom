...
def prove(Eq):
    ...
    Eq.ou = Algebra.Or.of.Cond.subs.apply(Eq[-1], t, x[j])
    ...
    Eq << Algebra.Cond.of.All.subs.apply(Eq[-1], Eq[-1].variable, j)

    Eq <<= Eq[-1] & Eq.xj_notcontains

    Eq << Discrete.Eq.of.Eq.index.Delta.Indexed.apply(Eq[0], i, j)

    Eq.ou1 = Algebra.Or.of.Cond.subs.apply(Eq[-1], i, Eq[1].lhs)

    Eq.ou2 = Algebra.Or.of.Cond.subs.apply(Eq[2], t, x[j])
