...
def prove(Eq):
    ...
    Eq << Discrete.Lamda.Indexed.eq.Dot.swap.apply(x, w, left=False, reference=None).subs(i, 0).subs(j, d[0])

    Eq.induct = Eq.hypothesis.subs(m, m + 1)

    Eq << Discrete.Indexed.eq.Dot.swap.apply(x, Lamda[k](Eq[1].lhs.expr.indices[0]).simplify(), w)
