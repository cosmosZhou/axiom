...
def prove(Eq):
    ...
    Eq.x_union_s0, *_ = Algebra.And.All.of.All_And.apply(Eq[-1], slice(1, None, 2))
    ...
    Eq << Algebra.Any.And.of.All.Any.apply(Eq.x_union_s0, Eq[-1].reversed, simplify=None)

    Eq << Eq[-1].this.expr.apply(Set.EqUnionS.of.Eq.Eq)

    Eq << Eq[-1].this().expr.lhs.simplify()

    Eq << Algebra.Any.of.All_Eq.Any.subs.apply(Eq.x_union_s0, Eq[-1])

    Eq << Eq.plausible_notcontains.this.expr.apply(Set.Eq_EmptySet.Inter.of.NotMem)

    Eq.all_s0_equality = Eq[-1].this.expr.apply(Set.EqSDiff.of.Inter_Eq_EmptySet)

    x_hat = Symbol(r"\hat{x}", Lamda[i](Piecewise((x[i] - {n} , Equal(i, j)), (x[i], True))))
    Eq.x_hat_definition = x_hat[i].this.definition

    Eq << Logic.And.ou.OrAndS.of.Cond_Ite__Ite.apply(Eq.x_hat_definition)
    ...
    Eq << Eq[-1].this.expr.apply(Algebra.Any.And.of.All.Any)

    Eq.all_B_contains = Eq[-1].this.expr.expr.apply(Algebra.Eq.of.Eq.Eq.subs, swap=True).limits_subs(Eq[-1].variable, Eq.all_s0_equality.variable)

    Eq.all_s0_contains = Set.All_Mem.split.Imageset.apply(B)
    ...
    Eq.all_B_equality = Eq[-1].this.expr.apply(Set.EqUnion.of.Eq, Eq[-1].variable)

    Eq << Set.Eq.of.All_Mem.All_Mem.All_Eq.All_Eq.apply(Eq.all_s0_contains, Eq.all_B_contains, Eq.all_s0_equality, Eq.all_B_equality)
