...
def prove(Eq):
    ...
    Eq << Discrete.Eq_Adjoint.Imply_Gt_0.Eq_Piece.to.All.And.Cholesky.apply(*Eq[:3], t)
    ...
    Eq.eq_conj = Sets.is_real.to.Eq.Conj.apply(Eq[-2])

    Eq <<= Algebra.All_And.to.And.All.apply(Eq[-1]), Sets.In.to.In.relax.apply(Eq[-2], S.Complexes)
    ...
    Eq << Eq[-1].this.rhs.apply(Sets.In.to.In.relax, S.Complexes)
    ...
    Eq << Algebra.Eq.to.Eq.Conj.apply(Eq[0][i, j])
    ...
    Eq << Eq[-1].this.rhs.apply(Algebra.Eq.to.Eq.Conj)
