...
def prove(Eq):
    ...
    Eq <<= Set.Or.Eq.of.Mem_Finset.apply(Eq[0]), Set.Or.Eq.of.Mem_Finset.apply(Eq[1])

    Eq <<= Eq[-2].this.args[0].apply(Algebra.Eq.of.Eq.invoke, f), Eq[-1].this.args[0].apply(Algebra.Eq.of.Eq.invoke, f)

    Eq <<= Eq[-2].this.args[0].apply(Algebra.Eq.of.Eq.invoke, f), Eq[-1].this.args[0].apply(Algebra.Eq.of.Eq.invoke, f)

    Eq <<= Eq[-2].subs(Eq[2]), Eq[-1].subs(Eq[2])

    Eq << Algebra.Eq.of.Eq.Eq.apply(*Eq[-2:])
