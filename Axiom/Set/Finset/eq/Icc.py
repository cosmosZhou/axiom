...
def prove(Eq):
    ...
    Eq << Set.Subset.Finset.of.Mem.Mem.apply(Eq[-2], Eq[-1], simplify=None)

    Eq << Element(a + 2, Eq[0].rhs, plausible=True)

    Eq << Set.Subset.of.Mem.Subset.apply(Eq[-1], Eq[-2], simplify=None)

    Eq << Element(a + 3, Eq[0].rhs, plausible=True)

    Eq.subset = Set.Subset.of.Mem.Subset.apply(Eq[-1], Eq[-2], simplify=None)

    Eq.supset = Supset(*Eq.subset.args, plausible=True)

    Eq << Set.Supset.given.All_Mem.apply(Eq.supset)
