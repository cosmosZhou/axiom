...
def prove(Eq):
    ...
    Eq << Cup[y:F(x)](Piecewise((f(x, y), Element(x, A)), (g(x, y), True))).this.apply(Set.Cup.eq.Ite)

    Eq << Set.EqCup.of.Eq.apply(Eq[-1], (x, B))

    Eq << Eq[-1].this.rhs.apply(Set.Cup.eq.Union.single_variable)
