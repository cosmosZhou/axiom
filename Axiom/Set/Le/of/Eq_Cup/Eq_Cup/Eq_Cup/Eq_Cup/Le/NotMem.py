...
def prove(Eq):
    ...
    Eq.abs_union = Set.EqCard.of.NotMem.apply(Eq[5])
    ...
    Eq.contains = Set.Mem.of.Eq_Cup.apply(Eq[3])

    Eq.abs_complement = Set.Eq.Card.SDiff.of.Mem.apply(Eq.contains)
    ...
    Eq << Set.Any.Eq.of.Eq_Cup.Mem.apply(Eq[1], Eq.contains)

    Eq << Algebra.Any.And.of.Cond.Any.apply(Eq.given, Eq[-1], simplify=None)

    Eq << Eq[-1].this.expr.apply(Logic.Cond.of.Eq.Cond.subs, ret=0)

    Eq << Algebra.Any.And.of.Any.limits.Cond.apply(Eq[-1], 0, simplify=None)

    Eq << Algebra.Any.of.Any_And.limits_absorb.apply(Eq[-1], simplify=None)

    Eq << Eq[-1].this.expr.apply(Set.Le.of.Le_Abs.Mem, simplify=None, ret=1)

    Eq << Eq[-1].subs(Eq.add_ab.reversed)

    Eq << Eq[-1].this.find(Element).apply(Set.EqSum.of.Mem, Eq[-1].find(Sum[2]).find(Sum), simplify=None, ret=0)

    Eq << Eq[-1].this.expr.args[:2].apply(Logic.Cond.of.Eq.Cond.subs, simplify=None)

    Eq << Eq[-1].this.expr.args[1].apply(Set.EqSum.of.Mem, Eq[-1].expr.args[0].lhs.args[3])

    Eq << Eq[-1].this.expr.apply(Logic.Cond.of.Eq.Cond.subs, simplify=None)

    Eq << Algebra.Any.And.of.Any.limits.Cond.apply(Eq[-1], 0, simplify=None)

    Eq << Algebra.Any.of.Any_And.limits_absorb.apply(Eq[-1], -1, simplify=None)

    Eq << Eq[-1].this.expr.apply(Logic.Cond.of.Eq.Cond.subs, reverse=True)

    Eq << Set.EqSum.of.Eq_Cup.Eq_Cup.NotMem.apply(Eq[0], Eq[2], Eq[5], Eq.eq_X_union.rhs.find(Sum))
    ...
    Eq << Set.EqSum.of.Eq_Cup.Eq_Cup.NotMem.apply(Eq[0], Eq[2], Eq[5], Eq.eq_X_union.rhs)

    Eq << Eq[-2].subs(Eq[-1].reversed)

    Eq << Algebra.Any.And.of.Cond.Any.apply(Eq[1] & Eq[3], Eq[-1], simplify=None)

    Eq << Algebra.Any.And.of.Any.limits.unleash.apply(Eq[-1], simplify=None)

    Eq << Algebra.Any.of.Any_And.limits_absorb.apply(Eq[-1], -2, simplify=None)

    Eq << Eq[-1].this.expr.apply(Set.EqSum.of.Eq.Eq.Eq.Mem, Eq[-1].find(Sum[2]).find(Sum), simplify=None, ret=slice(None))

    Eq << Algebra.Any.And.of.Any.limits.unleash.apply(Eq[-1], simplify=None)

    Eq << Eq[-1].this.expr.args[3:5].apply(Logic.Cond.of.Eq.Cond.subs, simplify=None)

    Eq << Algebra.Any.of.Any_And.limits_absorb.apply(Eq[-1], -2, simplify=None)

    Eq << Eq[-1].this.expr.apply(Set.EqSum.of.Eq.Eq.Eq.Mem, Eq[-1].find(Sum[2]), simplify=None)
