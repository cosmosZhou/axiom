...
def prove(Eq):
    ...
    Eq <<= Eq[-2].subs(Eq.eq_max), Eq[-3].this.expr.apply(Algebra.Any.given.Cond.subs, x, (m + y) / 2), Eq[-1].this.find(NotElement).apply(Set.NotMem_Icc.given.Or)
    ...
    Eq <<= Eq[-3].this.lhs.apply(Algebra.Gt.of.Lt.Gt, ret=1), Eq[-1].this.rhs.apply(Set.Mem.given.And.strengthen, lower=M, strict=True)
