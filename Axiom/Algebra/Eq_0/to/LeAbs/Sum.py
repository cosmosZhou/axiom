...
def prove(Eq):
    ...
    Eq << Algebra.Eq_0.to.Sum.eq.Neg.Sum.apply(Eq[0], Eq[1].find(Sum), i=i)

    Eq << Algebra.Eq.to.Eq.Abs.apply(Eq[-1])

    Eq << Algebra.AbsSum.le.Sum_Abs.apply(Eq[-1].rhs.arg)

    Eq << Algebra.Eq.Le.to.Le.trans.apply(Eq[-2], Eq[-1])

    Eq << Eq[-1].this.find(Abs[Mul]).apply(Algebra.Abs.eq.Mul)

    Eq << Algebra.All_Le_Maxima.apply(Eq[1].find(Maxima))

    Eq << Eq[-1].this.expr.apply(Algebra.Le.to.Le.Mul, Eq[-2].rhs.find(Abs[Sum]))

    Eq << Algebra.All_Le.to.Le.Sum.apply(Eq[-1])

    Eq << Algebra.Le.Le.to.Le.trans.apply(Eq[-4], Eq[-1])
