...
def prove(Eq):
    ...
    Eq.delta_is_nonnegative = Algebra.Ge_0.Sqrt.of.Ge_0.apply(Eq[1])

    Eq << Eq.delta_is_nonnegative - b

    Eq << Algebra.LeDiv.of.Lt_0.Ge.apply(Eq[0], Eq[-1])

    Eq << Eq[-1] / 2

    Eq << Set.Mem.Icc.of.Le.apply(Eq[-1])

    Eq << Set.Mem_Union.of.Mem.apply(Eq[-1], Reals, simplify=None)

    epsilon = Symbol(negative=True)
    Eq << Set.MemAdd.of.Mem_Icc.apply(Eq[-1], epsilon, simplify=None)

    Eq << Algebra.Any.given.Cond.subs.apply(Eq[2], x, Eq[-1].lhs)

    Eq << Algebra.And.given.And.apply(Eq[-1], simplify=None)
