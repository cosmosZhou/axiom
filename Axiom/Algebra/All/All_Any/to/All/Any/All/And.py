...
def prove(Eq):
    ...
    Eq << Eq[-1].this.expr.expr.apply(Algebra.All.of.Or)

    Eq << Eq[-1].this.expr.apply(Algebra.Any_Or.of.OrAnyS, simplify=None)

    Eq << Eq[-1].this.find(Any).apply(Algebra.Any.of.Cond, simplify=None)

    Eq << Eq[-1].this.expr.apply(Algebra.Or.of.All)

    Eq << Eq[-1].this.expr.apply(Algebra.Any_And.of.And, index=0)
