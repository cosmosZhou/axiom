...
def prove(Eq):
    ...
    Eq << Eq[-1].this.expr.expr.apply(Algebra.All.given.Or)

    Eq << Eq[-1].this.expr.apply(Algebra.Any_Or.given.OrAnyS, simplify=None)

    Eq << Eq[-1].this.find(Any).apply(Algebra.Any.given.Cond, simplify=None)

    Eq << Eq[-1].this.expr.apply(Algebra.Or.given.All)
