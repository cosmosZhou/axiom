...
def prove(Eq):
    ...
    Eq << Eq.induct.this.lhs.apply(Algebra.All.equ.And.split, cond={n})

    Eq << Eq[-1].this.find(All).apply(Algebra.All_Ne.to.Eq.Sum, Eq.hypothesis.rhs.lhs)

    Eq << Eq[-1].this.lhs.find(Equal).apply(Algebra.Eq.transport, rhs=0)

    Eq << Eq[-1].this.lhs.find(Equal).reversed

    Eq << Algebra.Imply.to.Imply.And.both_sided.apply(Eq.hypothesis, cond=Eq[-1].lhs.find(Equal))

    Eq << Eq[-1].this.rhs.apply(Algebra.Eq.Eq.to.Eq.Add)
