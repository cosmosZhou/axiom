...
def prove(Eq):
    ...
    Eq.w3_conj = Algebra.Eq.to.Eq.Conj.apply(Eq.w3)
    ...
    Eq << Eq.expand.find(Symbol * ~Add).this.args[0].apply(Algebra.Mul.eq.Add, deep=True)

    Eq << Eq[-1].this.rhs.find(Mul[Add]).apply(Algebra.Mul.eq.Add, deep=True, simplify=None)
    ...
    Eq << -Eq.expand.rhs.args[-1].this.apply(Algebra.Mul.eq.Add, deep=True)
    ...
    Eq.A3_def = Algebra.Eq.to.Eq.Pow.apply(Eq.A_def, exp=3)

    Eq.B3_def = Algebra.Eq.to.Eq.Pow.apply(Eq.B_def, exp=3)
    ...
    Eq.eq_pow = Eq[-1].this.rhs.apply(Algebra.Mul.eq.Add, deep=True)
    ...
    Eq << Algebra.Eq_Pow.to.Eq.cubic_root.omega.apply(Eq.eq_pow)
    ...
    Eq << Eq[-1].this.find(Exp).apply(Algebra.Expr.eq.Add.complex)
