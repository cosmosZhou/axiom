...
def prove(Eq):
    ...
    Eq <<= Algebra.Eq.to.Eq.Pow.apply(Eq.A, exp=3), Algebra.Eq.to.Eq.Pow.apply(Eq.B, exp=3)
    ...
    Eq << Algebra.Eq.to.Eq.Pow.apply(Eq[-1], exp=3)

    Eq << Eq[-1].this.rhs.apply(Algebra.Mul.eq.Add, deep=True)

    Eq << Algebra.Eq_Pow.to.Eq.cubic_root.omega.apply(Eq[-1]) * 3

    Eq << Eq[-1].this.rhs.subs(Eq.A, Eq.B)

    Eq << Eq[-1].this.find(Ceiling).apply(Algebra.Ceiling.Arg.eq.Piece)

    Eq << Eq[-1].subs(Eq[1])

    Eq << Eq[-1].this.find(Exp).apply(Geometry.ExpI.eq.Add.Euler)
