...
def prove(Eq):
    ...
    Eq <<= Eq[-2].this.rhs.apply(Algebra.Add.eq.Zero.to.And_Imply_Or_EqS_Div.quadratic, x=x), Eq[-1].this.rhs.apply(Algebra.Ne_0.Eq.to.Eq.Div)

    Eq <<= Algebra.Imply.to.And.Imply.apply(Eq[-2], None), Algebra.Imply.to.And.Imply.apply(Eq[-1].this.rhs.apply(Algebra.Eq_.Add.Zero.to.And.Imply.Or.Eq.cubic.one_leaded, x), None)
