...
def apply(self, swap=False, deep=False, *, simplify=True):
    ...
    if simplify and rhs.is_Piecewise:
        from Axiom.Logic.Ite_Ite.eq.Ite__Ite import flatten
        rhs = flatten(rhs)
...
def prove(Eq):
    ...
    Eq << Eq[-1].this.find(Add).apply(Algebra.Add.eq.Ite)

    Eq << Eq[-1].this.lhs.args[1].find(Add).apply(Algebra.Add.eq.Ite)
