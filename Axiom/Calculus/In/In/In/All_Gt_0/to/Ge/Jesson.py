...
def prove(Eq):
    ...
    Eq << Algebra.All.to.Cond.subs.apply(Eq[3], x, x_)

    Eq << Calculus.Gt_0.to.Ge.Jesson.apply(Eq[-1], w=w_)

    Eq << Algebra.Cond.to.All.apply(Eq[-1], w_)
    ...
    Eq << Algebra.Cond.to.All.apply(Eq[-1], x0)
    ...
    Eq << Algebra.Cond.to.All.apply(Eq[-1], x1)
