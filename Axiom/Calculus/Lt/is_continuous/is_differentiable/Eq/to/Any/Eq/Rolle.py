...
def prove(Eq):
    ...
    Eq << Eq[2].this.expr.apply(Sets.In.to.Eq.definition)

    Eq.All_And = Eq[-1].this.expr.apply(Calculus.Eq_Grad.to.And.Eq_Grad)

    Eq << Algebra.All_And.to.And.All.apply(Eq.All_And)

    y = Eq[-1].expr.rhs
    Eq << Element(y, Interval(-oo, oo, left_open=False, right_open=False), plausible=True)

    Eq <<= Algebra.Cond.All.to.All.And.apply(Eq[-1], Eq[-3]), Algebra.Cond.All.to.All.And.apply(Eq[-1], Eq[-2])

    Eq <<= Eq[-2].this.expr.apply(Algebra.Eq.Cond.to.Cond.subs, reverse=True, simplify=None), Eq[-1].this.expr.apply(Algebra.Eq.Cond.to.Cond.subs, reverse=True, simplify=None)

    Eq << Calculus.Lt.is_continuous.is_extended_real.is_extended_real.Eq.to.Any.Le.Rolle.apply(*Eq[:2], *Eq[-2:], Eq[3])

    Eq << Eq.All_And.this.expr.apply(Algebra.Eq.Eq.to.Eq.Mul)

    Eq << Algebra.All.Any.to.Any.And.apply(*Eq[-2:])

    Eq << Eq[-1].this.expr.apply(Algebra.Eq.Cond.to.Cond.subs)

    Eq << Eq[-1].this.expr.apply(Algebra.Le_0.to.Eq_0)

    Eq << Eq[-1].this.expr.apply(Algebra.Eq_.Square.Zero.to.Eq_0)
