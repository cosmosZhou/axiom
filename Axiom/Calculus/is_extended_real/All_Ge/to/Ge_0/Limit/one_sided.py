...
def prove(Eq):
    ...
    Eq << Eq[-2].this.lhs.apply(Calculus.is_real.All_Ge.to.Ge_0.Limit.one_sided)
    ...
    Eq << Calculus.Eq_oo.to.Any.All.Lt_0.apply(Eq[-1], delta=δ_quote)

    Eq << Eq[-1].this.expr.apply(Algebra.All.to.All.limits.restrict, domain=Interval.open(x0 - Min(delta, δ_quote), x0))

    Eq << Algebra.All.to.All.limits.restrict.apply(Eq[1], domain=Interval.open(x0 - Min(delta, δ_quote), x0))
    ...
    Eq << Eq[-1].this.lhs.apply(Algebra.Eq.to.Ge_0)
