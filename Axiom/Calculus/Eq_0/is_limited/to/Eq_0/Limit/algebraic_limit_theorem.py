...
def prove(Eq):
    ...
    Eq << Calculus.Eq_Limit.to.Any.All.limit_definition.apply(Eq[0], ε, δ_0)

    δ_1 = Symbol(real=True, positive=True)
    Eq << Calculus.is_limited.to.Any.All.Le.boundedness.apply(Eq[1], delta=δ_1, var='B')

    B = Eq[-1].variables[1]
    assert B > 0
    Eq << Eq[-2].subs(ε, ε / B)

    Eq << Algebra.Any_All.Any_All.to.Any.All.And.limits_Intersect.apply(Eq[-1], Eq[-2])

    Eq << Eq[-1].this.expr.expr.apply(Algebra.Lt.Lt.to.Lt.Abs.Mul)
