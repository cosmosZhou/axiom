...
def prove(Eq):
    ...
    Eq.lt_zero = Sets.In_Interval.to.Gt.apply(Eq[-1]).this.apply(Algebra.Gt.to.Lt_0)

    Eq << Algebra.Cond.Imply.to.Imply.And.lhs.apply(Eq[-1], Eq[-2])

    Eq << Eq[-1].this.lhs.apply(Sets.In.In.of.And.In, stop=True)
    ...
    Eq << Eq[-1].this.rhs.apply(Sets.is_negative.Le.to.Ge.Div)

    Eq << Algebra.Imply.to.All.apply(Eq[-1])

    Eq << Sets.Lt_0.to.is_negative.apply(Eq.lt_zero, simplify=None)
    ...
    Eq << Eq[-1].this.expr.apply(Calculus.is_extended_real.All_Ge.to.Ge_0.Limit.one_sided)
