...
def prove(Eq):
    ...
    Eq << Eq[-1].this.expr.apply(Algebra.Eq.Cond.given.And.subs, swap=True)
    ...
    Eq << Eq[-1].this.expr.apply(Set.Mem.given.Mem.Add, f(b) - f(a))

    Eq << Set.All.And.of.All.apply(Eq[2], simplify=None)

    Eq << Eq[-1].this.find(Unequal).apply(Set.Gt_0.of.Icc_Ne_EmptySet, simplify=None)

    Eq << Eq[-1].this.expr.apply(Set.Mem.Mul.of.Gt_0.Mem)
