...
def prove(Eq):
    ...
    Eq <<= Set.Mem.Icc.of.Le.Ge.apply(Eq[-4], Eq[-1])

    Eq << Algebra.Or.of.All.subs.apply(Eq.intermediate_value, Eq.intermediate_value.rhs, Eq[-1].lhs)

    Eq << Algebra.Any.Or.of.Or.apply(Eq[-1], simplify=None)

    Eq << Algebra.Any.And.of.Cond.Any.apply(Eq[-1], Eq[-3], simplify=None)
