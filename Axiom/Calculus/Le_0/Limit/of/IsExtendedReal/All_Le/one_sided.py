...
def prove(Eq):
    ...
    Eq << Eq[-1].this.expr.apply(Algebra.All.of.All.limits.restrict, domain=Interval.open(x0, x0 + Min(delta, δ_quote)))

    Eq << Algebra.All.of.All.limits.restrict.apply(Eq[1], domain=Interval.open(x0, x0 + Min(delta, δ_quote)))
