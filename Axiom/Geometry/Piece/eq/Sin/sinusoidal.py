...
def prove(Eq):
    ...
    Eq << Eq[0].this.rhs.find(Mod).apply(Algebra.Mod.eq.Piece)

    Eq << Eq[-1].this.find(Mul[Piecewise]).apply(Algebra.Mul.eq.Piece, simplify=None)

    Eq << Eq[-1].this.find(Floor).apply(Algebra.Floor.eq.Piece, simplify=None)
    ...
    Eq << Eq[-1].this.find(Pow[Piecewise]).apply(Algebra.Pow.eq.Piece.exponent, simplify=None)

    Eq << Eq[-1].this.find(Mul[Piecewise]).apply(Algebra.Mul.eq.Piece, simplify=None)


    Eq << Eq[-1].this.find(Add[Piecewise]).apply(Algebra.Add.Piece.eq.Piece, simplify=None)
    Eq << Eq[-1].this.rhs.apply(Geometry.Sin.eq.Piece, simplify=None)
