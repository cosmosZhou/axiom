...
def prove(Eq):
    ...
    Eq << Eq[-1].this.lhs.apply(Geometry.Sin.eq.Piece)

    Eq << Eq[-1].this.find(Sin[Piecewise]).apply(Geometry.Sin.eq.Piece)

    Eq << Eq[-1].this.find(Sin[Piecewise]).apply(Geometry.Sin.eq.Piece, simplify=0)
