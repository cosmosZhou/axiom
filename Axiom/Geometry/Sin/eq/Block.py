...
def prove(Eq):
    ...
    Eq << Eq[-1].this.lhs.apply(Geometry.Sin.eq.Ite)

    Eq << Eq[-1].this.find(Sin[Piecewise]).apply(Geometry.Sin.eq.Ite)

    Eq << Eq[-1].this.find(Sin[Piecewise]).apply(Geometry.Sin.eq.Ite, simplify=0)
