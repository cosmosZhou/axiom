...
def prove(Eq):
    ...
    Eq << Eq[-1].this.lhs.apply(Geometry.Cos.eq.Ite)

    Eq << Eq[-1].this.find(Cos[Piecewise]).apply(Geometry.Cos.eq.Ite)

    Eq << Eq[-1].this.find(Cos[Piecewise]).apply(Geometry.Cos.eq.Ite, simplify=0)
