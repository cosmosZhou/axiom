...
@prove
def prove(Eq):
    from Axiom import Discrete, Algebra, Set, Geometry, Logic
    ...
    Eq << Set.Mem.Icc.of.Le.Gt.apply(Eq[-1], Eq.gt_zero_eta)

    Eq << Geometry.Gt_0.Sin.of.Mem_Icc.apply(Eq[-1])

    Eq.gt_zero = Algebra.All.of.Cond.apply(Eq[-1], j)
    ...
    Eq << Algebra.Cond.of.Cond.Cond.subs.apply(Eq.ne, Eq.Sum_ge_piece)

    Eq << Eq[-1].this.rhs.find(Sum).apply(Algebra.Sum.eq.Mul.series.geometric)

    Eq << Eq[-1].this.find(Piecewise).apply(Logic.Ite__Ite.eq.IteAnd_Not__Ite)

    Eq << Algebra.Cond.of.Cond.Cond.subs.apply(Eq.ne4, Eq[-1])
    ...
    Eq << Geometry.LeSin.of.Gt_0.apply(Eq.gt_zero_eta)
    ...
    Eq << Algebra.Cond.of.Cond.Cond.subs.apply(Eq.ne, Eq[-1])

    Eq << Eq[-1] * (2 * ReducedMax(x ** 2))

    Eq.Sum_le = Algebra.Le.of.Le.Le.apply(Eq.Sum_le, Eq[-1])

    Eq << Set.Mem.Icc.of.Ge.Le.apply(Eq.Sum_ge, Eq.Sum_le)
    ...
    Eq << Set.Mem.Div.of.Gt_0.Mem.apply(Eq[-1], Eq[-3])
