...
def apply(eq_h_embed, eq_h_global_in, eq_h_global_out, eq_h_local_in, eq_h_local_out, eq_prob):
    ...
    ((((x, S[P * k + p]), S[x[P * k + p].var]), S[x[:P * k + p].as_boolean()]), [S[x[P * k + p].var]]), (S[E_local_embed], S[h_local_out[k, p]]) = eq_prob.of(Equal[Lamda[Pr[Conditioned[Equal[Indexed]]]], Softmax[MatMul]])
    assert x.var == x_var
...
def prove(Eq):
    ...
    *Eq[-6:], (Eq.h_global_in, Eq.h_local_in, Eq.prob) = apply(
        ...
        Equal(Lamda[x.var[k * P + p]](Pr(x[k * P + p] | x[:k * P + p])), softmax(E_local_embed @ h_local_out[k, p])),
        )
