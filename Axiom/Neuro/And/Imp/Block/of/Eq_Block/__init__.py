...
def apply(eq):
    ...
    return Imply(i < Min(l, n, n - Min(u, n)), Equal(z[i], BlockMatrix(-oo * OneMatrix(Min(l, n) - i - 1), z[i, Min(l, n) - i - 1:]))), \
        Imply(Element(i, Range(n - Min(u, n), Min(l, n))), Equal(z[i], BlockMatrix(-oo * OneMatrix(Min(l, n) - i - 1), z[i, Min(l, n) - i - 1:n + Min(l, n) - i - 1], -oo * OneMatrix(Min(u, n) + i - n)))), \
        Imply(i >= Max(Min(l, n), n - Min(u, n)), Equal(z[i], BlockMatrix(z[i,:n + Min(l, n) - i - 1], -oo * OneMatrix(Min(u, n) + i - n)))), \
...
def prove(Eq):
    ...
    Eq << Eq[-1].this.expr.rhs.apply(Algebra.Ite.eq.Add, Eq[-1].find(logsumexp))
