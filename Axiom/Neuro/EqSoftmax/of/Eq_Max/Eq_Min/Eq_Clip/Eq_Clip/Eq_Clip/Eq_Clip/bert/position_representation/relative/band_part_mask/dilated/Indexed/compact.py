...
def prove(Eq):
    ...
    (Eq.beta, Eq.zeta, Eq.K_quote, Eq.V_quote, Eq.K_dquote, Eq.V_dquote), Eq.objective = apply(
        ...
        Equal(K_dquote, Lamda[j:Ceil(Min(n, l + u - 1) / d), i:n](w_K[k + clip(r[Min(n - 1, β[i] + d * j)] - r[i], -k, k)])),
        Equal(V_dquote, Lamda[j:Ceil(Min(n, l + u - 1) / d), i:n](w_V[k + clip(r[Min(n - 1, β[i] + d * j)] - r[i], -k, k)])),
        Q, K, V)
