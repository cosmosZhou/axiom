...
def prove(Eq):
    ...
    Eq << Neuro.All.Eq.of.Eq_Block.Eq_Block.relative_distance.lower_triangle.upper_part.tf.apply(Eq[0], Eq[1])

    Eq << Algebra.EqLamda.of.All_Eq.apply(Eq[-1].this.expr.reversed, Eq[2].find(Lamda).expr)

    Eq << Neuro.All.Eq.of.Eq_Block.Eq_Block.relative_distance.lower_triangle.lower_part.tf.apply(Eq[0], Eq[1])

    Eq << Algebra.EqLamda.of.All_Eq.apply(Eq[-1].this.expr.reversed)

    Eq << Neuro.Eq.of.Eq_Block.Eq_Block.relative_distance.lower_triangle.apply(Eq[0], Eq[1])

    Eq << Eq[2].subs(Eq[-1].reversed, Eq[-2], Eq[-4])

    Eq << Eq[-1].this.find(Lamda + Lamda).apply(Algebra.Add.eq.Lamda)

    Eq << Eq[-1].this.find(-~Min).apply(Neuro.Min.eq.Add.Relu, index=1)
    ...
    Eq << Neuro.Softmax.eq.Block.of.Eq_Sub_Lamda_Mul_LogSumExp.lower_triangle.tf.apply(Eq[-1])
