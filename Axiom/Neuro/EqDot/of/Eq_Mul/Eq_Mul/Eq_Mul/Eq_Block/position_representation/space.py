...
def extract(eq_theta_r, eq_theta_c, eq_theta_z, eq_R):
    from Axiom.Neuro.EqDot.of.Eq_Mul.position_representation import extract_theta
...
def prove(Eq):
    ...
    Eq << Neuro.EqDot.of.Eq_Mul.position_representation.apply(Eq[0], i_quote)

    Eq << Neuro.EqDot.of.Eq_Mul.position_representation.apply(Eq[1], j_quote)

    Eq << Discrete.EqDot.of.EqDot.EqDot.apply(*Eq[-2:])
    ...
    Eq << Neuro.EqDot.of.Eq_Mul.position_representation.apply(Eq[2], k_quote)

    Eq << Discrete.EqDot.of.EqDot.EqDot.apply(*Eq[-2:])

    Eq << Eq[-1].rhs.args[1].find(ZeroMatrix).this.apply(Algebra.Expr.eq.Block, d_z / 2, d_r)

    Eq << Eq[-1].T

    Eq.rect_r = Eq[-2].rhs.find(ZeroMatrix).this.apply(Algebra.Expr.eq.Block, d_r / 2, axis=1)

    Eq.rect_r_T = Eq.rect_r.T

    Eq.rect_c = Eq[-2].rhs.find(ZeroMatrix[2]).this.apply(Algebra.Expr.eq.Block, d_c / 2, axis=1)
    ...
    Eq << Discrete.EqDot.of.Eq.apply(Eq[-1], D_c)
    ...
    Eq.eq_matmul = Algebra.Eq.of.Eq.Eq.apply(Eq.eq_matmul, Eq[-1])
    ...
    Eq << Eq[-1].rhs.args[1].find(ZeroMatrix).this.apply(Algebra.Expr.eq.Block, d_r / 2, axis=1)
    ...
    Eq.eq_matmul = Algebra.Eq.of.Eq.Eq.apply(Eq[-1], Eq.eq_matmul)
    ...
    Eq << Algebra.Eq.of.Eq.Eq.apply(Eq.eq_matmul, Eq[-1])
