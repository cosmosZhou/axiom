...
@prove
def prove(Eq):
    from Axiom import Neuro, Probability, Discrete, Algebra
    ...
    *Eq[-4:], Eq.hypothesis = apply(
    ...
    Eq << Neuro.EqExpect.of.Eq_Conditioned.Eq_Expect.Eq_Expect.Eq_Sub.temporal_difference_residual.apply(*Eq[:4]).reversed

    Eq << Eq[-1].subs(s[t].var, s[t]).subs(a[t].var, a[t])

    Eq << Eq.hypothesis.subs(Eq[-1])

    Eq << Eq[-1].this.rhs.find(Lamda).apply(Probability.Lamda.Expect.eq.Expect.Lamda)
