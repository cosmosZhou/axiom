...
def prove(Eq):
    ...
    Eq << Neuro.Softmax.eq.AddBlockS.of.Eq_Sub_Lamda_Mul_LogSumExp.tf.apply(Eq[1])
    ...
    Eq.ge_neg_min, Eq.lt_min = Set.And.of.Mem_Range.apply(Eq[2])
    ...
    Eq << Algebra.Cond.of.Cond.Cond.subs.apply(Eq.lt, Eq[-1])

    Eq << Eq[-1].this.rhs.apply(Algebra.Add.Ite.eq.Ite)

    Eq << Algebra.EqLog.of.Eq.apply(Eq[-1])

    Eq.loss = -Algebra.EqSum.of.Eq.apply(Eq[3] * (1 + log(1 + abs(h[i]) / 2)), (i, 0, n))
