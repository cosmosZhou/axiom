...
@prove
def prove(Eq):
    from Axiom import Neuro, Algebra, Set
    ...
    Eq << Neuro.Softmax.eq.Block.of.Eq_Sub_Lamda_Mul_LogSumExp.upper_triangle.tf.apply(Eq[1])
    ...
    Eq.ge_zero, Eq.lt_min = Set.And.of.Mem_Range.apply(Eq[2])
    ...
    Eq << Algebra.Cond.of.Cond.Cond.subs.apply(Eq.lt, Eq.eq)

    Eq << Eq[-1].this.find(Less) - i

    Eq << Algebra.Cond.of.Cond.Cond.subs.apply(Eq.ge_zero, Eq[-1], invert=True)

    Eq << Algebra.EqLog.of.Eq.apply(Eq[-1])

    Eq.loss = -Algebra.EqSum.of.Eq.apply(Eq[3] * (1 + log(1 + h[i] / 2)), (i, 0, n))
