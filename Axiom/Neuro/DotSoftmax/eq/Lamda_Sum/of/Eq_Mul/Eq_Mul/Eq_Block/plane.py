...
def apply(eq_theta_r, eq_theta_c, eq_R, Q, K, V, r, c, t):
    from Axiom.Neuro.EqDot.of.Eq_Mul.Eq_Mul.Eq_Block.position_representation.plane import extract
...
def prove(Eq):
    ...
    from Axiom.Neuro.EqDot.of.Eq_Mul.Eq_Mul.Eq_Block.position_representation.plane import rotary_matrix
    ...
    Eq << Neuro.EqDot.of.Eq_Mul.Eq_Mul.Eq_Block.position_representation.plane.apply(*Eq[:3], i_quote, j_quote)

    Eq << Eq[-1].subs(i, r[t]).subs(j, c[t]).subs(i_quote, r[i]).subs(j_quote, c[i])

    Eq.final = Eq[-3].subs(Eq[-1])

    Eq << Neuro.Dot.eq.Add.of.Eq_Mul.Eq_Mul.Eq_Block.position_representation.plane.apply(*Eq[:3], K[t], r, c)
