...
def prove(Eq):
    ...
    Eq <<= Algebra.All.of.Cond.restrict.apply(Eq[-1], (i, 0, n), simplify=None)
    ...
    Eq.V_relu = Eq[-1].this.rhs.find(-~Min).apply(Neuro.Min.eq.Add.Relu, index=1)
    ...
    Eq << Algebra.Eq.of.Eq.Eq.apply(Eq.V_relu, Eq[-1])
