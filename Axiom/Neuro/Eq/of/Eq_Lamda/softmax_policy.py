...
@apply
def apply(eq_given):
    ((((a, S[a.var]), (s, S[s.var])), [S[a], θ]), (S[a.var], S[0], m)), (phi_s, S[θ]) = eq_given.of(Equal[Lamda[Pr[Conditioned[Equal[2]]]], Softmax[MatMul]])
    return Equal(
...
def prove(Eq):
    ...
    Eq << apply(Equal(Lamda[a.var:m](Pr[a:θ](a | s)), softmax(φ(s.var) @ θ)))
