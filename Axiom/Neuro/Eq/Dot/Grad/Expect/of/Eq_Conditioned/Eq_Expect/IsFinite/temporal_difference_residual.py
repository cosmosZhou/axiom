...
def apply(eq, V_def, lt):
    ...
    return Equal(γ ** Lamda[t](t) @ Derivative[π](Expectation[r, a:π](r)),
                 γ ** Lamda[t](t) @ Lamda[t](Expectation[r, a:π, s](Derivative[π](log(Pr[a:π](a[t].surrogate | s[t].surrogate))) * At)))


@prove
def prove(Eq):
    from Axiom import Neuro, Probability
    ...
    *Eq[-3:], Eq.hypothesis = apply(
    ...
    Eq << Neuro.Eq.Dot.Grad.Expect.of.Eq_Conditioned.Eq_Expect.Eq_Expect.Eq_Sub.IsFinite.A_Function.apply(Eq[0], Eq.Q_Function, Eq[1], Eq.A_Function, Eq[2])

    Eq << Neuro.EqExpect.of.Eq_Conditioned.Eq_Expect.Eq_Expect.Eq_Sub.temporal_difference_residual.apply(Eq[0], Eq.Q_Function, Eq[1], Eq.A_Function).reversed
