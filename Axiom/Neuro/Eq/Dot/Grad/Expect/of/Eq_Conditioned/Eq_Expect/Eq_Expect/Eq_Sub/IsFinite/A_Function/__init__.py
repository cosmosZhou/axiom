...
def apply(eq, Q_def, V_def, A_def, lt):
    ...
    return Equal(γ ** Lamda[t](t) @ Derivative[π](Expectation[r, a:π](r)),
                 γ ** Lamda[t](t) @ Lamda[t](Expectation[a:π, s](Derivative[π](log(Pr[a:π](a[t].surrogate | s[t].surrogate))) * A_st_var._subs(s[t].var, s[t])._subs(a[t].var, a[t]))))


@prove
def prove(Eq):
    from Axiom import Neuro, Algebra, Probability
    ...
    *Eq[-5:], Eq.hypothesis = apply(
    ...
    Eq << Neuro.Eq.Dot.Grad.Expect.of.Eq_Conditioned.Eq_Expect.Eq_Expect.IsFinite.Q_Function.apply(*Eq[:3], Eq[4])
