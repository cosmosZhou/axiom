...
def prove(Eq):
    ...
    Eq <<= Algebra.EqLamda.of.Eq.apply(Eq[-2], (j, 0, m)), Algebra.EqLamda.of.Eq.apply(Eq[-1], (j, 0, m))

    Eq <<= Algebra.EqTranspose.of.Eq.apply(Eq[-2], (0, 1)), Algebra.EqTranspose.of.Eq.apply(Eq[-1], (0, 1))

    Eq <<= Algebra.Eq.of.Eq.Eq.apply(Eq[-2], Eq[4]), Algebra.Eq.of.Eq.Eq.apply(Eq[-1], Eq[5])

    Eq << Algebra.EqTranspose.of.Eq.apply(Eq[-2], (1, 1))
    Eq << Neuro.Eq.Dot.Softmax.of.Eq_Card.Subset_Cup.position_representation.relative.gather.apply(Eq[0], Eq[1], Q, K, V, K_quote, V_quote)
