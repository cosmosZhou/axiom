...
def prove(Eq):
    ...
    Eq << Eq[-1].lhs.this.apply(Neuro.Dot.Softmax.eq.Lamda.Sum.scaled_dot_product_attention, i, j, k)

    Eq << Eq[-1].this.find(Sum).apply(Algebra.Sum.limits.domain_defined, simplify=None)

    Eq << Neuro.EqDot.of.Eq_Mul.Eq_Block.position_representation.rotary.apply(*Eq[:2], i)

    Eq.final = Eq[-2].subs(Eq[-1])

    Eq << Neuro.Dot.eq.Add.of.Eq_Mul.Eq_Block.position_representation.rotary.apply(*Eq[:2], K[i])
    ...
    Eq << Algebra.Eq.of.Eq.Eq.apply(Eq[-1], Eq.matmul_QRK)
