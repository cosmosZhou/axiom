...
@prove
def prove(Eq):
    from Axiom import Discrete, Algebra, Calculus, Probability, Neuro
    ...
    *Eq[-4:], Eq.hypothesis = apply(Equal(r[t] | s[:t] & a[:t], r[t]), # history-irrelevant conditional independence assumption for rewards based on states and actions
    ...
    Eq << Algebra.All.of.Cond.apply(Eq[-1], k, simplify=None)
    ...
    Eq << Neuro.Eq.Dot.Grad.Expect.of.Eq_Conditioned.IsFinite.policy_gradient_theorem.apply(Eq[0], Eq[-1])
