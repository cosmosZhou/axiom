...
import Axiom.Algebra.Ne_.Length.Zero.IsConstantMap.to.Eq_.ProdConsSumMap.Length
  ...
  if h_ne_zero: s.length ≠ 0 then
    ⟨
      ...
      ⟨
        ...
        (Algebra.Ne_.Length.Zero.IsConstantMap.to.Eq_.ProdConsSumMap.Length h_ne_zero h).symm
      ⟩
    ⟩
  else
    default
