...
def prove(Eq):
    ...
    Eq <<= Sets.In_Range.of.And.apply(Eq[0]), Sets.In_Range.of.And.apply(Eq[1]), Sets.In_Range.to.And.apply(Eq[-1])

    Eq <<= Algebra.Ge_Max.to.And.Ge.apply(Eq[-2]), Algebra.Lt_Min.to.And.Lt.apply(Eq[-1])

    Eq << Algebra.Le.to.Lt.relax.apply(Eq[-2].reversed + n, lower=i)

    Eq << Algebra.Lt.to.Le.strengthen.apply(Eq[-1]).reversed + d
