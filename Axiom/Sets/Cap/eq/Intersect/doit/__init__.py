...
def prove(Eq):
    ...
    n -= 1
    Eq << Eq[-1].this.lhs.apply(Sets.Cap.eq.Intersect.split, {n})

    n -= 1
    Eq << Eq[-1].find(Cap).this.apply(Sets.Cap.eq.Intersect.split, {n})

    n -= 1
    Eq << Eq[-1].rhs.find(Cap).this.apply(Sets.Cap.eq.Intersect.split, {n})
