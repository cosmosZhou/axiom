...
def prove(Eq):
    ...
    Eq << Cup[y:F(x)](Piecewise((f(x, y), Element(x, A)), (g(x, y), True))).this.apply(Sets.Cup.eq.Piece)

    Eq << Sets.Eq.to.Eq.Cup.apply(Eq[-1], (x, B))

    Eq << Eq[-1].this.rhs.apply(Sets.Cup.eq.Union.single_variable)
