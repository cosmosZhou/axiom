...
def prove(Eq):
    ...
    Eq << Sets.Lt_0.to.Cup.eq.Interval.right_open.apply(Eq[1], k)

    Eq << Eq[-2].subs(Eq[-1])

    Eq << Eq[4].subs(Eq[-1], Eq[-4]).reversed

    Eq << Sets.Lt_0.to.Cup.eq.Interval.right_open.apply(Eq[0], k)
    ...
    Eq << Eq[-1].this.find(Expr <= 0).apply(Algebra.Le.to.Eq.Min)
