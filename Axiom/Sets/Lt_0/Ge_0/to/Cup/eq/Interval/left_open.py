...
def prove(Eq):
    ...
    Eq <<= Sets.Ge_0.to.Cup.eq.Interval.left_open.apply(Eq[1], k), Sets.Lt_0.to.Cup.eq.Interval.left_open.apply(Eq[0], k)

    Eq <<= Eq[-4].subs(Eq[-2]), Eq[-3].subs(Eq[-1])

    Eq << Eq[3].subs(Eq[-1], Eq[-2])

    Eq << Sets.Lt.Le.to.Eq.Union.Interval.left_open.apply(Eq[0], Eq[1].reversed, left_open=True)
