...
def prove(Eq):
    ...
    Eq << Stats.Expect.le.ExpectAbs.apply(Eq[0].lhs.find(Expectation))

    Eq << Stats.Expect.le.ExpectAbs.apply(Eq[0].lhs.find(Expectation)._subs(f(x), -f(x)))

    Eq << -Eq[-1].this.lhs.apply(Stats.Expect.eq.Mul)

    Eq << Algebra.Le.Ge.to.Le.Abs.apply(Eq[-3], Eq[-1])
