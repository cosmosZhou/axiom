...
def prove(Eq):
    ...
    Eq.x_marginal_probability = Stats.Integral.eq.Prob.marginal.apply(Integral[x.var](Probability(x, y)))
    ...
    Eq <<= Calculus.Gt.to.Gt.Integral.apply(Eq[-1], (y.var,)), \
        Calculus.Gt.to.Gt.Integral.apply(Eq[-1], (x.var,))

    Eq <<= Eq[-2].subs(Eq.y_marginal_probability), Eq[-1].subs(Eq.x_marginal_probability)

    Eq <<= Algebra.Gt.to.Ne.apply(Eq[-1]), Algebra.Gt.to.Ne.apply(Eq[-2])
