...
def prove(Eq):
    ...
    Eq << Eq[-1].lhs.this.apply(Stats.Prob.eq.Div.Prob.bayes)

    Eq << Eq[-1].rhs.find(Probability).this.apply(Stats.Prob.eq.Integral)
    ...
    Eq << Eq.plausible.rhs.find(Probability).this.apply(Stats.Prob.eq.Integral)

    Eq << Eq[-1].this.rhs.find(Probability).apply(Stats.Prob.eq.Integral.joint, x)
