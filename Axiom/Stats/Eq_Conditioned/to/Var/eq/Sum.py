...
def prove(Eq):
    ...
    Eq << Eq[-1].this.rhs.apply(Stats.Var.Sum.eq.Add.Sum)

    Eq.eq_var = Eq[-1].this.find(Sum[2]).apply(Algebra.Sum.eq.Add)

    j, i = Eq.eq_var.find(Sum[2]).variables
    Eq << Stats.Eq_Conditioned.to.Imply.Eq_0.Cov.apply(Eq[0], j)
    ...
    Eq << Algebra.Imply.to.Eq.Piece.apply(Eq.infer, Eq[-1].find(Piecewise))
