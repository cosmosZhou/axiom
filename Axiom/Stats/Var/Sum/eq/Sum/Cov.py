...
def prove(Eq):
    ...
    Eq << Eq[-1].this.lhs.apply(Stats.Var.Add.eq.Add.Cov)

    Eq << Eq[-1].this.find(Covariance).apply(Stats.Cov.Sum.eq.Sum.Cov)

    Eq << Eq[-1].this.rhs.find(Sum).apply(Algebra.Sum.eq.Add.pop)

    Eq << Eq[-1].this.lhs.find(Sum[Covariance]).limits_subs(i, j)

    Eq << Eq[-1].this.find(Variance).apply(Stats.Var.eq.Cov)
