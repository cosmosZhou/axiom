...
*/
function get_rows($sql, $arg = MYSQLI_ASSOC) {//the other choice is MYSQLI_NUM
	if (is_object($arg)) {
	    $conn = $arg;
...
function upload_error($error) {
	...
	return ($error ? 'Unable to upload a file.' . ($max_size ? " " . lang('Maximum allowed file size is %sB.', $max_size) : "") : 'File does not exist.');
}
...
* @return bool
*/
function hidden_fields(&$process, $prefix = '') {
	$return = [];
	foreach ($process as $key => $val) {
		if (is_array($val)) {
		    array_push($return, ...hidden_fields($val, $key));
		} else {
		    $return[] = [h($prefix ? "${prefix}[$key]" : $key), h($val)];
		}
	}
	return $return;
}
