...
*/
function echo_select($result, $connection2 = null, $orgtables = array(), $limit = 0) {
	global $jush;
...
<td id="label-type"><?php echo 'Type'; ?><textarea id="enum-edit" rows="4" cols="12" wrap="off" style="display: none;"></textarea><?php echo script("qs('#enum-edit').onblur = editingLengthBlur;"); ?>
<td id="label-length"><?php echo 'Length'; ?>
<td><?php echo 'Options'; /* no label required, options have their own label */ ?>
<?php if ($type == "TABLE") { ?>
<td id="label-null">NULL
<td><input type="radio" name="auto_increment_col" value=""><acronym id="label-ai" title="<?php echo 'Auto Increment'; ?>">AI</acronym><?php echo doc_link(array(
...
<td id="label-default"<?php echo $default_class; ?>><?php echo 'Default value'; ?>
<?php echo (support("comment") ? "<td id='label-comment'$comment_class>" . 'Comment' : ""); ?>
<?php } ?>
<td><?php echo "<input type='image' class='icon' name='add[" . (support("move_col") ? 0 : count($fields)) . "]' src='./static/plus.gif' alt='+' title='" . 'Add next' . "'>" . script("row_count = " . count($fields) . ";"); ?>
</thead>
<tbody>
<?php
...
<td><label class="block"><input type="radio" name="auto_increment_col" value="<?php echo $i; ?>"<?php if ($field["auto_increment"]) { ?> checked<?php } ?> aria-labelledby="label-ai"></label><td<?php echo $default_class; ?>><?php
		...
		echo (support("move_col") ?
			"<input type='image' class='icon' name='add[$i]' src='./static/plus.gif' alt='+' title='" . 'Add next' . "'> "
			. "<input type='image' class='icon' name='up[$i]' src='./static/up.gif' alt='↑' title='" . 'Move up' . "'> "
			. "<input type='image' class='icon' name='down[$i]' src='./static/down.gif' alt='↓' title='" . 'Move down' . "'> "
		: "");
		echo ($orig == "" || support("drop_col") ? "<input type='image' class='icon' name='drop_col[$i]' src='./static/cross.gif' alt='x' title='" . 'Remove' . "'>" : "");
	}
}
