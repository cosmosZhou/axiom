...
def prove(Eq):
    ...
    Eq << apply(Equal(R, rotary_matrix(n, d, b, i, j, k)), x[i])
    ...
    Eq << Eq[-1].this.rhs.expr.args[1:3].apply(algebra.add_piece.to.piece, simplify=None)

    Eq << Eq[-1].this.rhs.expr.args[::2].apply(algebra.add_piece.to.piece, simplify=None)

    Eq << Eq[-1].this.rhs.expr.apply(algebra.add_piece.to.piece, simplify=None)
