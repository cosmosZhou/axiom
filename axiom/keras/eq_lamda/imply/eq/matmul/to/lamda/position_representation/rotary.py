...
def rotary_matrix(n, d, b, i, j, k):
    return Lamda[k:d, j:d, i:n](
        Piecewise(
            (Piecewise(
...
def extract(eq_R):
    ...
    S[i], (b, (S[j], S[d])) = even_expr.of(Expr * Expr ** (-Expr / Expr))
...
def prove(Eq):
    ...
    Eq << Eq[1].this.rhs.expr.apply(algebra.piece.to.add.piece, [-1, slice(1, None)])
