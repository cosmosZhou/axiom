...
@apply
def apply(eq_R, eq_T, V):
    (R, n, d), b, (i, j, k) = extract(eq_R)
    
    ((((((K, S[j], S[2 * k]), (Q, S[i], S[2 * k])), S[K[j, 2 * k + 1] * Q[i, 2 * k + 1]]), S[b ** (-2 * k / d) * (-i + j)]), (S[K[j, 2 * k] * Q[i, 2 * k + 1] - Q[i, 2 * k] * K[j, 2 * k + 1]], S[b ** (-2 * k / d) * (-i + j)])), (S[k], S[0], S[d / 2])), (T, S[i], S[j]) = \
    eq_T.of(Equal[Sum[(Indexed * Indexed + Expr) * Cos + Expr * Sin], Indexed])
    return Equal(
...
def prove(Eq):
    ...
    Eq << apply(
        ...
        Equal(T[i, j], Sum[k:d / 2]((K[j, 2 * k] * Q[i, 2 * k] + K[j, 2 * k + 1] * Q[i, 2 * k + 1]) * cos(b ** (-2 * k / d) * (-i + j)) + (K[j, 2 * k] * Q[i, 2 * k + 1] - Q[i, 2 * k] * K[j, 2 * k + 1]) * sin(b ** (-2 * k / d) * (-i + j)))),
        V)

    Eq << Eq[2].lhs.this.apply(keras.matmul_softmax.to.lamda.sum.scaled_dot_product_attention, i, j, k)

    Eq << Eq[-1].this.find(Sum).apply(algebra.sum.limits.domain_defined.insert, simplify=None)

    Eq << keras.eq_lamda.imply.eq.matmul.to.piece.position_representation.rotary.apply(Eq[0]).subs(j, k)
    ...
    Eq <<= keras.eq_lamda.imply.eq.matmul.to.lamda.position_representation.rotary.apply(Eq[0], K[k]), \
        keras.eq_lamda.imply.eq.matmul_transpose.to.lamda.position_representation.rotary.apply(Eq[0], K[k])
    ...
    Eq <<= Eq[-2].this.rhs.expr.args[1:].apply(algebra.add.to.mul), Eq[-1].this.rhs.expr.args[1:].apply(algebra.add.to.mul)

    Eq <<= algebra.eq.eq.imply.eq.transit.apply(Eq[1].subs(j, k), Eq[-2])

    Eq <<= Eq[-2].this.find(Cos).apply(geometry.cos.neg)

    Eq <<= Eq[-1].this.find(Sin).apply(geometry.sin.to.neg.sin)

    Eq <<= algebra.eq.eq.imply.eq.transit.apply(Eq[1].subs(j, k), Eq[-1])
