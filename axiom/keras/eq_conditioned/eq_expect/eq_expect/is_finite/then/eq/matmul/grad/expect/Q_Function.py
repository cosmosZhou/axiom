...
def prove(Eq):
    ...
    *Eq[-4:], Eq.hypothesis = apply(
    ...
    Eq.expect = keras.eq_conditioned.eq_expect.eq_expect.then.eq.grad.expect.policy_gradient.apply(*Eq[:3], n)

    Eq << calculus.eq.then.eq.limit.apply(Eq.expect, (n, oo))

    Eq.limit = Eq[-1].this.find(Limit).apply(calculus.limit.to.add)

    Eq << stats.then.abs_le.expect.abs.apply(Eq.limit.find(Expectation[Derivative]).subs(n, t))

    Eq << stats.then.expect_le.sup.apply(Eq[-1].rhs)

    Eq.le_sup = algebra.le.le.then.le.trans.apply(Eq[-2], Eq[-1])

    Eq << algebra.then.all.le_sup.apply(Eq[3].lhs)

    Eq << algebra.cond.then.all.apply(Eq[-1], s[t].var, simplify=None)

    Eq << algebra.all_le.then.sup_le.apply(Eq[-1])

    Eq << algebra.le.le.then.le.trans.apply(Eq[-1], Eq.le_sup)

    Eq << algebra.cond.then.all.apply(Eq[-1], t, simplify=None)

    Eq << algebra.all_le.then.sup_le.apply(Eq[-1])

    Eq << algebra.le.lt.then.lt.trans.apply(Eq[3], Eq[-1])
    ...
    Eq << calculus.abs_lt.is_finite.then.is_zero.limit.apply(Eq[-2], Eq[-1])
