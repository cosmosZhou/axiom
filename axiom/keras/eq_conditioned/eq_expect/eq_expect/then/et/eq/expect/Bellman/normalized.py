...
def prove(Eq):
    ...
    Eq << keras.eq_expect.then.eq.expect.V_Function.normalized.apply(Eq[1])

    Eq << algebra.eq.eq.then.eq.trans.apply(Eq[2], Eq[-1])

    Eq << stats.eq_conditioned.then.eq.conditioned.getitem.apply(Eq[0], 1)

    Eq << keras.eq_conditioned.then.eq.expect.Bellman.V_Function.normalized.apply(Eq[-1], γ, t)
    ...
    Eq << keras.eq_conditioned.then.eq.expect.Bellman.Q_Function.normalized.apply(Eq[0], γ, t)
