...
def apply(eq, Q_def, V_def):
    ...
    return Equal(Derivative[π](V_st_var),
                 ...
                 γ * Sum[s[t + 1].var](Derivative[π](V_st_var._subs(t, t + 1)) * Probability(s[t + 1] | s[t])))
...
def prove(Eq):
    ...
    Eq << keras.eq_conditioned.eq_expect.eq_expect.then.et.eq.expect.Bellman.apply(*Eq[:3])
    ...
    Eq << Eq[-1].this.find(Derivative[Sum]).apply(calculus.grad.to.sum)
    ...
    Eq << calculus.eq.then.eq.grad.apply(Eq[4], [π])
    Eq << Eq[-1].this.find(Expectation).apply(stats.expect.to.sum)
    Eq << Eq[-1].this.rhs.apply(calculus.grad.to.sum)
    Eq << Eq[-1].subs(Eq[-4])
    Eq << Eq[-1].this.rhs.apply(algebra.sum.to.add)
    Eq << Eq[-1].this.find(Probability * Sum).apply(algebra.mul.to.sum)
    ...
    Eq << algebra.cond.then.cond.domain_defined.apply(Eq[0]).subs(t, t + 1)
    Eq << stats.ne_zero.then.ne_zero.joint_slice.apply(Eq[-1], [-1, -1])
    Eq << stats.cond.then.cond.prob.weighted.apply(Eq[-1], (a, π))
    Eq << stats.ne_zero.then.eq.prob.conditioned.to.mul.prob.conditioned.bayes.apply(Eq[-1], s[t + 1], a[t])
