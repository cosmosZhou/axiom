...
def apply(eq, Q_def, V_def, n=None):
    ...
    return Equal(Derivative[π](V_st_var._subs(s[t].var, s[0].var)),
                 Sum[t:n](γ ** t * Integral[s[1:t + 1].var](Product[t:t](Probability(s[t + 1] | s[t])) * Sum[a[t].var](Derivative[π](Probability[a:π](a[t] | s[t])) * Q_st_var))) + \
                 γ ** n * Integral[s[1:n + 1].var](Product[t:n](Probability(s[t + 1] | s[t])) * Derivative[π](V_st_var._subs(s[t].var, s[n].var))))
...
def prove(Eq):
    ...
    *Eq[-3:], Eq.hypothesis = apply(
    ...
    Eq.recursion = keras.eq_conditioned.eq_expect.eq_expect.imply.eq.grad.policy_gradient.recursion.apply(*Eq[:3])
