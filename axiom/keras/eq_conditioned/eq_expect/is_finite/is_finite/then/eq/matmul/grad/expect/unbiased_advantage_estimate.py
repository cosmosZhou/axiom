...
def prove(Eq):
    ...
    *Eq[-4:], Eq.hypothesis = apply(Equal(r[t] | s[:t] & a[:t], r[t]), # history-irrelevant conditional independence assumption for rewards based on states and actions
    ...
    Eq << calculus.eq.then.eq.limit.apply(Eq[-1], (k, oo))
    ...
    Eq << algebra.cond.then.all.apply(Eq[-1], k, simplify=None)

    Eq << algebra.all_le.then.sup_le.apply(Eq[-1])

    Eq << algebra.le.lt.then.lt.trans.apply(Eq[-1], Eq[3])
    ...
    Eq << keras.eq_conditioned.is_finite.then.eq.matmul.grad.expect.policy_gradient_theorem.apply(Eq[0], Eq[-1])
