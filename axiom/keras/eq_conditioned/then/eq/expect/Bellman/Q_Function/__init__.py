...
def prove(Eq):
    ...
    Eq << stats.ne_zero.then.ne_zero.joint_slice.apply(Eq[-1], [t, slice(t, t + 2)])

    Eq << Eq[-1].this.find(Equal[Sliced]).apply(algebra.eq.to.et.eq.split)

    Eq << stats.eq_conditioned.then.eq.conditioned.joint.independence_assumption.apply(Eq[0])

    Eq << stats.ne_zero.eq_conditioned.then.eq.conditioned.joint.apply(*Eq[-2:])
