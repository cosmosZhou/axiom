...
def prove(Eq):
    ...
    Eq << algebra.cond_piece.then.et.infer.apply(Eq[-1])

    Eq << Eq[-2].subs(t, n - 1)

    Eq << algebra.infer.then.infer.et.domain_defined.apply(Eq[-1], t)

    Eq << Eq[-1].this.find(Element).apply(sets.el_range.of.et)
