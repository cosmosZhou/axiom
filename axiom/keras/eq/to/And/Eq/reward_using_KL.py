...
def prove(Eq):
    ...
    Eq << Eq[0].this.find(KroneckerDelta).apply(Algebra.Delta.eq.Piece)
    ...
    Eq << Algebra.Imply.to.Imply.And.domain_defined.apply(Eq[-1], t)

    Eq << Eq[-1].this.find(Element).apply(Sets.In_Range.of.And)

    Eq << Eq[-1].this(t).find(GreaterEqual).simplify()

    Eq << Eq[-1].this.find(And).apply(Algebra.Ne.Lt.of.Lt)
