...
def prove(Eq):
    ...
    Eq << keras.eq.eq.then.iff.kmeans.apply(Eq[0], Eq[1], x=x)

    Eq << algebra.iff.then.eq.sum.collapse.apply(Eq[-1], Eq[3].rhs.expr)
    ...
    Eq << keras.eq.eq.then.iff.kmeans.w_quote.apply(Eq[0], Eq[1], x=x)

    Eq << algebra.iff.then.eq.sum.collapse.apply(Eq[-1], Eq.plausible.lhs.expr)
    ...
    Eq << sets.eq_cup.then.all.apply(Eq[-1])

    Eq << Eq[-1].this.expr.apply(algebra.eq_argmin.then.all.le)

    Eq << Eq[-1].this.expr.apply(algebra.le.then.le.square)
    ...
    Eq << algebra.le.of.ge_zero.apply(Eq[-1])

    Eq << Eq[-1].this.lhs.apply(algebra.add.to.sum.sub)

    Eq << Eq[-1].this.lhs.expr.expand()

    Eq << algebra.sum_ge_zero.of.sum_ge_zero.apply(Eq[-1])
