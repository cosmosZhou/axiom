...
from axiom.keras.eq.eq.then.eq.kmeans.nonoverlapping import cluster, mean
...
def prove(Eq):
    ...
    Eq << keras.eq.eq.then.eq.kmeans.nonoverlapping.apply(Eq[0], Eq[1], x=x)

    Eq << keras.eq.eq.then.iff.kmeans.apply(Eq[-2], Eq[-1], x=x)
