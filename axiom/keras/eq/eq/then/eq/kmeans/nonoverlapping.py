...
def prove(Eq):
    ...
    Eq << sets.eq.of.et.infer.apply(Eq[4], wrt=j)
    ...
    Eq << sets.eq.eq.then.eq.intersect.apply(Eq.omega_i_definition0, Eq[-1])

    Eq << Eq[-1].this.find(And).apply(sets.eq.eq.then.el.finiteset)

    Eq << Eq.nonoverlapping.subs(Eq[-1])

    Eq << Eq[-1].this().find(Intersection).simplify()

    Eq << sets.all_is_empty.then.eq.nonoverlapping.intlimit.utility.apply(Eq.nonoverlapping, k)
