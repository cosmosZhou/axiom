...
def prove(Eq):
    ...
    Eq.M_def = Eq[0].this.find(Element).apply(Sets.In_Range.equ.And).this.find(Element).apply(Sets.In_Range.equ.And)
    ...
    Eq << Eq[-1].this.find(Min[Ceiling]).apply(Algebra.Min.eq.Add, 1).this.find(Min[Ceiling]).apply(Algebra.Min.eq.Add, 1)

    Eq << Eq[-1].this.find(Add[Ceiling]).apply(Algebra.Add.Ceiling.eq.Floor).this.find(Add[Ceiling]).apply(Algebra.Add.Ceiling.eq.Floor)

    Eq << Eq[-1].this.find(Min[Floor]).apply(Algebra.Min.eq.Floor).this.find(Min[Floor]).apply(Algebra.Min.eq.Floor)

    Eq << Eq[-1].this.find(-Floor).apply(Algebra.Mul.eq.Ceiling).this.find(-Floor).apply(Algebra.Mul.eq.Ceiling)
    ...
    Eq << Algebra.Eq.to.Eq.Lamda.apply(Eq[-1], (j, 0, n[1]), (i, 0, n[0]), (k, 0, m))
