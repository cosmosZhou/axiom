...
def prove(Eq):
    ...
    Eq << Eq[-1].this.find(Add[Ceiling]).apply(algebra.add.ceiling.to.floor).this.find(Add[Ceiling]).apply(algebra.add.ceiling.to.floor)
    ...
    Eq << algebra.eq.then.eq.lamda.apply(Eq[-1], (j, 0, n[1]), (i, 0, n[0]), (k, 0, m))
