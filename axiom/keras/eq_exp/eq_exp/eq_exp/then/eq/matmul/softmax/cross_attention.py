...
def prove(Eq):
    ...
    Eq << keras.eq_add_block.then.eq.mask.cross_attention.apply(Eq.ksi_def, a)
    ...
    Eq <<= algebra.eq.then.eq.exp.apply(Eq[-3]), algebra.eq.then.eq.exp.apply(Eq[-2]), algebra.eq.then.eq.exp.apply(Eq[-1])

    Eq << Eq[-1] * OneMatrix(d_z)

    Eq.lower_part, Eq.upper_part, Eq.diagonal_part = algebra.eq.eq.then.eq.trans.apply(Eq[-4], Eq[1][i]), \
        algebra.eq.eq.then.eq.trans.apply(Eq[-3], Eq[2][i - h]), \
        algebra.eq.eq.then.eq.trans.apply(Eq[-1], Eq.M_definition)
    ...
    Eq.zi_definition = algebra.eq.cond.then.cond.subs_with_expand_dims.apply(Eq[-1], Eq.zi_definition)
    ...
    Eq << algebra.eq.cond.then.cond.subs_with_expand_dims.apply(Eq.diagonal_part, Eq[-1])
    ...
    Eq << algebra.eq.eq.then.eq.trans.apply(Eq.z_def, Eq[-1])
