...
def prove(Eq):
    ...
    Eq << keras.eq_block.then.eq.mask.cross_attention.apply(Eq.ksi_def, a)
    ...
    Eq <<= algebra.eq.then.eq.exp.apply(Eq[-2]), algebra.eq.then.eq.exp.apply(Eq[-1])

    Eq.lower_part, Eq.upper_part = algebra.eq.eq.then.eq.trans.apply(Eq[-2], Eq[0][i]), \
        algebra.eq.eq.then.eq.trans.apply(Eq[-1], Eq[1][i - h])
