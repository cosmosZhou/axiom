...
def prove(Eq):
    ...
    Eq << keras.eq_mul.eq_mul.eq_mul.eq_block.then.eq.matmul.position_representation.space.apply(*Eq[:4], i_quote, j_quote, k_quote)

    Eq << Eq[-1].subs(i, r[t]).subs(j, c[t]).subs(k, z[t]).subs(i_quote, r[i]).subs(j_quote, c[i]).subs(k_quote, z[i])

    Eq.final = Eq[-3].subs(Eq[-1])

    Eq << keras.eq_mul.eq_mul.eq_mul.eq_block.then.eq.matmul.to.add.position_representation.space.apply(*Eq[:4], K[t], r, c, z)
