...
def prove(Eq):
    ...
    Eq << keras.eq_mul.eq_mul.eq_block.then.eq.matmul.position_representation.plane.apply(*Eq[:3], i_quote, j_quote)

    Eq << Eq[-1].subs(i, r[t]).subs(j, c[t]).subs(i_quote, r[i]).subs(j_quote, c[i])

    Eq.final = Eq[-3].subs(Eq[-1])

    Eq << keras.eq_mul.eq_mul.eq_block.then.eq.matmul.to.add.position_representation.plane.apply(*Eq[:3], K[t], r, c)
