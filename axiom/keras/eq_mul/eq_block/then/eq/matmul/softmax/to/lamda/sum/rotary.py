from util import *
from axiom.keras.eq_mul.then.eq.matmul.position_representation import rotary_matrix, extract
...
def prove(Eq):
    ...
    Eq << keras.eq_mul.eq_block.then.eq.matmul.position_representation.rotary.apply(*Eq[:2], i)

    Eq.final = Eq[-2].subs(Eq[-1])

    Eq << keras.eq_mul.eq_block.then.eq.matmul.to.add.position_representation.rotary.apply(*Eq[:2], K[i])
