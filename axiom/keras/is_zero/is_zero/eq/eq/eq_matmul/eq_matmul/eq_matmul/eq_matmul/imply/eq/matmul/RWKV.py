...
def prove(Eq):
    ...
    Eq <<= algebra.eq.imply.eq.rsolve.apply(Eq[2]), algebra.eq.imply.eq.rsolve.apply(Eq[3])

    Eq <<= Eq[-2].subs(Eq[0]), Eq[-1].subs(Eq[1])

    Eq <<= Eq[-2].this.find(Exp[~Mul]).apply(algebra.mul.negate), Eq[-1].this.find(Exp[~Mul]).apply(algebra.mul.negate)

    Eq <<= Eq[-2].this.find(Exp * Exp).args[1:].apply(algebra.mul.to.exp), Eq[-1].this.find(Exp * Exp).apply(algebra.mul.to.exp)
    ...
    #http://localhost/axiom/index.php?module=keras.matmul_softmax.to.lamda.sum.scaled_dot_product_attention
