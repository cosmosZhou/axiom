...
def apply(ne_zero, eq, γ=None, k=None, weights=None):
    s, S[s.var] = ne_zero.of(Unequal[Probability[Equal], 0])
    ((r, (t, S[oo])), S[s[t].as_boolean()]), S[r[t + 2:]] = eq.of(Equal[Conditioned[Sliced[Symbol, Tuple[Symbol + 2]]]])
    ...
    return Equal(Expectation(Sum[k:0:oo](γ ** k * r[t + k + 1]) | s[t], *limits_curr),
                 Expectation((γ * Expectation(Sum[k:0:oo](γ ** k * r[t + k + 2]) | s[t + 1].surrogate, *limits_next) + r[t + 1]) | s[t], *limits_coeff))
...
def prove(Eq):
    ...
    γ = Symbol(domain=Interval(0, 1, right_open=True)) #Discount factor; penalty to uncertainty of future rewards;
    Eq << apply(
        Unequal(Probability(s), 0),
        Equal(r[t + 2:] | s[t], r[t + 2:]), #conditional independence assumption
        γ, k)
    ...
    Eq << stats.ne_zero.imply.ne_zero.slice.apply(Eq[0], slice(t, t + 2))
