...
def apply(ne_zero, eq, Q_pi_def, V_pi_def, n=None):
    ...
    return Equal(Derivative[θ](V_pi_st_var._subs(s[t].var, s[0].var)),
                 Sum[k:n](γ ** k * Integral[s[1:k + 1].var](Product[t:k](Probability(s[t + 1] | s[t])) * Sum[a[k].var](Derivative[θ](Probability[a:θ](a[k] | s[k])) * Q_pi_st_var._subs(s[t].var, s[k].var)._subs(a[t].var, a[k].var)))) + \
                 γ ** n * Integral[s[1:n + 1].var](Product[t:n](Probability(s[t + 1] | s[t])) * Derivative[θ](V_pi_st_var._subs(s[t].var, s[n].var))))
...
@prove
def prove(Eq):
    from axiom import keras, algebra, calculus
    ...
    *Eq[-4:], Eq.hypothesis = apply(Unequal(Probability[a:θ](s, a), 0),
    ...
    Eq.recursion = keras.ne_zero.eq.eq.eq.imply.eq.policy_gradient.recursion.apply(*Eq[:4])
    ...
    Eq << Eq[-1].this.find(Integral[Add]).apply(calculus.integral.to.add)

    Eq << Eq[-1].this.find(Mul[Add]).apply(algebra.mul.to.add)

    Eq << Eq[-1].this.find(Pow * ~Integral[Mul[Integral]]).apply(calculus.integral.to.mul)

    Eq << Eq[-1].this.find(Symbol ** Symbol * Symbol).powsimp()

    Eq << Eq[-1].this.find(Integral[Mul[Probability]] * Product).apply(calculus.mul.to.integral)

    Eq << Eq[-1].this.find(Probability * Product).args[1:].apply(algebra.mul.to.prod.limits.push)
