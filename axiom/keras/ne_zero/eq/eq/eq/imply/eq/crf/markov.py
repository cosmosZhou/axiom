...
def prove(Eq):
    ...
    Eq << Eq[-1].this.lhs.arg.apply(algebra.et.concat, i=1, j=2)

    Eq << stats.ne_zero.imply.eq.bayes.conditioned.apply(Eq[-3], x[k], y[k])

    Eq << Eq[-1].this.lhs.find(And).apply(algebra.et.concat, i=0, j=1)
    ...
    Eq << stats.ne_zero.eq.eq.imply.eq.prob.conditioned.apply(Eq[-1], Eq.xy_independence, Eq[-2])

    Eq << stats.ne_zero.eq.imply.eq.conditioned.joint.apply(Eq[-1], Eq[0])
    ...
    _, Eq.y_nonzero_assumption = stats.ne_zero.imply.et.ne_zero.apply(Eq.xy_nonzero_assumption)

    Eq <<= Eq[-1] & Eq.y_nonzero_assumption

    Eq.y_joint_y_historic = Eq[-1].this.lhs.arg.apply(algebra.eq.imply.et.eq.split)
    ...
    Eq << stats.ne_zero.eq.imply.eq.conditioned.joint.apply(Eq.y_joint_y_historic, Eq[-1])
