from util import *


def markov_assumptions(x, y):
    # d is the number of output labels
    # n is the length of the sequence
    n, d = x.shape
    [S[n]] = y.shape

    k = Symbol(domain=Range(1, n))
    return Equal(x[k] | x[:k] & y[:k], x[k]), Equal(y[k] | y[:k], y[k] | y[k - 1]), Equal(y[k] | x[:k], y[k]), Unequal(Probability(x, y), 0)


def process_assumptions(x_independence_assumption, y_independence_assumption, xy_independence_assumption, xy_nonzero_assumption):
    x = x_independence_assumption.rhs.base
    y = y_independence_assumption.lhs.lhs.base
    assert y_independence_assumption.lhs.lhs == y_independence_assumption.rhs.lhs

    assert xy_nonzero_assumption.of(Unequal[0]) == Probability(x, y)

    assert xy_independence_assumption.rhs.base == y
    return x, y


@apply
def apply(x_independence_assumption, y_independence_assumption, xy_independence_assumption, xy_nonzero_assumption, t=None):
    x, y = process_assumptions(x_independence_assumption, y_independence_assumption, xy_independence_assumption, xy_nonzero_assumption)
    n, _ = x.shape
    if t is None:
        t = Symbol(integer=True, domain=Range(n))
    i = Symbol(integer=True)
    return Equal(Probability(x[:t + 1], y[:t + 1]),
                    Probability(x[0] | y[0]) * Probability(y[0]) * Product[i:1:t + 1](Probability(y[i] | y[i - 1]) * Probability(x[i] | y[i])))


@prove
def prove(Eq):
    from axiom import algebra, stats

    d, n = Symbol(domain=Range(2, oo))
    x = Symbol(shape=(n, d), real=True, random=True)
    y = Symbol(shape=(n,), domain=Range(d), random=True)
    (Eq.x_independence, Eq.first_order_markov_assumption, Eq.xy_independence, Eq.xy_nonzero_assumption), Eq.factorization = apply(*markov_assumptions(x, y))

    x = Eq.x_independence.rhs.base
    y, k = Eq.first_order_markov_assumption.rhs.lhs.of(Indexed)
    Eq << algebra.cond.then.cond.domain_defined.apply(Eq.x_independence)

    Eq << stats.ne_zero.then.et.ne_zero.apply(Eq[-1])

    Eq << stats.ne_zero.then.ne_zero.conditioned.apply(Eq[-3], y[:k])

    Eq << stats.ne_zero.then.eq.prob.to.mul.prob.bayes.apply(Eq[-2], x[:k + 1], y[k])

    Eq << Eq[-1].this.lhs.arg.apply(algebra.et.concat, i=2, j=0)

    Eq << stats.ne_zero.then.eq.bayes.conditioned.apply(Eq[-3], x[k], y[k])

    Eq << Eq[-1].this.lhs.find(And).apply(algebra.et.concat, i=2, j=0)

    Eq << Eq[-3].subs(Eq[-1])

    Eq.xy_joint_probability = stats.ne_zero.then.eq.prob.to.mul.prob.bayes.apply(Eq[2], x[:k])

    Eq << Eq[-1].subs(Eq.xy_joint_probability.reversed)

    Eq.recursion = algebra.ne_zero.eq.then.eq.scalar.apply(Eq[0], Eq[-1])

    Eq << stats.ne_zero.then.ne_zero.joint_slice.apply(Eq.xy_nonzero_assumption, [k, k])

    Eq << stats.eq_conditioned.then.eq.conditioned.getitem.apply(Eq.x_independence)

    Eq << stats.eq_conditioned.eq_conditioned.then.eq.prob.joint.apply(Eq[-1], Eq.xy_independence)

    Eq << stats.ne_zero.eq_conditioned.then.eq.conditioned.joint.apply(Eq[-1], Eq[0])

    Eq.recursion = Eq.recursion.subs(Eq[-1])

    Eq << algebra.cond.then.ou.subs.apply(Eq[2], k, k + 1)

    Eq << Eq[-1].this.find(NotElement).simplify()

    Eq << algebra.ou.then.all.apply(Eq[-1])

    _, Eq.y_nonzero_assumption = stats.ne_zero.then.et.ne_zero.apply(Eq.xy_nonzero_assumption)

    Eq <<= Eq[-1] & Eq.y_nonzero_assumption

    Eq.y_joint_y_historic = Eq[-1].this.lhs.arg.apply(algebra.eq.then.et.eq.split)

    Eq << stats.ne_zero.then.ne_zero.conditioned.apply(Eq.y_joint_y_historic, y[:k])

    Eq << stats.ne_zero.then.eq.bayes.conditioned.apply(Eq[-1], Eq.x_independence.lhs.lhs)

    Eq.recursion = Eq.recursion.subs(Eq[-1])

    Eq.recursion = Eq.recursion.subs(Eq.first_order_markov_assumption)

    Eq << stats.eq_conditioned.then.eq.conditioned.getitem.apply(Eq.x_independence, wrt=y[:k])

    Eq << stats.ne_zero.eq_conditioned.then.eq.conditioned.joint.apply(Eq.y_joint_y_historic, Eq[-1])

    Eq.recursion = Eq.recursion.subs(Eq[-1])

    Eq << algebra.eq.then.eq.prod.apply(Eq.recursion, (k, 1, k + 1))

    Eq << Eq[-1].this.rhs.limits_subs(Eq[-1].rhs.variable, Eq.factorization.rhs.args[-1].variable)

    Eq << Eq[-1] * Eq[-1].lhs.args[0].base

    Eq.first = Eq.xy_joint_probability.subs(k, 1)

    Eq << Eq[-1].subs(Eq.first)

    t = Eq.factorization.rhs.args[-1].limits[0][2] - 1
    Eq << algebra.cond.then.ou.subs.apply(Eq[-1], k, t)

    Eq << Eq[-1].this.find(NotElement).simplify()

    Eq << algebra.ou.then.all.apply(Eq[-1])

    Eq <<= Eq[-1] & Eq.first

    # reference: Neural Architectures for Named Entity Recognition.pdf




if __name__ == '__main__':
    run()
# created on 2020-12-17
# updated on 2023-05-20
