...
def prove(Eq):
    ...
    n = x.shape[0]
    s, t = Eq[4].lhs.args
    Eq.x_quote_definition = Eq[5].apply(algebra.eq.then.eq.lamda, (Eq[5].lhs.indices[-1],))

    y = Eq.x_quote_definition.rhs.variable.base
    Eq << keras.ne_zero.eq.eq.eq.then.eq.crf.markov.apply(*Eq[:4])

    Eq << keras.eq.then.infer.crf.logits.apply(Eq[-1], Eq.G_definition.lhs.base, Eq[6].lhs.base, s)
    ...
    Eq << algebra.eq.then.eq.maxima.apply(Eq[-1], (y[:t + 1],))

    Eq << Eq[-1].this.rhs.apply(algebra.maxima.to.exp)

    Eq << algebra.eq.then.eq.reducedMax.apply(Eq.x_quote_definition)
