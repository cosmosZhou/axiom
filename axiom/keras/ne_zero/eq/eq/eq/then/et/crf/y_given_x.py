...
def apply(x_independence_assumption, y_independence_assumption, xy_independence_assumption, xy_nonzero_assumption):
    from axiom.keras.ne_zero.eq.eq.eq.then.eq.crf.markov import process_assumptions
...
def prove(Eq):
    ...
    from axiom.keras.ne_zero.eq.eq.eq.then.eq.crf.markov import markov_assumptions
    ...
    n = x.shape[0]
    s, t = Eq[4].lhs.args
    Eq.z_definition = Eq[5].apply(algebra.eq.then.eq.lamda, (Eq[5].lhs.indices[-1],))

    Eq << keras.ne_zero.eq.eq.eq.then.eq.crf.markov.apply(*Eq[:4])

    Eq << keras.eq.then.infer.crf.logits.apply(Eq[-1], Eq[8].lhs.base, Eq[7].lhs.base, s)

    Eq << Eq[-2].subs(t, t + 1)

    Eq << Eq[-1].this.args[1].apply(sets.notin.then.notin.sub, 1)

    Eq << algebra.ou.then.infer.apply(Eq[-1])

    Eq << Eq.z_definition.subs(t, t + 1)

    Eq << Eq[-1].this.find(NotElement).apply(sets.notin.then.notin.sub, 1)

    Eq << algebra.ou.then.infer.apply(Eq[-1])

    Eq <<= Eq[-1] & Eq[-4]

    Eq << Eq[-1].this.rhs.apply(algebra.eq.eq.then.eq.subs.rhs)
    ...
    Eq.z_definition_by_x_quote = algebra.eq.then.eq.exp.apply(Eq[6].reversed)
    ...
    Eq << Eq[-1].this.find(NotElement).apply(sets.notin.then.notin.add, 1)

    Eq << algebra.ou.then.infer.apply(Eq[-1])

    Eq << Eq[-1].this.lhs.apply(algebra.ne_zero.of.gt_zero)

    Eq << Eq[-1].this.find(log).apply(keras.log.reducedSum.to.logsumexp)

    Eq.xy_joint_nonzero = stats.ne_zero.then.ne_zero.joint_slice.apply(Eq[3], (slice(0, t + 1), slice(0, t + 1)))

    Eq << stats.ne_zero.then.et.ne_zero.apply(Eq.xy_joint_nonzero)

    y = Eq[-1].lhs.arg.lhs.base
    Eq << stats.ne_zero.then.eq.prob.to.mul.prob.bayes.apply(Eq[-2], y[:t + 1])

    Eq << stats.sum.to.prob.apply(Sum[pspace(y[:t + 1]).symbol](Eq[-1].lhs))

    Eq << Eq[-2].subs(Eq[-1].reversed)

    Eq << Eq[-1].apply(algebra.eq.then.ou.log)

    Eq << algebra.et.then.et.apply(Eq[-1] & Eq.xy_joint_nonzero)

    Eq << Eq[-1].this.rhs.apply(algebra.log.to.add)

    Eq << algebra.eq.then.eq.exp.apply(Eq[4].reversed)

    Eq.y_given_x_log = Eq[-2].subs(Eq[-1])

    Eq << Eq.z_definition.apply(algebra.eq.then.eq.reducedSum)
