...
@apply
def apply(ne_zero, eq, Q_pi_def, V_pi_def, lt, n=None, θ_quote=None):
    ...
    return Equal(Derivative[θ](Expectation[r[1:], a:θ](Sum[k:0:oo](γ ** k * r[k + 1]))),
                 Expectation(Sum[k](γ ** k * Expectation[a:θ_quote](Derivative[θ](Probability[a:θ](a[k].surrogate | s[k].surrogate)) / Probability[a:θ_quote](a[k].surrogate | s[k].surrogate) * (Q_pi_st_var._subs(s[t].var, s[k])._subs(a[t].var, a[k]) - V_pi_st_var._subs(s[t].var, s[k])) | s[k].surrogate))))
...
def prove(Eq):
    ...
    *Eq[-5:], Eq.hypothesis = apply(Unequal(Probability[a:θ](s, a), 0),
    ...
    Eq << keras.ne_zero.eq.eq.eq.is_finite.imply.eq.policy_gradient_theorem.apply(*Eq[:5], n)

    Eq.hypothesis = Eq.hypothesis.this.find(Derivative[Probability]).apply(calculus.grad.to.mul.grad.log)

    Eq << Eq.hypothesis.find(Mul[~Expectation]).this.apply(stats.expect.conditioned.importance_sampling, θ)

    Eq << Eq[-1].this.rhs.find(Mul[Add]).apply(algebra.mul.to.add)

    Eq << Eq[-1].this.rhs.apply(stats.expect.to.add)

    Eq << Eq[-1].this.find(Expectation[Conditioned[Mul[NegativeOne]]]).apply(stats.expect.to.mul)

    Eq << Eq[-1].this.find(Mul[~Expectation[Conditioned[Derivative]]]).apply(stats.expect_conditioned.to.zero.st.grad.log.prob)
