...
def prove(Eq):
    ...
    Eq << algebra.then.lt.ceiling.apply(Eq.eq_V_dquote.find(Ceiling).arg) - 1

    Eq << Eq[-1] * d + β[i]

    Eq << Eq[-1].this.rhs.subs(Eq.zeta[i])

    Eq << algebra.lt.then.lt.relax.apply(Eq[-1], upper=n)

    Eq.le_ceiling = algebra.lt.then.le.strengthen.apply(Eq[-1])
    ...
    Eq <<= algebra.cond.cond.then.cond.subs.apply(Eq.le_ceiling, Eq[-2]), algebra.cond.cond.then.cond.subs.apply(Eq.le_ceiling, Eq[-1])
    ...
    Eq << algebra.le.then.le.ceiling.apply(Eq.le_min / d)

    Eq <<= algebra.le.eq.then.eq.slice.apply(Eq[-1], Eq[-3]), algebra.le.eq.then.eq.slice.apply(Eq[-1], Eq[-2])

    Eq <<= algebra.eq.eq.then.eq.trans.apply(Eq.slice_K_dquote, Eq[-2]), algebra.eq.eq.then.eq.trans.apply(Eq.slice_V_dquote, Eq[-1])

    Eq << keras.eq_max.eq_min.eq.eq.then.eq.softmax.bert.position_representation.relative.band_part_mask.dilated.apply(Eq.beta, Eq.zeta, Eq[0], Eq[1], Q, K, V)
