...
def prove(Eq):
    ...
    Eq << keras.eq_max.eq_min.eq.eq.then.eq.softmax.bert.position_representation.relative.band_part_mask.dilated.apply(Eq.beta, Eq.zeta, Eq[-2], Eq[-1], Q, K, V)

    Eq <<= Eq[0][i], Eq[1][i]

    Eq.le_min = algebra.eq_max.eq_min.then.le.apply(Eq.beta, Eq.zeta)

    Eq <<= algebra.le.eq.then.eq.slice.apply(Eq.le_min, Eq[-2], step=d), algebra.le.eq.then.eq.slice.apply(Eq.le_min, Eq[-1], step=d)
    ...
    Eq.le_zeta = algebra.eq.then.le.relax.apply(Eq.zeta[i], upper=n)

    Eq <<= algebra.cond.cond.then.cond.subs.apply(Eq.le_zeta, Eq[-2]), algebra.cond.cond.then.cond.subs.apply(Eq.le_zeta, Eq[-1])

    Eq <<= Eq[-2].this.rhs().find(Min).simplify(), Eq[-1].this.rhs().find(Min).simplify()

    Eq <<= algebra.le.eq.then.eq.slice.apply(Eq.le_zeta, Eq.K_quote[i], start=β[i], step=d), algebra.le.eq.then.eq.slice.apply(Eq.le_zeta, Eq.V_quote[i], start=β[i], step=d)

    Eq <<= algebra.eq.eq.then.eq.trans.apply(Eq[-4], Eq[-2]), algebra.eq.eq.then.eq.trans.apply(Eq[-3], Eq[-1])

    Eq <<= Eq[-2].this.rhs.apply(algebra.lamda.range.simplify), Eq[-1].this.rhs.apply(algebra.lamda.range.simplify)

    Eq <<= algebra.le.eq.then.eq.slice.apply(Eq.le_min, Eq.K_dquote[i], step=d), algebra.le.eq.then.eq.slice.apply(Eq.le_min, Eq.V_dquote[i], step=d)

    Eq <<= Eq[-2].this.rhs.apply(algebra.lamda.range.simplify), Eq[-1].this.rhs.apply(algebra.lamda.range.simplify)

    Eq <<= algebra.eq.eq.then.eq.trans.apply(Eq[-6], Eq[-2]), algebra.eq.eq.then.eq.trans.apply(Eq[-5], Eq[-1])
