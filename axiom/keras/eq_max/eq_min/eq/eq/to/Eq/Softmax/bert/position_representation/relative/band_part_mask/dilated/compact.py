...
def prove(Eq):
    ...
    Eq.le = Algebra.Eq_Max.Eq_Min.to.Le.apply(Eq.beta, Eq.zeta)
    ...
    Eq <<= Algebra.Le.Eq.to.Eq.Slice.apply(Eq.le, Eq[-2], step=d), Algebra.Le.Eq.to.Eq.Slice.apply(Eq.le, Eq[-1], step=d)

    Eq.eq_K_dquote = Eq[-2].this.rhs.apply(Algebra.Lamda.Range.simp)

    Eq.eq_V_dquote = Eq[-1].this.rhs.apply(Algebra.Lamda.Range.simp)

    Eq <<= Eq.K_dquote[i], Eq.V_dquote[i]

    Eq << Algebra.Le.to.Le.Ceiling.apply(Eq.le / d)

    Eq <<= Algebra.Le.Eq.to.Eq.Slice.apply(Eq[-1], Eq[-3]), Algebra.Le.Eq.to.Eq.Slice.apply(Eq[-1], Eq[-2])

    Eq <<= Algebra.Eq.Eq.to.Eq.trans.apply(Eq.eq_K_dquote, Eq[-2]), Algebra.Eq.Eq.to.Eq.trans.apply(Eq.eq_V_dquote, Eq[-1])
