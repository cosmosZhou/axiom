...
def prove(Eq):
    ...
    Eq.le_zeta_i = algebra.eq.then.le.relax.apply(Eq.zeta_i, upper=n)

    Eq << algebra.le.eq.then.eq.slice.apply(Eq.le_zeta_i, Eq[-1], start=β[i], step=d)

    Eq.zi_definition = Eq[-3].subs(Eq[-1])

    Eq.le = algebra.eq_max.eq_min.then.le.apply(Eq.beta, Eq.zeta)

    Eq <<= algebra.le.eq.then.eq.slice.apply(Eq.le, Eq.K_dquote[i], step=d), algebra.le.eq.then.eq.slice.apply(Eq.le, Eq.V_dquote[i], step=d)
    ...
    Eq <<= algebra.cond.cond.then.cond.subs.apply(Eq.le_zeta_i, Eq[-2]), algebra.cond.cond.then.cond.subs.apply(Eq.le_zeta_i, Eq[-1])
