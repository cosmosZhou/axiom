...
def prove(Eq):
    ...
    Eq << algebra.ne.then.eq.sum.geometric_series.apply(Eq[-1], Eq[-2].rhs.find(Mul[~Sum]))
    ...
    Eq << sets.abs_lt.is_finite.then.is_real.sum.apply(Eq.lt, Eq[-1], simplify=None)
    ...
    Eq << calculus.abs_lt.then.is_zero.limit.geometric_series.apply(Eq.lt, n)

    Eq << calculus.is_zero.is_limited.then.is_zero.limit.algebraic_limit_theorem.apply(Eq[-1], Eq[-2])
