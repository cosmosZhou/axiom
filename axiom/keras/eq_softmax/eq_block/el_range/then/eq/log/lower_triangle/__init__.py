...
def prove(Eq):
    ...
    Eq.ge_relu, Eq.lt_1 = sets.el_range.then.et.apply(Eq[2])
    ...
    Eq.ge = -algebra.le.then.le.relax.apply(Eq[-1], upper=Min(l - 1, n - 1))



    Eq << algebra.cond.cond.then.cond.subs.apply(Eq.ge, Eq.eq, invert=True)

    Eq << algebra.cond.cond.then.cond.subs.apply(Eq.lt_1, Eq[-1])
