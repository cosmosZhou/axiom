...
def prove(Eq):
    ...
    Eq <<= Algebra.Cond.to.All.restrict.apply(Eq[-1], (i, 0, u), simplify=None)

    Eq << Algebra.All_Eq.to.All.Eq.Slice.apply(Eq[-1], slice(i + n - u, n))

    Eq << Eq[-1].this.find(~Indexed - Indexed).args[1].apply(Algebra.Expr.eq.Piece, upper=n - 1)
    ...
    Eq << Algebra.All_Eq.to.All.Eq.Slice.apply(Eq[-1], slice(0, u - i))
