...
def prove(Eq):
    ...
    Eq << keras.eq_block.eq_block.then.all.eq.relative_distance.upper_triangle.lower_part.apply(Eq[0], Eq[1])

    Eq << algebra.all_eq.then.eq.lamda.apply(Eq[-1].this.expr.reversed, Eq[2].find(Lamda).expr)

    Eq << keras.eq_block.eq_block.then.all.eq.relative_distance.upper_triangle.upper_part.apply(Eq[0], Eq[1])

    Eq << algebra.all_eq.then.eq.lamda.apply(Eq[-1].this.expr.reversed)

    Eq << keras.eq_block.eq_block.then.eq.relative_distance.upper_triangle.apply(Eq[0], Eq[1])
    ...
    Eq << algebra.cond.then.all.restrict.apply(Eq[-1], (i, 0, n - u))

    Eq << algebra.all_eq.then.all.eq.slice.apply(Eq[-1], slice(i, i + u))
    ...
    Eq << keras.eq_block.then.eq.softmax.upper_triangle.st.logsumexp.apply(Eq[-1])
