...
def prove(Eq):
    ...
    Eq << Keras.Eq_Block.Eq_Block.to.All.Eq.relative_distance.upper_triangle.lower_part.apply(Eq[0], Eq[1])

    Eq << Algebra.All_Eq.to.Eq.Lamda.apply(Eq[-1].this.expr.reversed, Eq[2].find(Lamda).expr)

    Eq << Keras.Eq_Block.Eq_Block.to.All.Eq.relative_distance.upper_triangle.upper_part.apply(Eq[0], Eq[1])

    Eq << Algebra.All_Eq.to.Eq.Lamda.apply(Eq[-1].this.expr.reversed)

    Eq << Keras.Eq_Block.Eq_Block.to.Eq.relative_distance.upper_triangle.apply(Eq[0], Eq[1])

    Eq << Eq[2].subs(Eq[-1].reversed, Eq[-2], Eq[-4])

    Eq.z_def = Eq[-1].this.find(Lamda + Lamda).apply(Algebra.Add.eq.Lamda)
    ...
    Eq << Algebra.Cond.to.All.restrict.apply(Eq[-1], (i, 0, n - u))

    Eq << Algebra.All_Eq.to.All.Eq.Slice.apply(Eq[-1], slice(i, i + u))
    ...
    Eq << Keras.Eq_Block.to.Eq.Softmax.upper_triangle.st.LogSumExp.apply(Eq[-1])
