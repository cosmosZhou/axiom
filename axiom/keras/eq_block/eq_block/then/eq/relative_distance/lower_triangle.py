...
def prove(Eq):
    ...
    Eq <<= algebra.cond.then.all.restrict.apply(Eq[-1], (i, 0, n), simplify=None)

    Eq << algebra.all_eq.then.all.eq.slice.apply(Eq[-1], slice(i + 1 - Min(i + 1, l), i + 1))
    ...
    Eq << algebra.all_eq.then.all.eq.slice.apply(Eq[-1], slice(0, Min(i + 1, l)))

    Eq << algebra.eq.eq.then.eq.trans.apply(Eq.V_relu, Eq[-1])
