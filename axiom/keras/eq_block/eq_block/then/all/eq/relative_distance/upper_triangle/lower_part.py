...
def prove(Eq):
    ...
    Eq <<= algebra.cond.then.all.restrict.apply(Eq[-1], (i, 0, u), simplify=None)

    Eq << algebra.all_eq.then.all.eq.slice.apply(Eq[-1], slice(i + n - u, n))
    ...
    Eq << algebra.all_eq.then.all.eq.slice.apply(Eq[-1], slice(0, u - i))
