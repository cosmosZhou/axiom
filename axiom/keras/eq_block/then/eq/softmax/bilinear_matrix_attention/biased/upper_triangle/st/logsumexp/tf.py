...
def prove(Eq):
    ...
    Eq << algebra.cond.then.all.restrict.apply(Eq[-1], (i, 0, n + 1 - Min(u, n)))

    Eq << algebra.all_eq.then.all.eq.slice.apply(Eq[-1], slice(i, i + Min(u, n)))
    ...
    Eq << keras.eq_block.then.eq.softmax.biased.upper_triangle.st.logsumexp.tf.apply(Eq[-1])
