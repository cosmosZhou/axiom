...
def prove(Eq):
    ...
    Eq << keras.eq_block.then.eq.softmax.st.logsumexp.tf.apply(Eq[0])

    z_quote = Symbol(Eq[-1].lhs)
    Eq.z_quote_def = z_quote.this.definition

    Eq << algebra.eq.eq.then.eq.trans.apply(Eq.z_quote_def, Eq[-1])
    ...
    Eq << algebra.eq.then.eq.reducedArgMax.apply(Eq[-1])
    ...
    Eq << keras.eq_block.then.et.infer.block.tf.apply(Eq[0])

    Eq <<= Eq[-3].this.rhs.apply(algebra.eq.then.eq.reducedArgMax), Eq[-2].this.rhs.apply(algebra.eq.then.eq.reducedArgMax), Eq[-1].this.rhs.apply(algebra.eq.then.eq.reducedArgMax)
    ...
    Eq << algebra.infer.then.eq.piece.apply(Eq.block0, Eq.four_blocks.rhs, index=0, reverse=True)

    Eq << algebra.infer.then.eq.piece.apply(Eq.block1, Eq[-1].rhs, index=1, reverse=True)

    Eq << algebra.infer.then.eq.piece.apply(Eq.block3, Eq[-1].rhs, index=1, reverse=True)

    Eq << algebra.et.then.eq.trans.apply(Eq.four_blocks & Eq[-1] & Eq[-2] & Eq[-3]).reversed
