...
def prove(Eq):
    ...
    Eq <<= algebra.eq.then.eq.lamda.apply(Eq[-2], (j, 0, m)), algebra.eq.then.eq.lamda.apply(Eq[-1], (j, 0, m))

    Eq <<= algebra.eq.then.eq.transpose.apply(Eq[-2], (0, 1)), algebra.eq.then.eq.transpose.apply(Eq[-1], (0, 1))

    Eq <<= algebra.eq.eq.then.eq.trans.apply(Eq[-2], Eq[4]), algebra.eq.eq.then.eq.trans.apply(Eq[-1], Eq[5])

    Eq << algebra.eq.then.eq.transpose.apply(Eq[-2], (1, 1))
    Eq << keras.eq_card.subset_cup.then.eq.matmul.softmax.position_representation.relative.gather.apply(Eq[0], Eq[1], Q, K, V, K_quote, V_quote)
