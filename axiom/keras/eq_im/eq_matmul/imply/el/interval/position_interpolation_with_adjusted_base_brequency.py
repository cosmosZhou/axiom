...
def prove(Eq):
    ...
    Eq << algebra.le.imply.le.pow.apply(Eq[-2], 2 * j / d)

    Eq << LessEqual(α, 1, plausible=True)

    Eq << algebra.le.le.imply.le.mul.apply(Eq[-1], Eq[-2])
    ...
    Eq << sets.le.gt.imply.el.interval.apply(Eq[-1], Eq.gt_zero_eta)

    Eq << geometry.el_interval.imply.gt_zero.sin.apply(Eq[-1])

    Eq.gt_zero = algebra.cond.imply.all.apply(Eq[-1], j)

    Eq << algebra.imply.all.le.reducedMax.apply(ReducedMax(x ** 2))
    ...
    Eq << Eq[-1].this.expr.apply(algebra.gt_zero.le.imply.le.mul)

    Eq.sum_le = algebra.all_le.imply.le.sum.apply(Eq[-1])

    Eq << algebra.imply.all.ge.reducedMin.apply(ReducedMin(x ** 2))
    ...
    Eq << Eq[-1].this.expr.apply(algebra.gt_zero.ge.imply.ge.mul)

    Eq.sum_ge = algebra.all_ge.imply.ge.sum.apply(Eq[-1])

    Eq << geometry.imply.sin_ge.quadratic.apply(Eq.gt_zero_eta.lhs)
    ...
    Eq.gt_zero_2_bβ = algebra.lt.imply.gt_zero.apply(Eq[-1])

    Eq.ne = algebra.lt.imply.ne.apply(Eq[-1])

    Eq << algebra.lt.imply.lt.pow.apply(Eq[-1], 2, evaluate=False)

    Eq.ne4 = algebra.lt.imply.ne.apply(Eq[-1])

    Eq << algebra.cond.cond.imply.cond.subs.apply(Eq.ne, Eq.sum_ge_piece)

    Eq << Eq[-1].this.rhs.find(Sum).apply(algebra.sum.to.mul.series.geometric)

    Eq << Eq[-1].this.find(Piecewise).apply(algebra.piece.swap)

    Eq << algebra.cond.cond.imply.cond.subs.apply(Eq.ne4, Eq[-1])
    ...
    Eq.sum_ge = algebra.ge.ge.imply.ge.transit.apply(Eq.sum_ge, Eq[-1])

    Eq << geometry.gt_zero.imply.sin_le.apply(Eq.gt_zero_eta)
    ...
    Eq << algebra.cond.cond.imply.cond.subs.apply(Eq.ne, Eq[-1])

    Eq << Eq[-1] * (2 * ReducedMax(x ** 2))

    Eq.sum_le = algebra.le.le.imply.le.transit.apply(Eq.sum_le, Eq[-1])

    Eq << sets.ge.le.imply.el.interval.apply(Eq.sum_ge, Eq.sum_le)

    Eq << Eq[-1].subs(Eq.eq_cos.reversed)

    Eq << Greater(2 * α * (1 - 1 / (b * β)), 0, plausible=True)

    Eq << algebra.gt_zero.gt_zero.imply.gt_zero.div.apply(Eq[-1], Eq.gt_zero_2_bβ)
