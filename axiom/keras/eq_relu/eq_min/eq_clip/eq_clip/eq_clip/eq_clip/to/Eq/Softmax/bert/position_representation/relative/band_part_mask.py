...
def prove(Eq):
    ...
    Eq << Keras.Eq_Relu.Eq_Min.Eq.Eq.to.Eq.Softmax.bert.position_representation.relative.band_part_mask.apply(Eq.beta, Eq.zeta, Eq[-2], Eq[-1], Q, K, V)

    Eq <<= Eq[0][i], Eq[1][i]

    Eq.le_min = Keras.Eq_Relu.Eq_Min.to.Le.apply(Eq.beta, Eq.zeta)

    Eq <<= Algebra.Le.Eq.to.Eq.Slice.apply(Eq.le_min, Eq[-2]), Algebra.Le.Eq.to.Eq.Slice.apply(Eq.le_min, Eq[-1])

    Eq <<= Eq[-2].this.find(Symbol + Indexed).apply(Algebra.Expr.eq.Piece, upper=ζ[i] - 1), Eq[-1].this.find(Symbol + Indexed).apply(Algebra.Expr.eq.Piece, upper=ζ[i] - 1)

    Eq <<= Eq[-2].this.rhs().find(GreaterEqual).simplify(), Eq[-1].this.rhs().find(GreaterEqual).simplify()

    Eq <<= Eq[-2].this.find(Symbol - 1).apply(Algebra.Expr.eq.Piece, lower=ζ[i] - 1), Eq[-1].this.find(Symbol - 1).apply(Algebra.Expr.eq.Piece, lower=ζ[i] - 1)
    ...
    Eq <<= Algebra.Le.Eq.to.Eq.Slice.apply(Eq.le_zeta, Eq.K_quote[i], start=β[i]), Algebra.Le.Eq.to.Eq.Slice.apply(Eq.le_zeta, Eq.V_quote[i], start=β[i])

    Eq <<= Algebra.Eq.Eq.to.Eq.trans.apply(Eq[-4], Eq[-2]), Algebra.Eq.Eq.to.Eq.trans.apply(Eq[-3], Eq[-1])

    Eq <<= Algebra.Le.Eq.to.Eq.Slice.apply(Eq.le_min, Eq.K_dquote[i]), Algebra.Le.Eq.to.Eq.Slice.apply(Eq.le_min, Eq.V_dquote[i])
