...
def prove(Eq):
    ...
    Eq << keras.eq_relu.eq_min.eq.eq.then.eq.softmax.bert.position_representation.relative.band_part_mask.apply(Eq.beta, Eq.zeta, Eq[-2], Eq[-1], Q, K, V)

    Eq <<= Eq[0][i], Eq[1][i]

    Eq.le_min = keras.eq_relu.eq_min.then.le.apply(Eq.beta, Eq.zeta)

    Eq <<= algebra.le.eq.then.eq.slice.apply(Eq.le_min, Eq[-2]), algebra.le.eq.then.eq.slice.apply(Eq.le_min, Eq[-1])
    ...
    Eq <<= algebra.le.eq.then.eq.slice.apply(Eq.le_zeta, Eq.K_quote[i], start=β[i]), algebra.le.eq.then.eq.slice.apply(Eq.le_zeta, Eq.V_quote[i], start=β[i])

    Eq <<= algebra.eq.eq.then.eq.trans.apply(Eq[-4], Eq[-2]), algebra.eq.eq.then.eq.trans.apply(Eq[-3], Eq[-1])

    Eq <<= algebra.le.eq.then.eq.slice.apply(Eq.le_min, Eq.K_dquote[i]), algebra.le.eq.then.eq.slice.apply(Eq.le_min, Eq.V_dquote[i])
