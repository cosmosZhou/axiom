...
def prove(Eq):
    ...
    Eq << Algebra.Mul.eq.Exp.oo.apply(exp(a) * band_part).reversed
    ...
    Eq.Xi_def = Eq[-1].this.rhs.expr.apply(Algebra.Bool.eq.Piece)
    ...
    Eq.le_zeta_i = Algebra.Eq.to.Le.relax.apply(Eq.zeta[i], upper=n)

    Eq << Algebra.Le.Eq.to.Eq.Slice.apply(Eq.le_zeta_i, Eq[-1], start=β[i])

    Eq.zi_definition = Eq[-3].subs(Eq[-1])

    Eq << Keras.Eq_Relu.Eq_Min.to.Le.apply(Eq.beta, Eq.zeta)

    Eq <<= Algebra.Le.Eq.to.Eq.Slice.apply(Eq[-1], Eq.K_dquote[i]), Algebra.Le.Eq.to.Eq.Slice.apply(Eq[-1], Eq.V_dquote[i])
    ...
    Eq <<= Algebra.Cond.Cond.to.Cond.subs.apply(Eq.le_zeta_i, Eq[-2]), Algebra.Cond.Cond.to.Cond.subs.apply(Eq.le_zeta_i, Eq[-1])
