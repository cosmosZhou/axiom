...
def prove(Eq):
    ...
    Eq.le_zeta_i = algebra.eq.then.le.relax.apply(Eq.zeta[i], upper=n)

    Eq << algebra.le.eq.then.eq.slice.apply(Eq.le_zeta_i, Eq[-1], start=β[i])

    Eq.zi_definition = Eq[-3].subs(Eq[-1])

    Eq << keras.eq_relu.eq_min.then.le.apply(Eq.beta, Eq.zeta)

    Eq <<= algebra.le.eq.then.eq.slice.apply(Eq[-1], Eq.K_dquote[i]), algebra.le.eq.then.eq.slice.apply(Eq[-1], Eq.V_dquote[i])
