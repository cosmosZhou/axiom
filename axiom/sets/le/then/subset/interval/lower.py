from util import *


@apply
def apply(given, lower, left_open=False, right_open=False):
    a, b = given.of(LessEqual)
    kwargs = {'right_open' : right_open, 'left_open': left_open}
    return Subset(Interval(lower, a, **kwargs), Interval(lower, b, **kwargs))


@prove
def prove(Eq):
    from axiom import sets, algebra

    x, y, z = Symbol(real=True, given=True)
    Eq << apply(x <= y, z, left_open=True)

    Eq << sets.subset.of.all_el.apply(Eq[1])

    Eq << Eq[-1].this.expr.apply(sets.el_interval.of.et)

    Eq << algebra.all_et.of.et.all.apply(Eq[-1])

    Eq <<= algebra.all.of.ou.apply(Eq[-2]), algebra.all.of.ou.apply(Eq[-1])

    Eq <<= Eq[-2].this.args[1].apply(sets.notin_interval.of.ou), Eq[-1].this.find(NotElement).apply(sets.notin_interval.of.ou)

    Eq << algebra.ou.of.ou.apply(Eq[-1], slice(0, 2))

    Eq << sets.ou.of.notin.interval.apply(Eq[-1])

    Eq << sets.le.then.is_empty.interval.apply(Eq[0], left_open=True)

    Eq << Eq[-2].subs(Eq[-1])




if __name__ == '__main__':
    run()
# created on 2019-07-09
# updated on 2023-05-19
