...
def prove(Eq):
    ...
    Eq << Eq[-1].this.expr.apply(sets.notin_range.then.ou)
    ...
    Eq <<= algebra.any_et.then.any.limits_absorb.apply(Eq.any_ax, index=1), algebra.any_et.then.any.limits_absorb.apply(Eq.any_by, index=1)

    Eq <<= Eq[-2].this.expr.apply(algebra.lt.ge.then.lt.trans), Eq[-1].this.expr.apply(algebra.lt.ge.then.gt.trans)
