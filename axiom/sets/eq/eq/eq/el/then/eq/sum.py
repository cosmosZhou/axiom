...
def prove(Eq):
    ...
    Eq.contains = sets.eq_cup.then.el.apply(Eq[1])

    Eq << sets.el.then.eq.card.complement.apply(Eq.contains)

    Eq << Eq[1].subs(Eq[-1].reversed)

    Eq << sets.eq_cup.then.eq.sum.apply(Eq[-1], Sum[x:X - {y}](f(x)))

    Eq.eq = Eq[-1].subs(Eq[-3])

    Eq << sets.eq_cup.then.eq.sum.apply(Eq[0], Sum[x:X](f(x)))

    Eq << Eq[-1].this.lhs.apply(algebra.sum.to.add.split, cond={y})

    Eq << algebra.cond.cond.then.cond.subs.apply(Eq.contains, Eq[-1])
