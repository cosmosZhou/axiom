...
def prove(Eq):
    ...
    Eq.suffice, Eq.necessary = sets.eq.of.et.infer.apply(Eq[0], wrt=t)

    Eq << Eq.suffice.this.find(Element).apply(algebra.cond_piece.then.ou)

    Eq << Eq[-1].this.lhs.apply(algebra.ou.to.infer, index=slice(None, 2))

    Eq << Eq[-1].this.rhs.apply(sets.el_union.of.ou, simplify=None)

    Eq << Eq[-1].this.rhs.find(Element).apply(algebra.cond_piece.of.ou)

    Eq << Eq[-1].this.rhs.find(Element).apply(algebra.cond_piece.of.ou)

    Eq << Eq[-1].this.rhs.apply(algebra.ou.to.infer, index=slice(None, 2))

    Eq << Eq.necessary.this.find(Element).apply(sets.el_union.then.ou, simplify=None)

    Eq << Eq[-1].this.lhs.find(Element).apply(algebra.cond_piece.then.ou)
