...
def prove(Eq):
    ...
    Eq << Eq[1].this.expr.apply(sets.el.then.subset, simplify=False)

    Eq.subset_A = sets.all_subset.then.subset.cup.lhs.apply(Eq[-1])

    Eq.supset_A = Eq.subset_A.func.reversed_type(*Eq.subset_A.args, plausible=True)

    Eq << sets.supset.of.all_el.apply(Eq.supset_A, var=Eq[0].variable)

    Eq << Eq[-1].this.expr.apply(sets.el.of.any.eq.split.imageset)
    ...
    Eq << Eq[-1].this.expr.expr.apply(algebra.eq.then.eq.invoke, g)
