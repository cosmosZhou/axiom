...
def apply(all_a, all_b, equality_a, equality_b):
    from axiom.sets.all_el.all_el.all_eq.then.eq import analyze
...
def prove(Eq):
    ...
    Eq << sets.all_el.all_el.all_eq.then.eq.apply(Eq[0], Eq[1], Eq[2])

    Eq << sets.all_el.all_el.all_eq.then.eq.apply(Eq[1], Eq[0], Eq[3])

    Eq << sets.eq.eq.then.eq.card.apply(Eq[-1], Eq[-2]).reversed
