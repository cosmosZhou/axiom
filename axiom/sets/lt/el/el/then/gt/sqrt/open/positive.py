...
def prove(Eq):
    ...
    Eq.x_is_positive = sets.is_positive.then.gt_zero.apply(Eq[1])

    Eq.y_is_positive = sets.is_positive.then.gt_zero.apply(Eq[2])
    ...
    Eq << algebra.gt_zero.then.gt_zero.square.apply(Eq.x_is_positive)

    Eq << algebra.gt_zero.gt_zero.then.gt_zero.apply(Eq[-1], Eq[-2])

    Eq << algebra.gt_zero.then.ge_zero.apply(Eq[-1])

    Eq << algebra.ge_zero.gt.then.gt.sqrt.apply(Eq[-1], Eq[4])

    Eq <<= algebra.gt_zero.then.eq.abs.apply(Eq.x_is_positive), algebra.gt_zero.then.eq.abs.apply(Eq.y_is_positive)
