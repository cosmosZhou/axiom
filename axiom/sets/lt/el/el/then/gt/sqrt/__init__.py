...
def prove(Eq):
    ...
    Eq << sets.lt.el.then.lt.trans.apply(Eq[0], Eq[2])

    Eq.x_contains = sets.lt.el_interval.then.el.interval.intersect.apply(Eq[-1], Eq[1])

    Eq << sets.gt.el.then.gt.trans.apply(Eq[0].reversed, Eq[1])

    Eq.y_contains = sets.gt.el_interval.then.el.interval.intersect.apply(Eq[-1], Eq[2])

    Eq << algebra.cond.of.et.infer.split.apply(Eq[3], cond=Equal(x, -1))

    Eq << algebra.infer.of.infer.subs.apply(Eq[-2])

    Eq << algebra.infer.of.cond.apply(Eq[-1])

    Eq << -sets.el.then.lt.square.apply(Eq.y_contains) + 1

    Eq << algebra.gt_zero.then.gt_zero.sqrt.apply(Eq[-1])

    Eq << algebra.cond.then.infer.apply(Eq.x_contains, cond=Eq[-4].lhs)
    ...
    Eq << Eq[-1].this.rhs.apply(sets.lt.el.el.then.gt.sqrt.open)
