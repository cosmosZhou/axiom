...
def prove(Eq):
    ...
    Eq << sets.el_cartesianSpace.then.all.el.apply(Eq[1])

    Eq << Eq[-1].this.expr.apply(sets.el_interval.then.ge)

    Eq << Eq[0].this.lhs.apply(algebra.reducedSum.to.sum)

    Eq << algebra.eq_sum.all_ge_zero.then.all.le.apply(Eq[-1], Eq[-2])

    Eq << sets.el_cartesianSpace.of.all.el.apply(Eq[2])
