...
def prove(Eq):
    ...
    Eq << algebra.infer_et.given.infer.et.subs.apply(Eq.initial, index=0)
    ...
    Eq << algebra.infer_et.given.infer.et.subs.apply(Eq[-1], index=1)
    ...
    Eq << Eq[-1].this.lhs.apply(algebra.eq.lt.imply.eq.div, ret=1)
    ...
    Eq << algebra.cond.imply.cond.subs.apply(Eq.hypothesis, w[:n], w_)

    Eq << algebra.cond.imply.infer.apply(Eq[-1], cond=Eq[-2].rhs.lhs)

    Eq << Eq[-1].this.apply(algebra.infer.swap)

    Eq << Eq[-1].this.rhs.apply(algebra.infer_et.imply.infer.et)
