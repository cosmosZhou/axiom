...
def prove(Eq):
    ...
    Eq.eq, Eq.ne = algebra.cond.of.et.infer.split.apply(Eq[-1], cond=Equal(Card(Y), 1))

    Eq.suffice_et = algebra.cond.then.infer.et.apply(Eq[1], cond=Eq.eq.lhs)

    Eq << Eq.suffice_et.this.rhs.apply(sets.eq.el.then.is_empty)
    ...
    Eq.suffice_eq = Eq.suffice_et.this.rhs.apply(sets.eq.el.then.eq.finiteset)
    ...
    Eq << Eq[-1].this.rhs.apply(algebra.eq.cond.then.cond.subs)

    Eq << Eq[-1].this.rhs.apply(algebra.abs_le_zero.then.is_zero)

    Eq << algebra.cond.then.infer.apply(Eq[2], cond=Eq[-1].lhs)

    Eq <<= Eq[-1] & Eq[-2]

    Eq << Eq[-1].this.rhs.apply(sets.eq.notin.then.eq.st.variance)

    Eq << sets.notin.then.eq.card.apply(Eq[2])

    Eq << Eq[-2].subs(Eq[-1])

    Eq << sets.el.then.ge.card.apply(Eq[1])

    Eq << algebra.cond.then.infer.et.apply(Eq[-1], cond=Eq.ne.lhs)

    Eq << Eq[-1].this.rhs.apply(algebra.gt.then.ge.strengthen)

    Eq << algebra.cond.then.infer.apply(Eq[0] & Eq[1] & Eq[2], cond=Eq.ne.lhs)

    Eq <<= Eq[-1] & Eq[-2]

    Eq << Eq[-1].this.rhs.apply(sets.le.ge.el.notin.then.le.st.variance)
