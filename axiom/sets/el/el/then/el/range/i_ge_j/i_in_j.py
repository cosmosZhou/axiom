...
def prove(Eq):
    ...
    Eq <<= sets.el_range.of.et.apply(Eq[-2]), sets.el_range.of.et.apply(Eq[-1])

    Eq <<= sets.el_range.then.et.apply(Eq[0]), sets.el_range.then.et.apply(Eq[1])

    Eq << Eq[-2] - d

    Eq << algebra.ge.ge.then.ge.trans.apply(Eq[-1], Eq[6]) + d
