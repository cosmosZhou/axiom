...
def prove(Eq):
    ...
    Eq << sets.subset.then.subset.complement.apply(Eq[-1], {y})

    Eq << sets.el.then.eq.complement.apply(Eq[1])

    Eq << Eq[-2].subs(Eq[-1])

    Eq.subset = sets.subset.then.subset.complement.apply(Eq[-1], Eq[-1].rhs.args[1])

    Eq << sets.el_interval.then.le.apply(Eq[1])

    Eq << sets.le.then.subset.interval.minus_oo.apply(Eq[-1], right_open=True)

    Eq << sets.subset.then.subset.intersect.apply(Eq[-1], Interval.open(a, oo))
    Eq << sets.subset.subset.then.subset.trans.apply(Eq.subset, Eq[-1])
