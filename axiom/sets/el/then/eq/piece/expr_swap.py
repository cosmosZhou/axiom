...
def prove(Eq):
    ...
    Eq << Eq[-1].apply(algebra.cond.of.et.ou, cond=cond_contains)

    Eq.all_not_s, Eq.all_s = algebra.et.of.et.apply(Eq[-1])

    Eq << ~Eq.all_not_s

    Eq << Eq[-1].this.apply(algebra.cond.cond.then.cond.subs, swap=True, ret=1)
    ...
    Eq << Eq[-1].this.apply(algebra.cond.cond.then.cond.subs, swap=True, invert=True, ret=1)
