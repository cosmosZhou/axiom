...
def prove(Eq):
    ...
    Eq << sets.el_range.then.et.apply(Eq[-1], right_open=False)

    Eq << algebra.ge.ge.then.ge.trans.apply(Eq[-2], algebra.then.ge.floor.integer.apply(a + 1, 2))

    Eq << algebra.le.le.then.le.trans.apply(Eq[-2], algebra.then.floor_le.apply(b / 2) * 2)
