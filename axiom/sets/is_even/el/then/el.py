...
def prove(Eq):
    ...
    Eq << Eq[-1].this.find(Element).apply(sets.el.then.el.div.range, 2, simplify=None)

    Eq << Eq[-1].this.find(Equal).apply(algebra.eq.then.eq.div, 2, simplify=None)

    Eq << Eq[-1].this.find(Equal).apply(algebra.eq.then.eq.reverse, simplify=None)
