...
def prove(Eq):
    ...
    Eq <<= sets.imply.el_cup.reducedMax.apply(Eq[1].lhs.arg), sets.imply.el_cup.reducedMax.apply(Eq[1].rhs.arg)

    Eq <<= Eq[-2].subs(Eq[0]), Eq[-1].subs(Eq[0].reversed)

    Eq <<= sets.el_cup.imply.le_reducedMax.apply(Eq[-2]), sets.el_cup.imply.le_reducedMax.apply(Eq[-1])
