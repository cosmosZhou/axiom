...
def prove(Eq):
    ...
    Eq << sets.eq_cup.then.eq.sum.apply(Eq[0], Eq[6].rhs.args[0].find(Pow, Sum))

    Eq.given = Eq[4].subs(Eq[-1])

    Eq << sets.eq_cup.then.eq.sum.apply(Eq[0], Eq[6].rhs.args[0])

    Eq << Eq[-1].this.rhs.subs(Eq[-2])

    Eq << sets.eq_cup.then.eq.sum.apply(Eq[1], Eq[6].rhs.args[1].find(Pow, Sum))

    Eq.given = Eq.given.subs(Eq[-1])

    Eq << sets.eq_cup.then.eq.sum.apply(Eq[1], Eq[6].rhs.args[1])

    Eq << Eq[-1].this.rhs.subs(Eq[-2])

    Eq.add_ab = Eq[-4] + Eq[-1]

    Eq.abs_union = sets.notin.then.eq.card.apply(Eq[5])

    Eq << Eq[2].subs(Eq.abs_union.reversed)

    Eq << sets.eq_cup.then.eq.sum.apply(Eq[-1], Eq[6].lhs.args[0].find(Pow, Sum))

    Eq << sets.eq_cup.then.eq.sum.apply(Eq[-2], Eq[6].lhs.args[0])

    Eq << Eq[-1].this.rhs.subs(Eq[-2])

    Eq.eq_X_union = Eq[-1].subs(Eq.abs_union)

    Eq.contains = sets.eq_cup.then.el.apply(Eq[3])

    Eq.abs_complement = sets.el.then.eq.card.complement.apply(Eq.contains)

    Eq << Eq[3].subs(Eq.abs_complement.reversed)

    Eq << sets.eq_cup.then.eq.sum.apply(Eq[-1], Eq[6].lhs.args[1].find(Pow, Sum))

    Eq << sets.eq_cup.then.eq.sum.apply(Eq[-2], Eq[6].lhs.args[1])

    Eq << Eq[-1].this.rhs.subs(Eq[-2])

    Eq.eq_Y_complement = Eq[-1].subs(Eq.abs_complement)

    Eq << sets.eq_cup.el.then.any.eq.apply(Eq[1], Eq.contains)

    Eq << algebra.cond.any.then.any.et.apply(Eq.given, Eq[-1], simplify=None)

    Eq << Eq[-1].this.expr.apply(algebra.eq.cond.then.cond.subs, ret=0)

    Eq << algebra.any.then.any.et.limits.cond.apply(Eq[-1], 0, simplify=None)

    Eq << algebra.any_et.then.any.limits_absorb.apply(Eq[-1], simplify=None)

    Eq << Eq[-1].this.expr.apply(sets.le_abs.el.then.le, simplify=None, ret=1)

    Eq << Eq[-1].subs(Eq.add_ab.reversed)

    Eq << Eq[-1].this.find(Element).apply(sets.el.then.eq.sum, Eq[-1].find(Sum[2]).find(Sum), simplify=None, ret=0)

    Eq << Eq[-1].this.expr.args[:2].apply(algebra.eq.cond.then.cond.subs, simplify=None)

    Eq << Eq[-1].this.expr.args[1].apply(sets.el.then.eq.sum, Eq[-1].expr.args[0].lhs.args[3])

    Eq << Eq[-1].this.expr.apply(algebra.eq.cond.then.cond.subs, simplify=None)

    Eq << algebra.any.then.any.et.limits.cond.apply(Eq[-1], 0, simplify=None)

    Eq << algebra.any_et.then.any.limits_absorb.apply(Eq[-1], -1, simplify=None)

    Eq << Eq[-1].this.expr.apply(algebra.eq.cond.then.cond.subs, reverse=True)

    Eq << sets.eq_cup.eq_cup.notin.then.eq.sum.apply(Eq[0], Eq[2], Eq[5], Eq.eq_X_union.rhs.find(Sum))
    ...
    Eq << sets.eq_cup.eq_cup.notin.then.eq.sum.apply(Eq[0], Eq[2], Eq[5], Eq.eq_X_union.rhs)

    Eq << Eq[-2].subs(Eq[-1].reversed)

    Eq << algebra.cond.any.then.any.et.apply(Eq[1] & Eq[3], Eq[-1], simplify=None)

    Eq << algebra.any.then.any.et.limits.unleash.apply(Eq[-1], simplify=None)

    Eq << algebra.any_et.then.any.limits_absorb.apply(Eq[-1], -2, simplify=None)

    Eq << Eq[-1].this.expr.apply(sets.eq.eq.eq.el.then.eq.sum, Eq[-1].find(Sum[2]).find(Sum), simplify=None, ret=slice(None))

    Eq << algebra.any.then.any.et.limits.unleash.apply(Eq[-1], simplify=None)

    Eq << Eq[-1].this.expr.args[3:5].apply(algebra.eq.cond.then.cond.subs, simplify=None)

    Eq << algebra.any_et.then.any.limits_absorb.apply(Eq[-1], -2, simplify=None)

    Eq << Eq[-1].this.expr.apply(sets.eq.eq.eq.el.then.eq.sum, Eq[-1].find(Sum[2]), simplify=None)
