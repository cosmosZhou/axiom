...
def prove(Eq):
    ...
    Eq << sets.then.le.union.apply(*Eq[-1].lhs.arg.args)
    ...
    Eq << algebra.le.le.then.le.subs.apply(Eq[-1], Eq[0])

    Eq << Eq.induct.this.rhs.apply(algebra.sum.to.add.pop)

    Eq << Infer(Eq[0], Eq.induct, plausible=True)

    Eq << algebra.cond.infer.then.cond.induct.apply(Eq[1], Eq[-1], n=n, start=1)
