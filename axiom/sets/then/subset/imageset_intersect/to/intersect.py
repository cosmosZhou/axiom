...
def prove(Eq):
    ...
    Eq << Eq[-1].this.rhs.apply(sets.el_intersect.of.et, simplify=False)

    Eq << Eq[-1].apply(algebra.infer.of.et.infer)
    ...
    Eq <<= Eq[-2].this.rhs.apply(sets.el_union.of.ou, simplify=False), Eq[-1].this.rhs.apply(sets.el_union.of.ou, simplify=False)

    Eq << sets.infer.then.subset.apply(Eq.suffice)
