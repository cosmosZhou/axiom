...
def prove(Eq):
    ...
    Eq << algebra.lt_zero.then.eq.sup.to.mul.inf.apply(Eq[0], Eq[-1].lhs)

    Eq << Eq[-1].this.find(Inf).expr.expand()

    Eq.is_positive = -Eq[0]

    Eq << algebra.gt_zero.then.eq.inf.to.mul.apply(Eq.is_positive, Eq[-1].find(Inf))

    Eq << -Eq[-1].this.lhs.expr.expand()

    Eq << Eq[-3].subs(Eq[-1].reversed)

    Eq << sets.gt_zero.el.then.eq.inf.st.quadratic.apply(Eq.is_positive, Eq[1], Eq[-1].find(Inf).expr, x)
    Eq << Eq[-2].subs(Eq[-1])
