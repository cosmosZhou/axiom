...
def prove(Eq):
    ...
    Eq <<= algebra.lt.then.eq.max.apply(Eq[0]), algebra.ge.then.eq.min.apply(Eq[1])

    Eq <<= Eq[-3].rhs.args[1].this.subs(Eq[-2]), Eq[-3].rhs.args[0].this.subs(Eq[-1])

    Eq <<= sets.ge_zero.then.eq.cup.to.interval.right_open.apply(Eq[1], k), sets.lt_zero.then.eq.cup.to.interval.right_open.apply(Eq[0], k)

    Eq <<= Eq[-4].subs(Eq[-2]), Eq[-3].subs(Eq[-1])

    Eq << Eq[3].subs(Eq[-1], Eq[-2])

    Eq << sets.lt.le.then.eq.union.interval.right_open.apply(Eq[0], Eq[1].reversed, right_open=True)
