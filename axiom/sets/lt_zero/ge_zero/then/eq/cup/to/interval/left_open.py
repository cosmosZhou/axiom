...
def prove(Eq):
    ...
    Eq <<= sets.ge_zero.then.eq.cup.to.interval.left_open.apply(Eq[1], k), sets.lt_zero.then.eq.cup.to.interval.left_open.apply(Eq[0], k)

    Eq <<= Eq[-4].subs(Eq[-2]), Eq[-3].subs(Eq[-1])

    Eq << Eq[3].subs(Eq[-1], Eq[-2])

    Eq << sets.lt.le.then.eq.union.interval.left_open.apply(Eq[0], Eq[1].reversed, left_open=True)
