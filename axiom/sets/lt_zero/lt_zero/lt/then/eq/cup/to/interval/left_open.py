...
def prove(Eq):
    ...
    Eq.min_b0 = algebra.lt.then.eq.min.apply(Eq[1])

    Eq << algebra.lt.then.eq.max.apply(Eq[2])

    Eq <<= Eq[-2].rhs.args[0].this.subs(Eq.min_b0), Eq[-2].rhs.args[1].this.subs(Eq[-1])

    Eq << sets.lt_zero.then.eq.cup.to.interval.left_open.apply(Eq[1], k)

    Eq << Eq[-2].subs(Eq[-1])

    Eq << Eq[4].subs(Eq[-1], Eq[-4]).reversed

    Eq << sets.lt_zero.then.eq.cup.to.interval.left_open.apply(Eq[0], k)

    Eq << Eq[-2].subs(Eq[-1])

    interval_b = Eq[-1].lhs.args[0]
    Eq << sets.eq.then.eq.complement.apply(Eq[-1], interval_b)

    Eq << algebra.lt.then.eq.max.apply(Eq[0])
    ...
    Eq << Eq[-1].this.expr.apply(algebra.et.then.cond, slice(0, 2))

    Eq << algebra.cond.any.then.any.et.apply(Eq[1], Eq[-1])

    Eq << Eq[-1].this.expr.args[1:].apply(algebra.lt.lt.then.lt.trans, ret=1)

    Eq << Eq[-1].this.expr.args[1].apply(algebra.lt.then.le.strengthen.plus)

    Eq << Eq[-1].this.expr.find(LessEqual).apply(algebra.le.then.eq.min)

    Eq << Eq[-1].this.expr.args[:2].apply(algebra.eq.cond.then.cond.subs)

    Eq << Eq[-1].this.expr.args[1].apply(algebra.lt.then.eq.max, ret=0)

    Eq << Eq[-1].this.expr.args[:2].apply(algebra.eq.cond.then.cond.subs)

    Eq << Eq[-1].this.expr.args[0].apply(sets.interval_ne_empty.then.gt)

    Eq << Eq[-1].this.find(Greater).apply(algebra.gt.then.ge.strengthen)

    Eq << sets.eq.eq.then.eq.union.apply(Eq.eq_complement, Eq.is_empty)
