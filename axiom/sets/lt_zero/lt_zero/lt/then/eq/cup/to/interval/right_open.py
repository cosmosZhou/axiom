...
def prove(Eq):
    ...
    Eq << sets.lt_zero.then.eq.cup.to.interval.right_open.apply(Eq[1], k)

    Eq << Eq[-2].subs(Eq[-1])

    Eq << Eq[4].subs(Eq[-1], Eq[-4]).reversed

    Eq << sets.lt_zero.then.eq.cup.to.interval.right_open.apply(Eq[0], k)
    ...
    Eq << Eq[-1].this.find(Expr <= 0).apply(algebra.le.then.eq.min)
