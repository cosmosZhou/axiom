...
def prove(Eq):
    ...
    Eq.ge, Eq.lt = sets.el_range.then.et.apply(Eq[0])

    Eq << Eq.ge.this.apply(algebra.ge.transport, rhs=0)

    Eq << Eq.lt.this.apply(algebra.lt.transport, rhs=-1)

    Eq << sets.lt.ge.then.el.range.apply(Eq[-1], Eq[-2])

    Eq <<= Eq[1] & Eq[-1]

    Eq << sets.el_range.then.et.apply(Eq[1])

    Eq <<= algebra.ge.ge.then.ge.trans.apply(Eq.ge, Eq[-2] + d), algebra.lt.then.le.strengthen.apply(Eq[-1])

    Eq << algebra.lt.le.then.lt.trans.apply(Eq.lt, Eq[-1] + n)

    Eq << sets.lt.ge.then.el.range.apply(Eq[-1], Eq[-3])
