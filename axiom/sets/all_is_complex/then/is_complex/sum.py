...
def prove(Eq):
    ...
    Eq << algebra.infer.then.infer.et.both_sided.apply(Eq[2], cond=Element(x[n], S.Complexes))

    Eq << Eq[-1].this.lhs.apply(algebra.cond.all.of.all.push)

    Eq << Eq[-1].this.rhs.apply(sets.is_complex.is_complex.then.is_complex.add)
