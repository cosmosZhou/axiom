...
def prove(Eq):
    ...
    Eq <<= sets.el_finiteset.then.ou.eq.apply(Eq[0]), sets.el_finiteset.then.ou.eq.apply(Eq[1])

    Eq <<= Eq[-2].this.args[0].apply(algebra.eq.then.eq.invoke, f), Eq[-1].this.args[0].apply(algebra.eq.then.eq.invoke, f)
