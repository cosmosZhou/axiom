...
def prove(Eq):
    ...
    Eq << Eq.induct.this.lhs.apply(Sets.Cap.eq.Intersect.split, {n})

    Eq.induct_dissected = Eq[-1].this.find(Intersection[~Cap]).apply(Sets.Cap.eq.Intersect.doit.outer.setlimit)

    s = Symbol(Cap[j:n + 1](f[i] | g[i, j]))
    Eq << s.this.definition

    Eq << Eq[-1].apply(Sets.Eq.to.Eq.Cap, (i, 0, m))

    Eq << Eq[-2].this.rhs.apply(Sets.Cap.eq.Intersect.split, {n})

    Eq << Eq[-2].subs(Eq[-1])

    Eq << Eq[-1].this.lhs.apply(Sets.Cap.eq.Intersect)

    Eq << Eq.induct_dissected.subs(Eq[-1].reversed)

    Eq << Sets.Eq.to.Eq.Intersect.apply(Eq[0], Eq[-1].find(Cap))
