...
def prove(Eq):
    ...
    Eq << Eq.y_definition.apply(algebra.cond.then.all.restrict, (i, 0, n))

    Eq.yi_definition = Eq[-1].this().expr.rhs.simplify()

    Eq << Eq.yi_definition.reversed

    Eq << algebra.all.all.then.all.et.apply(Eq[0], Eq[-1], simplify=None)

    Eq << Eq[-1].this.expr.apply(algebra.eq.cond.then.cond.subs)

    Eq << algebra.all.all.then.all.et.apply(Eq[-1], Eq[-3].limits_subs(i, j), simplify=None)

    Eq.nonoverlapping = Eq[-1].this.expr.apply(algebra.eq.cond.then.cond.subs)

    Eq << Eq.y_definition.apply(algebra.cond.then.all.restrict, (i, n, oo))

    Eq << Eq[-1].this().expr.rhs.simplify()

    Eq << Eq[-1].this.expr.apply(sets.eq.then.eq.intersect, y[j])

    Eq << Eq[-1].apply(algebra.cond.then.all.restrict, (j, j_domain))

    Eq <<= Eq[-1] & Eq.nonoverlapping

    Eq << sets.all_is_empty.then.eq.nonoverlapping.setlimit.utility.apply(Eq[-1])

    Eq << algebra.all_eq.cond.then.all.subs.apply(Eq.yi_definition, Eq[-1])
