...
def prove(Eq):
    ...
    Eq << algebra.all.then.cond.subs.apply(Eq[-1], j, 0)

    Eq << sets.then.eq.principle.inclusion_exclusion.basic.apply(*Eq[-1].lhs.args).subs(Eq[-1])

    Eq.induct = Eq[1].subs(n, n + 1)

    Eq << Eq.induct.lhs.arg.this.apply(sets.cup.to.union.split, cond={n})

    Eq << sets.then.eq.principle.inclusion_exclusion.basic.apply(*Eq[-1].rhs.args)
