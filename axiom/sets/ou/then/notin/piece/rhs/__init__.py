...
def prove(Eq):
    ...
    Eq << Eq[0].this.find(Element[Complement]).apply(sets.el_complement.then.et, simplify=None)

    Eq << Eq[-1].this.find(NotElement[Union]).apply(sets.notin.then.et.split.union, simplify=None)

    Eq << Eq[-1].apply(algebra.ou.then.ou.collect, cond=NotElement(x, A))

    Eq << Eq[-1].this.find(Or).apply(sets.ou.then.notin.piece.rhs.two, wrt=y)

    Eq << Eq[-1].apply(sets.ou.then.notin.piece.rhs.two, wrt=y)
