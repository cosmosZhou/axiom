...
def prove(Eq):
    ...
    Eq << Eq[-1].this.find(Less).apply(algebra.maxima_lt.of.all.lt)
    ...
    Eq.eq, Eq.all_gt, Eq.all_ne = algebra.et.of.et.apply(Eq[-1], None)
    ...
    Eq << Eq.all_gt.this.expr.apply(algebra.gt.of.et)

    Eq.all_ne_piece, Eq.all_ge = algebra.all_et.of.et.all.apply(Eq[-1])

    Eq << algebra.all.of.infer.apply(Eq.all_ne_piece)
    ...
    Eq <<= Eq[-2].this.lhs.apply(sets.lt.el_range.then.el.range.intersect), Eq[-1].this.lhs.apply(sets.ge.el_range.then.el.range.intersect)

    Eq <<= algebra.infer.of.all.apply(Eq[-2]), algebra.infer.of.all.apply(Eq[-1])

    Eq << Eq[-1].limits_subs(i, i - 1)

    Eq << algebra.all.then.cond.subs.apply(Eq[0], i, k)

    Eq << Eq[0].this.apply(algebra.all.limits.swap.intlimit)

    Eq << algebra.all.then.cond.subs.apply(Eq[-1], j, k)

    Eq << Eq[-1].this.expr.reversed

    Eq << algebra.all.of.infer.apply(Eq.all_ge)
    ...
    Eq << algebra.eq_reducedArgMax.then.all.ge.apply(Eq.k_def)

    Eq << algebra.all.then.et.all.split.apply(Eq[-1], cond=i < k)

    Eq << algebra.all.then.all.limits.restrict.apply(Eq[-1], Range(k + 1, n + 1))

    Eq << algebra.all.of.infer.apply(Eq.all_ne)
    ...
    Eq <<= Eq[-2].this.lhs.args[::2].apply(sets.lt.el_range.then.el.range.intersect), Eq[-1].this.lhs.args[::2].apply(sets.ge.el_range.then.el.range.intersect)

    Eq <<= Eq[-2].this.rhs.apply(algebra.cond_piece.of.et.infer), Eq[-1].this.rhs.apply(algebra.cond_piece.of.et.infer)

    Eq <<= algebra.infer.of.et.infer.apply(Eq[-2]), algebra.infer.of.et.infer.apply(Eq[-1])
    ...
    Eq << Eq[-2].this.find(Element[2]).apply(sets.el_range.then.gt.domain)

    Eq <<= Eq[-4].this.lhs.args[:2].apply(sets.lt.el_range.then.el.range.intersect), \
        Eq[-3].this.lhs.args[:2].apply(sets.ge.el_range.then.el.range.intersect),\
        Eq[-1].this.lhs.args[:2].apply(sets.ge.el_range.then.el.range.intersect)

    Eq <<= Eq[-3].this.apply(algebra.infer.fold, 0),\
        Eq[-2].this.apply(algebra.infer.fold, 0),\
        Eq[-1].this.apply(algebra.infer.fold, 0)

    Eq <<= algebra.infer.of.all.apply(Eq[-3]), \
        algebra.infer.of.all.apply(Eq[-2]), \
        algebra.infer.of.all.apply(Eq[-1])

    Eq <<= Eq[-3].this.expr.apply(algebra.infer.of.all), \
        Eq[-2].this.expr.apply(algebra.infer.of.all), \
        Eq[-1].this.expr.apply(algebra.infer.of.all)
