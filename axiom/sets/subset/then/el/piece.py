...
def prove(Eq):
    ...
    sets.subset.then.el
    Eq << sets.subset.then.el.apply(Eq[0])

    Eq.bool_fx = sets.el.then.eq.bool.el.apply(Eq[-1])

    Eq << sets.subset.then.el.apply(Eq[0], 1)

    Eq.bool_gx = sets.el.then.eq.bool.el.apply(Eq[-1])

    Eq << sets.subset.then.el.apply(Eq[0], 2)

    Eq.bool_hx = sets.el.then.eq.bool.el.apply(Eq[-1])
    ...
    Eq << sets.ou.then.el.piece.apply(Eq[-1], wrt=S)
