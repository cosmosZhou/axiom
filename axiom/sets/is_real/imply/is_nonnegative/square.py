...
def prove(Eq):
    ...
    Eq << sets.is_real.is_real.imply.is_real.apply(Eq[0], Eq[0])

    Eq << sets.el_interval.imply.et.apply(Eq[-1])

    Eq << sets.is_real.imply.eq.square.abs.apply(Eq[0])

    Eq <<= Eq[1].subs(Eq[-1].reversed), Eq[2].subs(Eq[-1].reversed)

    
    Eq <<= sets.el_interval.given.et.apply(Eq[-2]),sets.el_interval.imply.et.apply(Eq[-1])
