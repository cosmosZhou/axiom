...
def prove(Eq):
    ...
    Eq << sets.eq_card.imply.any.eq.apply(Eq[0], x)

    Eq << algebra.any.imply.any.et.limits.unleash.apply(Eq[-1])

    Eq << algebra.any_et.imply.any.limits_absorb.apply(Eq[-1], 0)

    Eq << Eq[-1].this.expr.apply(sets.all_ne.imply.any.eq.cup.finiteset)

    Eq << algebra.any.imply.any.et.limits.unleash.apply(Eq[-1], 1)
