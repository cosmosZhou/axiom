...
def prove(Eq):
    ...
    Eq << algebra.cond.of.et.infer.split.apply(Eq[-1], cond=Equal(x, S.Pi))
    ...
    Eq << Eq[-1].this.rhs.args[::2].apply(sets.ne.el.then.el.complement, simplify=None)

    Eq << Eq[-1].this.rhs.apply(sets.ne.el.then.el.complement)

    Eq << Eq[-1].this.rhs.apply(geometry.el_interval.then.gt_zero.sin)

    Eq << Eq[-1].this.rhs.apply(algebra.gt_zero.then.ge_zero)
