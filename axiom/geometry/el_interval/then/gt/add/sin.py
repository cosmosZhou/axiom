...
def prove(Eq):
    ...
    Eq.eq_grad = algebra.eq.eq.then.eq.trans.apply(Eq.eq_grad, Eq[-1])

    Eq << algebra.cond.then.cond.domain_defined.apply(Eq.eq_grad)

    Eq << algebra.ne_zero.then.gt_zero.square.apply(Eq[-1])

    Eq << sets.el_interval.then.gt.apply(Eq[0])

    Eq << geometry.gt_zero.then.sin_lt.apply(Eq[-1])

    Eq << algebra.lt.then.gt_zero.apply(Eq[-1])

    Eq << algebra.gt_zero.then.gt_zero.square.apply(Eq[-1])

    Eq << algebra.gt_zero.gt_zero.then.gt_zero.div.apply(Eq[-1], Eq[-5]) / 2

    Eq << algebra.eq.gt.then.gt.trans.apply(Eq[-1], Eq.eq_grad)

    Eq << algebra.cond.then.all.restrict.apply(Eq[-1], (x, Interval(0, S.Pi, right_open=True)))

    Eq << calculus.all_gt_zero.then.all.gt.monotony.right_open.apply(Eq[-1])
