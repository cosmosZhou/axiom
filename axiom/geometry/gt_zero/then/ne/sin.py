...
def prove(Eq):
    ...
    Eq.eq = algebra.eq.eq.then.eq.trans.apply(Eq.f0, Eq[-1])
    ...
    Eq << calculus.lt.is_continuous.is_differentiable.eq.then.any.eq.Rolle.apply(Eq.lt, Eq.all_eq, Eq.all_el, Eq.eq)
    ...
    Eq << geometry.then.el.sin.apply(x)

    Eq << algebra.cond.any.then.any.et.apply(Eq[-2], Eq[-1])

    Eq << Eq[-1].subs(Eq[2])

    Eq << Eq[-1].this.find(Element).apply(sets.el_interval.then.le)

    Eq << Eq[-1].this.expr.args[1::2].apply(algebra.lt.le.then.lt.trans)

    Eq << Eq[-1].this.expr.args[1:].apply(sets.lt.gt.then.el.interval, simplify=None)

    Eq << Eq[-1].this.find(Element).apply(geometry.el_interval.then.gt_zero.sin)

    Eq << Eq[-1].this.find(Equal).reversed

    Eq << geometry.then.eq.add.square.apply(t)

    Eq << algebra.cond.any.then.any.et.apply(Eq[-2], Eq[-1])

    Eq << Eq[-1].this.expr.args[:2].apply(algebra.eq.eq.then.eq.subs, swap=True)

    Eq << Eq[-1].this.find(Expr > 0).apply(algebra.gt_zero.then.gt_zero.square)
