...
def prove(Eq):
    ...
    Eq << Eq.induct.this.find(Mul).apply(Algebra.Mul.eq.Add).reversed

    Eq << Eq[-1].this.lhs.apply(Geometry.Cos.Neg)
    Eq << Imply(Eq[2], Eq.induct, plausible=True)

    Eq << Algebra.Cond.Imply.to.Cond.induct.apply(Eq[0], Eq[-1], n=n, start=0)

    Eq << Algebra.Cond.to.All.apply(Eq[2], n)
