...
def prove(Eq):
    ...
    Eq << Sets.Gt.In_Interval.to.In.Interval.Intersect.apply(Eq.gt, Eq[1])

    Eq << Geometry.In_Interval.to.Lt_0.Cos.apply(Eq[-1])

    Eq <<= Eq[-1] & Eq[0]

    Eq.lt = Less(x, S.Pi / 2, plausible=True)

    Eq << Sets.Lt.In_Interval.to.In.Interval.Intersect.apply(Eq.lt, Eq[1])

    Eq << Geometry.In_Interval.to.Gt_0.Cos.apply(Eq[-1])
