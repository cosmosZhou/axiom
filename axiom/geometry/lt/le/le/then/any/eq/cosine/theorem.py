...
def prove(Eq):
    ...
    Eq << algebra.le.le.then.le.quadratic.apply(Eq.x_bound, Eq.y_bound)
    ...
    Eq << Eq[-1].apply(sets.gt.le.then.el.interval)

    Eq << sets.el.then.el.div.interval.apply(Eq[-1], 2 * x * y)

    Eq << sets.el.then.el.acos.interval.apply(Eq[-1])
