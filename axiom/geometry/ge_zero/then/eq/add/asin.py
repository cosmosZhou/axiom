...
def prove(Eq):
    ...
    Eq.any_eq = geometry.cos_is_zero.then.any.eq.apply(Eq[-1])

    Eq << LessEqual(x, 1, plausible=True)

    Eq << sets.le.ge.then.el.interval.apply(Eq[-1], Eq[0])

    Eq <<= geometry.el.then.el.asin.apply(Eq[-1]), sets.el.then.el.sqrt.max.apply(Eq[-1])

    Eq << geometry.el.then.el.asin.apply(Eq[-1])

    Eq << sets.el.el.then.el.add.interval.apply(Eq[-1], Eq[-3])
