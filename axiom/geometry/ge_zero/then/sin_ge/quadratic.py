...
def prove(Eq):
    ...
    Eq << algebra.cond.of.et.infer.split.apply(Eq[1], cond=x > S.Pi)

    Eq << (x <= 0).this.apply(geometry.le_zero.then.sin_ge.quadratic)

    Eq << Eq[-1].subs(x, S.Pi - x)

    Eq << Eq[-1].this.lhs.apply(algebra.le_zero.of.ge)
    ...
    Eq << algebra.infer.of.et.infer.split.apply(Eq[3], cond=x > S.Pi / 2)

    Eq << algebra.infer.of.et.infer.invert.apply(Eq[-1], cond=x >= 0)

    Eq << Eq[-1].this.lhs.apply(sets.le.ge.then.el.interval)

    Eq << Eq[-1].this.lhs.apply(geometry.el_interval.then.sin_ge.quadratic)
