...
def prove(Eq):
    ...
    Eq << algebra.infer.of.et.infer.split.apply(Eq[-1], cond=x >= 0)

    Eq << algebra.infer.of.infer.subs.apply(Eq[-2])

    Eq << algebra.infer.of.cond.invert.apply(Eq[-1])

    Eq.lt, Eq.ge = algebra.infer.of.et.infer.split.apply(Eq[2], cond=x < S.Pi)

    Eq << Eq.lt.this.lhs.apply(sets.lt.gt.then.el.interval)

    Eq << Eq[-1].this.lhs.apply(geometry.el_interval.then.sin_gt)

    Eq << Eq[-1].this.lhs.apply(algebra.gt.then.ge.relax)

    Eq << algebra.infer.of.et.infer.split.apply(Eq.ge, cond=x > S.Pi)
    ...
    Eq << algebra.infer.of.infer.et.apply(Eq[-1])

    Eq << Eq[-1].this.rhs.apply(algebra.gt_zero.gt.of.et.div)

    Eq << algebra.infer.then.infer.et.apply(Eq.gt_1)

    Eq << Eq[-1].this.rhs.apply(algebra.gt_zero.gt.then.gt.mul)

    Eq << Eq[-1].lhs.this.apply(geometry.gt_zero.then.sin_le)

    Eq <<= Eq[-1] & Eq[-2]

    Eq << Eq[-1].this.rhs.apply(algebra.gt.le.then.lt.trans)

    Eq << Eq[-1].this.rhs.apply(algebra.lt.then.le.relax)
