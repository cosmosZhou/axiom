...
def prove(Eq):
    ...
    Eq.gt, Eq.lt = sets.el_interval.then.et.apply(Eq[0])

    Eq << Element(a, Interval(-1, 1), plausible=True)

    Eq << Element(b, Interval(-1, 1), plausible=True)

    Eq << sets.el.el.then.subset.interval.right_open.apply(Eq[-2], Eq[-1])

    Eq << Subset(Interval(a, b, left_open=True, right_open=True), Interval(a, b, right_open=True), plausible=True)

    Eq << sets.subset.subset.then.subset.trans.apply(Eq[-1], Eq[-2])

    Eq << sets.el.subset.then.el.apply(Eq[0], Eq[-1])

    Eq << Element(b, Interval(-1, 1, right_open=True), plausible=True)

    Eq << geometry.lt.el.el.then.gt.acos.apply(Eq.lt, Eq[-2], Eq[-1])

    Eq << geometry.lt.el.el.then.gt.acos.apply(Eq.gt.reversed, Eq[4], Eq[-2]).reversed
