...
def prove(Eq):
    ...
    Eq << Eq[-1].this.lhs.apply(geometry.tan.to.piece)

    Eq << Eq[-1].this.find(Tan[Piecewise]).apply(geometry.tan.to.piece)

    Eq << Eq[-1].this.find(Tan[Piecewise]).apply(geometry.tan.to.piece, simplify=0)
