...
@apply
def apply(self):
    lhs, rhs = self.of(KL[Probability[Equal], Probability[Equal]])
    if lhs[-1].is_Tuple:
    ...
    if limits_lhs:
        ...
        limits = [(x,)]         
    return Equal(self, Expectation(log(Probability(Equal(x, x.surrogate), *limits_lhs) / Probability(Equal(x_quote, x.surrogate), *limits_rhs)), *limits))
