...
def prove(Eq):
    ...
    i = Eq[-1].lhs.variable
    Eq << stats.ne_zero.then.ne_zero.joint_slice.apply(Eq[1], [i, None])

    Eq << stats.ne_zero.eq.then.eq.mul.prod.prob.conditioned.apply(Eq[0], Eq[-1])

    Eq << algebra.eq.then.eq.argmax.apply(Eq[-1], (i,))
