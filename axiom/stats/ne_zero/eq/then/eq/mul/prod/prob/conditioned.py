...
def prove(Eq):
    ...
    Eq << stats.ne_zero.then.ne_zero.joint_slice.apply(Eq[1], [None, slice(0, t)])

    Eq.yt_given_y_historic = stats.ne_zero.eq_conditioned.then.eq.conditioned.joint.apply(Eq[0], Eq[-1])

    Eq.yt_given_x_nonzero = stats.ne_zero.then.ne_zero.conditioned.apply(Eq[-1], x)

    Eq << stats.ne_zero.then.eq.bayes.conditioned.apply(Eq.yt_given_x_nonzero, y[t])

    Eq << Eq[-1].this.lhs.find(And).apply(algebra.eq.eq.then.eq.concat)

    Eq << Eq[-1].subs(Eq.yt_given_y_historic)

    Eq << algebra.ne_zero.eq.then.eq.scalar.apply(Eq[-1], Eq.yt_given_x_nonzero)

    Eq << algebra.eq.then.eq.prod.apply(Eq[-1], (t, 1, m))
