...
def prove(Eq):
    ...
    Eq << Eq[-1].this.lhs.arg.apply(algebra.et.concat, i=0, j=1)

    Eq << Eq[-1].this.lhs.arg.apply(algebra.et.concat, i=1, j=2)

    Eq << Eq[-1].this.lhs.arg.apply(algebra.et.concat, i=2, j=3)
    ...
    Eq << stats.ne_zero.eq.imply.eq.conditioned.joint.apply(Eq[1], Eq[-1])
    ...
    Eq << stats.ne_zero.imply.ne_zero.joint_slice.apply(Eq[-1], [k, k + 1])

    Eq << stats.ne_zero.eq.eq.imply.eq.prob.conditioned.apply(Eq[0], Eq[2], Eq[-1])
