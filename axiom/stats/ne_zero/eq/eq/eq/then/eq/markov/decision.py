...
def prove(Eq):
    ...
    Eq.ne_zero = stats.ne_zero.then.ne_zero.joint_slice.apply(Eq[3], [slice(0, k), slice(0, k), slice(0, k + 1)])

    Eq << stats.ne_zero.then.eq.prob.to.mul.prob.bayes.apply(Eq.ne_zero, s[k + 1], a[k], r[k])
    ...
    Eq.recursion = algebra.ne_zero.eq.then.eq.scalar.apply(Eq.ne_zero, Eq[-1])

    Eq << stats.ne_zero.then.ne_zero.joint_slice.apply(Eq[3], [slice(0, k + 1), slice(0, k), slice(0, k + 1)])
    ...
    Eq << stats.ne_zero.ne_zero.then.ne_zero.conditioned.apply(Eq.ne_zero, Eq[-1])

    Eq << stats.ne_zero.then.eq.bayes.conditioned.apply(Eq[-1], r[k], s[k + 1])
    ...
    Eq << stats.ne_zero.eq_conditioned.then.eq.conditioned.joint.apply(Eq[1], Eq[-1])

    Eq << Eq[-3].subs(Eq[-1])

    Eq.recursion = Eq.recursion.subs(Eq[-1])

    Eq.ne_zero_a, Eq[-1] = stats.ne_zero.then.et.ne_zero.apply(Eq[3], 1)

    Eq.ne_zero_r, Eq.ne_zero_s = stats.ne_zero.then.et.ne_zero.apply(Eq[-1])

    Eq << stats.eq_conditioned.eq_conditioned.then.eq.prob.joint.apply(Eq[0], Eq[2])

    Eq << stats.ne_zero.then.ne_zero.conditioned.apply(Eq.ne_zero_sar, a[k], s[k])

    Eq << stats.ne_zero.eq.then.eq.conditioned.joint.st.conditioned.apply(Eq[-2], Eq[-1])

    Eq.recursion = Eq.recursion.subs(Eq[-1])

    Eq << algebra.eq.then.eq.prod.apply(Eq.recursion, (k, 1, T))
    ...
    Eq << stats.ne_zero.then.ne_zero.slice.apply(Eq.ne_zero_s, 0)

    Eq << stats.ne_zero.then.eq.prob.to.mul.prob.bayes.apply(Eq[-1], a[0], r[0], s[1])

    Eq.final = Eq[-3].subs(Eq[-1])

    Eq << stats.ne_zero.then.et.ne_zero.apply(Eq[3], slice(0, None, 2))[0]

    Eq << stats.ne_zero.then.ne_zero.joint_slice.apply(Eq[-1], [0, 0])

    Eq << stats.ne_zero.then.ne_zero.conditioned.apply(Eq[-1], s[0])

    Eq << stats.ne_zero.then.eq.bayes.conditioned.apply(Eq[-1], r[0], s[1])
