...
def prove(Eq):
    ...
    Eq << algebra.infer.of.et.infer.invert.apply(Eq[-1], cond=a < 0)

    Eq <<= algebra.cond.infer.of.et.infer.et.apply(Eq[1], Eq[-3]), algebra.cond.infer.of.et.infer.et.apply(Eq[1], Eq[-1])

    Eq << Eq[-2].this.lhs.apply(stats.gt_zero.distributed.then.distributed.scale.gaussian, b)

    Eq << Eq[-1].this.lhs.apply(stats.lt_zero.distributed.then.distributed.scale.gaussian, b)
