...
def prove(Eq):
    ...
    Eq.ne_zero_xy = stats.ne_zero.imply.ne_zero.delete.apply(Eq[0], 0)

    Eq.eq_div = stats.ne_zero.imply.eq.prob.conditioned.to.div.prob.conditioned.bayes.apply(Eq[0], slice(2, None, -2))

    Eq << stats.ne_zero.imply.ne_zero.delete.apply(Eq.ne_zero_xy)

    Eq << stats.ne_zero.imply.eq.prob.to.mul.prob.bayes.apply(Eq[-1], y)

    Eq << stats.ne_zero.imply.eq.prob.conditioned.to.mul.prob.conditioned.bayes.apply(Eq.ne_zero_xy, y_tilde_overline, y)

    Eq << stats.ne_zero.eq_conditioned.imply.eq.conditioned.joint.apply(Eq.ne_zero_xy, Eq[1])
