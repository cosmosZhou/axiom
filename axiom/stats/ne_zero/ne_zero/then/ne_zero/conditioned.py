...
def prove(Eq):
    ...
    Eq << stats.ne_zero.then.eq.prob.to.mul.prob.bayes.apply(Eq[0], x)

    Eq << algebra.ne_zero.eq.then.eq.div.apply(Eq[0], Eq[-1])

    Eq << algebra.ne_zero.ne_zero.then.ne_zero.div.apply(Eq[1], Eq[0])

    Eq << algebra.ne.eq.then.ne.trans.apply(Eq[-1], Eq[-2])
