...
def prove(Eq):
    ...
    Eq << stats.ne_zero.then.eq.prob.to.mul.prob.bayes.apply(Eq[0], y, spread_weight=True)

    Eq << stats.ne_zero.then.eq.prob.to.mul.prob.bayes.apply(Eq[1], y, spread_weight=True)
    ...
    Eq << stats.then.ge_zero.conditioned.KL.apply(*Eq[-1].find(KL).args)

    Eq << algebra.ge_zero.then.ge_zero.mul.apply(Eq[-1], Probability[θ](Equal(x, x.var)))

    Eq << calculus.ge_zero.then.ge_zero.integral.apply(Eq[-1], (x.var,))
