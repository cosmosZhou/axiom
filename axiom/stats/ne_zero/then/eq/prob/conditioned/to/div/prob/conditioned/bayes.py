...
def prove(Eq):
    ...
    Eq << stats.ne_zero.then.et.ne_zero.apply(Eq[0])

    Eq <<= stats.ne_zero.then.eq.prob.to.mul.prob.bayes.apply(Eq[-2], y), stats.ne_zero.then.eq.prob.to.mul.prob.bayes.apply(Eq[-1], x)

    Eq << algebra.ne_zero.eq.then.eq.scalar.apply(Eq[-1], Eq[-3]).reversed
