...
def prove(Eq):
    ...
    Eq << stats.ne_zero.then.eq.prob.to.mul.prob.bayes.apply(Eq[-1], x, y)

    Eq.lhs = algebra.ne_zero.eq.then.eq.div.apply(Eq[-2], Eq[-1])

    Eq << stats.ne_zero.then.ne_zero.joint.apply(Eq[0])

    Eq << stats.ne_zero.then.eq.prob.to.mul.prob.bayes.apply(Eq[-1], x)

    Eq << algebra.ne_zero.eq.then.eq.div.apply(Eq[-2], Eq[-1]).reversed

    Eq << stats.ne_zero.then.eq.prob.to.mul.prob.bayes.apply(Eq[2], y)
