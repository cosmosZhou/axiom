...
def prove(Eq):
    ...
    Eq << Eq[0].this.lhs.arg.args[-1].apply(algebra.eq.then.et.eq.split, t)

    Eq << Eq[-1].this.lhs.arg.args[0].apply(algebra.eq.then.et.eq.split, t)

    Eq << stats.ne_zero.then.et.ne_zero.apply(Eq[-1])

    Eq << stats.ne_zero.then.ne_zero.delete.apply(Eq[-2], index=1)
