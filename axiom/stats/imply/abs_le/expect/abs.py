...
def prove(Eq):
    ...
    Eq << stats.imply.expect_le.expect.abs.apply(Eq[0].lhs.find(Expectation))

    Eq << stats.imply.expect_le.expect.abs.apply(Eq[0].lhs.find(Expectation)._subs(f(x), -f(x)))

    Eq << -Eq[-1].this.lhs.apply(stats.expect.to.mul)

    Eq << algebra.le.ge.imply.le.abs.apply(Eq[-3], Eq[-1])
