...
def prove(Eq):
    ...
    Eq << Eq[1].this.find(Probability[2]).apply(stats.prob.to.integral.joint, s[1:k])

    Eq << Eq[-1].this.find(Equal & Equal).apply(algebra.et_eq.to.eq.concat)
    ...
    Eq << Eq[-1].this.find(And).args[::2].apply(algebra.et_eq.to.eq.concat)
