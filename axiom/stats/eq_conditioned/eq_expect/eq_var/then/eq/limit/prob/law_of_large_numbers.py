...
def prove(Eq):
    ...
    *Eq[-3:], Eq.hypothesis = apply(Equal(x[k] | x[:k], x[k]), Equal(Expectation(x[k]), μ), Equal(Variance(x[k]), σ ** 2), ε, n)
    ...
    Eq << stats.eq_conditioned.then.eq.var.to.sum.apply(Eq[0], n=n)
    ...
    Eq << stats.eq_expect.eq_var.then.le.prob.Chebyshev_Inequality.apply(Eq[-1], Eq.eq_var, sqrt(n) * ε / σ)

    Eq << Eq[-1].this.lhs.apply(stats.prob.to.sub.complement)

    Eq << algebra.le.then.le.transport.apply(Eq[-1], lhs=0)
