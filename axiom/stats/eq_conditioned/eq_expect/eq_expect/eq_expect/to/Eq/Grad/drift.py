...
def prove(Eq):
    ...
    Eq << Calculus.Eq.to.Eq.Grad.apply(Eq[-1], (π_quote,))

    Eq << Eq[-1].this.rhs.apply(Calculus.Grad.eq.Add)

    Eq << Eq[-1].this.find(Derivative[Expectation]).apply(Stats.Grad.Expect.eq.Expect.Grad)

    Eq << Eq[-1].this.find(Conditioned[~Derivative]).apply(Calculus.Grad.eq.Mul)

    Eq << Eq[-1].subs(π_quote, π)

    Eq << Eq[-1].this.rhs.find(Subs).apply(Stats.Subs.Grad.eq.Zero.st.KL)

    Eq << Eq[-1].this.rhs.apply(Stats.Expect.limits.desimp)
