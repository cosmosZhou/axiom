...
def prove(Eq):
    ...
    *Eq[-6:], (Eq.ge_VF, Eq.ge_reward) = apply(Equal(r[t] | s[:t] & a[:t], r[t]), # history-irrelevant conditional independence assumption for rewards based on states and actions
    ...
    Eq << algebra.cond.then.cond.domain_defined.apply(Eq[3])

    Eq.ne_zero = Eq[-1].subs(t, 0)

    Eq << algebra.eq_argmax.then.all.ge.apply(Eq[5])
    ...
    Eq << algebra.et.of.et.apply(Eq[-1], slice(None, 2))

    Eq << algebra.infer_et.of.infer.et.subs.apply(Eq[-1], 0)

    Eq << algebra.infer_et.of.infer.et.subs.apply(Eq[-1], 1)

    Eq << algebra.cond.then.cond.domain_defined.apply(Eq[0])

    Eq << Eq[4].this.find(Equal[Piecewise]).apply(algebra.cond_piece.then.et.infer)

    Eq << Eq[-1].this.find(Equal[Piecewise]).apply(algebra.cond_piece.then.et.infer)

    Eq << Eq[-1].this.expr.apply(algebra.et.then.et.delete, 1)

    Eq << Eq[-1].this.expr.args[1:].apply(algebra.infer.infer.then.infer.et)

    Eq << Eq[-1].subs(π_quote, π_tilde)

    Eq << algebra.cond.any.then.any.et.apply(Eq[-1], Eq.infer)

    Eq.any = Eq[-1].this.expr.args[1:].apply(algebra.infer.infer.then.infer.trans)
    ...
    Eq << Eq.any.this.find(Infer[2]).apply(algebra.infer.then.eq.piece, Eq[-1].find(Piecewise))

    Eq << algebra.cond.any.then.any.et.apply(Eq[-2], Eq[-1])

    Eq << Eq[-1].this.expr.args[:2].apply(algebra.eq.cond.then.cond.subs)

    Eq << algebra.et.then.et.apply(Eq.ne_zero)[1].subs(s[0].var, s[t].var)

    Eq << algebra.ne_zero.then.gt_zero.apply(Eq[-1])

    Eq << algebra.cond.any.then.any.et.apply(Eq[-1], Eq[-3])

    Eq << Eq[-1].this.expr.args[:2].apply(algebra.gt_zero.ge.then.ge.div)

    Eq << Eq[-1].this.find(GreaterEqual).apply(algebra.ge_zero.then.ge)

    Eq.ge_MDV = GreaterEqual((MDV[π, π_tilde] ^ γ)(s[t].var), (MDV[π, π] ^ γ)(s[t].var), plausible=True)

    Eq << ~Eq.ge_MDV

    Eq << algebra.any.any.then.any.et.apply(Eq[-1], Eq[-2])

    Eq << Eq[-1].this.expr.args[:2].apply(algebra.ge.lt.then.lt.trans)

    Eq << Eq[-1].this.find(Infer).apply(algebra.infer.then.infer.subs)
    ...
    Eq << stats.eq_conditioned.eq_expect.eq_expect.eq_expect.ge.then.et.ge.drift.apply(*Eq[:3], Eq[-1], Eq.ge_MDV)
