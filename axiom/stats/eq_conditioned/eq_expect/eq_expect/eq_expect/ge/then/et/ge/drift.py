...
def prove(Eq):
    ...
    *Eq[-5:], (Eq.ge_VF, Eq.ge_reward) = apply(Equal(r[t] | s[:t] & a[:t], r[t]), # history-irrelevant conditional independence assumption for rewards based on states and actions
    ...
    Eq << algebra.ge.of.ge_zero.apply(Eq.ge_VF)

    Eq.VQ, Eq.VV, Eq.QV = keras.eq_conditioned.eq_expect.eq_expect.then.et.eq.expect.Bellman.apply(*Eq[:3])

    Eq.Q_quote = Eq[1].subs(π, π_quote)

    Eq.V_quote = Eq[2].subs(π, π_quote)

    Eq.VQ_quote, Eq.VV_quote, Eq.QV_quote = keras.eq_conditioned.eq_expect.eq_expect.then.et.eq.expect.Bellman.apply(Eq[0], Eq.Q_quote, Eq.V_quote)

    Eq << Eq[3].subs(π_quote, π)

    Eq << Eq[3] - Eq[-1]

    Eq << algebra.ge.then.ge_zero.apply(Eq[4])

    Eq << Eq[-1].subs(Eq[-2])

    Eq << algebra.ge.then.ge.transport.apply(Eq[-1], lhs=0)
    ...
    Eq << stats.then.expect_ge.inf.apply(Eq.ge.find(Expectation))

    Eq << GreaterEqual(γ, 0, plausible=True)

    Eq << algebra.ge_zero.ge.then.ge.mul.apply(Eq[-1], Eq[-2])

    Eq << -Eq[-1].reversed

    Eq << algebra.ge.ge.then.ge.add.apply(Eq[-1], Eq.ge)

    Eq << Eq[-1].this.find(Inf).limits_subs(s[t + 1].var, s[t].var)

    Eq << algebra.ge.then.ge.inf.apply(Eq[-1], (s[t].var,))

    Eq << Eq[-1].this.lhs.apply(algebra.add.to.mul)

    Eq.gt_zero = Greater(1 - γ, 0, plausible=True)

    Eq.ge = algebra.gt_zero.ge.then.ge.div.apply(Eq.gt_zero, Eq[-1])

    Eq <<= stats.then.ge_zero.KL.apply(Eq.ge.find(KL)), stats.then.ge_zero.prob.apply(Eq.ge.find(Probability)), algebra.cond.then.cond.domain_defined.apply(Eq.ge)

    Eq <<= algebra.ge_zero.ge_zero.then.ge_zero.apply(Eq[-2], Eq[-3]), algebra.ne_zero.then.gt_zero.div.apply(Eq[-1])

    Eq << algebra.gt_zero.ge.then.ge.mul.apply(Eq[-1], Eq[-2])

    Eq << algebra.cond.then.all.apply(Eq[-1], s[t].var, simplify=None)

    Eq << algebra.all_ge.then.inf_ge.apply(Eq[-1])

    Eq << algebra.gt_zero.ge.then.ge.div.apply(Eq.gt_zero, Eq[-1])

    Eq << algebra.ge.ge.then.ge.trans.apply(Eq.ge, Eq[-1])

    Eq << algebra.inf_ge.then.all.ge.apply(Eq[-1])

    Eq <<= Eq[2].subs(s[t].var, s[t]).subs(t, 0), Eq.V_quote.subs(s[t].var, s[t]).subs(t, 0)

    Eq <<= stats.eq.then.eq.expect.apply(Eq[-2]), stats.eq.then.eq.expect.apply(Eq[-1])
    ...
    Eq << stats.ge.then.ge.expect.apply(Eq[-1], (s[0],))
