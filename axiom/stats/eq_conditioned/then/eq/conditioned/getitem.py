...
def prove(Eq):
    ...
    Eq.y_nonzero, Eq.z_nonzero = stats.ne_zero.then.et.ne_zero.apply(Eq[-1])

    Eq.xy_probability = stats.ne_zero.then.eq.prob.to.mul.prob.bayes.apply(Eq.y_nonzero, x)

    Eq << stats.ne_zero.then.eq.prob.to.mul.prob.bayes.apply(Eq[2], x)
    ...
    Eq <<= stats.integral.to.prob.marginal.apply(Integral[z.var](Eq[-1].lhs)), \
        ...
        calculus.eq.then.eq.integral.apply(Eq[-1], (z.var,))
