...
def prove(Eq):
    ...
    Eq << stats.eq_conditioned.then.eq.conditioned.induct.independence_assumption.historic.apply(Eq[0], k)

    Eq << stats.eq_conditioned.eq_conditioned.then.eq.prob.joint.apply(Eq[1], Eq[2])
    ...
    Eq << algebra.eq.infer.then.eq.induct.apply(Eq[0], Eq[-1], k, start=1)
