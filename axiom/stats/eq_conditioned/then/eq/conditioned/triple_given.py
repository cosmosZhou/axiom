...
def prove(Eq):
    ...
    Eq << Eq[0].apply(algebra.cond.then.all.restrict, (k, 2, oo))

    Eq << Eq[-1].this().expr.lhs.rhs.args[1].apply(algebra.eq.then.et.eq.split)

    Eq << stats.eq_conditioned.then.eq.single_condition_w.apply(Eq[-1], wrt=Eq[-1].lhs.rhs.args[-1].lhs)

    Eq << Eq[1].apply(algebra.cond.of.et.all, cond=k >= 2)

    Eq << Eq[-1].this().expr.lhs.rhs.apply(algebra.eq.of.et.eq.block)
