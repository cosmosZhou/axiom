...
def prove(Eq):
    ...
    Eq << Eq[1].this.lhs.apply(algebra.ne.then.ou)

    Eq << algebra.infer_ou.of.et.infer.apply(Eq[-1])

    Eq << stats.eq_conditioned.then.infer.is_zero.cov.using.gt.apply(Eq[0], i)

    Eq << Eq[-1].this.lhs.reversed

    Eq << stats.eq_conditioned.then.infer.is_zero.cov.using.lt.apply(Eq[0], i)
