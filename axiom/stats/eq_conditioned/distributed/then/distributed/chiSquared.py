...
def prove(Eq):
    ...
    Eq << stats.distributed.then.distributed.chiSquared.apply(Eq[-1])
    ...
    Eq << stats.eq_conditioned.then.eq.conditioned.sum.square.apply(Eq[0], i=i, y=Y.var)

    Eq << Eq[-1].subs(Eq.Y_def.reversed)

    Eq << stats.eq_conditioned.then.eq.mul.prob.apply(Eq[-1])

    Eq << Eq[1].subs(i, k)

    Eq << stats.distributed.then.eq.prob.apply(Eq[-1])
    ...
    Eq << stats.distributed.then.eq.prob.apply(Eq[2], Y.var)

    Eq << Eq[-1].subs(Eq.Y_def.reversed)

    Eq << algebra.eq.cond.of.et.subs.apply(Eq[-1], Eq[-3])
    ...
    Eq << algebra.cond.infer.then.cond.induct.apply(Eq.initial, Eq[-1], n=k, start=1)
