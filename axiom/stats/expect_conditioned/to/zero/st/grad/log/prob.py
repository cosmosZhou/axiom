...
@apply
def apply(self):
    grad, (x_class, θ) = self.of(Expectation)
    grad, given = grad.of(Conditioned)
    ((((x, S[x.surrogate]), S[given]), [S[x_class], S[θ]]), (S[θ], S[1])) = grad.of(Derivative[Log[Probability[Conditioned[Equal]]]])
    assert x_class.index_contains(x)
