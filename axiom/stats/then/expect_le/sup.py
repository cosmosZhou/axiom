...
def prove(Eq):
    ...
    Eq << algebra.then.all.le_sup.apply(Eq[1].rhs)

    Eq << stats.then.ge_zero.prob.apply(Eq[-2].find(Probability))

    Eq << algebra.ge_zero.le.then.le.mul.apply(Eq[-1], Eq[-2])

    Eq << calculus.le.then.le.integral.apply(Eq[-1], [x.var])
