...
def prove(Eq):
    ...
    Eq.eq = Eq[-1].this.rhs.apply(calculus.integral.to.add.split, 0)

    Eq << Eq.eq.find(Integral).this.apply(calculus.integral.to.add.split, a)

    Eq << algebra.eq.then.ge.relax.apply(Eq[-1], Eq[-1].find(Integral[2]))
    ...
    Eq << algebra.ge.then.ge.relax.apply(Eq[-1], Eq[-1].rhs.find(Integral))
    ...
    Eq << Eq.eq.find(Integral[2]).this.apply(calculus.integral.to.add.split, -a)

    Eq << algebra.eq.then.ge.relax.apply(Eq[-1], Eq[-1].rhs.find(Integral))
    ...
    Eq << algebra.ge.then.ge.relax.apply(Eq[-1], Eq[-1].find(Mul[Integral]))
    ...
    Eq << algebra.eq.ge.then.ge.subs.apply(Eq.eq, Eq[-1])
    ...
    Eq << Eq[-1].this.rhs.apply(stats.prob.ou.to.add)
