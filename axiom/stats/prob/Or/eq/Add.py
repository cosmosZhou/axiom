...
def sigmar(conds, k):
    ...
    if k == 1:
        return sum(Probability(cond) for cond in conds)

    sigmar = 0
    for indices in generate_combination(len(conds), k):
        sigmar += Probability(reduce(lambda x, y: x & y, (conds[i] for i in indices)))
    return sigmar


@apply
def apply(self):
    conds = self.of(Probability[Or])
    sgm = 0
    coeff = 1
