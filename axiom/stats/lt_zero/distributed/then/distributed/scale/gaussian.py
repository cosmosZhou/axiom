...
def prove(Eq):
    ...
    Eq << algebra.lt_zero.then.eq.abs.apply(Eq[0])

    Eq << Eq[-2].this.subs(Eq[-1])

    Eq.eq_prob, *Eq[-2:] = algebra.et.of.et.apply(Eq[-1], None)

    Eq << algebra.lt_zero.then.ne_zero.apply(Eq[0])

    Eq << algebra.ne_zero.then.ne_zero.abs.apply(Eq[-2])
    ...
    Eq << algebra.lt_zero.then.iff.apply(Eq[0], cond=Eq[-1].find(LessEqual))
