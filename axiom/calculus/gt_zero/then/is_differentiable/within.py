...
def prove(Eq):
    ...
    Eq << algebra.cond.then.all.apply(Eq[0], x)

    Eq << calculus.all_gt_zero.then.is_differentiable.apply(Eq[-1])

    Eq << algebra.all.then.all.limits.restrict.apply(Eq[-1], domain=Interval(x0, x1))
