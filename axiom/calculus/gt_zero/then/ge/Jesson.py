...
def prove(Eq):
    ...
    x0 = fx0.arg
    x1 = fx1.arg
    Eq << algebra.cond.of.et.infer.split.apply(Eq[-1], cond=x0 <= x1)

    Eq << algebra.cond.then.infer.apply(Eq[0], cond=x0 <= x1)

    Eq << algebra.infer_et.then.infer.et.apply(Eq[-1])

    Eq << Eq[-1].this.rhs.apply(calculus.le.gt_zero.then.ge.Jesson, w=w)

    Eq << algebra.cond.then.infer.apply(Eq[0], cond=x0 > x1)

    Eq << algebra.infer_et.then.infer.et.apply(Eq[-1])

    Eq << Eq[-1].this.rhs.args[0].apply(algebra.gt.then.le.relax)

    Eq << Eq[-1].this.rhs.apply(calculus.le.gt_zero.then.ge.Jesson, w=1-w)
