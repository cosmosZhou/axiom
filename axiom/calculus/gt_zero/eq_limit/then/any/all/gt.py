...
def prove(Eq):
    ...
    delta = Eq[-1].variable
    Eq << calculus.eq_limit.then.any.all.limit_definition.apply(Eq[1], epsilon, delta)

    Eq << algebra.cond.then.ou.subs.apply(Eq[-1], epsilon, A / 2)

    Eq << algebra.cond.ou.then.cond.apply(Eq[0] / 2, Eq[-1])

    Eq << Eq[-1].this.expr.expr.apply(algebra.lt.then.et.split.abs)

    Eq << Eq[-1].this.expr.expr.apply(algebra.et.then.cond, index=1)
