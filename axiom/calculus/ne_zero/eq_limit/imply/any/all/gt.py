...
def prove(Eq):
    ...
    Eq << algebra.infer_et.imply.infer.et.apply(Eq.gt)

    Eq << Eq[-1].this.rhs.apply(calculus.gt_zero.eq_limit.imply.any.all.gt)
    ...
    Eq << Eq[-1].this.rhs.apply(calculus.lt_zero.eq_limit.imply.any.all.lt)
    ...
    Eq << Eq[-1].this.find(Expr < 0).apply(algebra.lt_zero.imply.eq.abs)
