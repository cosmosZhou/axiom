...
def prove(Eq):
    ...
    Eq << Eq[-1].this.expr.apply(algebra.all.all.imply.all.et, simplify=None)

    # then the sequence x[a0] >= x[a1] >= x[a2] >= x[a3] > ... > x[a[j]] is a subsequence with the upper bound M
    Eq << Eq[-1].this.find(All).apply(algebra.all.imply.ou.subs, m, a[i + 1], ret=0, simplify=None)

    Eq << Eq[-1].this.find(Or).apply(algebra.ou.imply.all)

    Eq << Eq[-1].this.find(All[~And]).apply(algebra.et.imply.cond, simplify=0)

    Eq << Eq[-1].this.find(Less).apply(algebra.lt.imply.le.strengthen, simplify=None)

    Eq << Eq[-1].this.find(Or).apply(algebra.ou.imply.et, simplify=None)

    Eq << Eq[-1].this.find(And).apply(algebra.all.all.imply.all.et, simplify=None)

    Eq << Eq[-1].this.find(All).apply(algebra.all.imply.all.subs.inner, m, a[i] + 1, simplify=None)

    Eq << Eq[-1].this.find(All).apply(algebra.all_et.imply.et.all, simplify=None)

    Eq << Eq[-1].this.find(All).limits_subs(i, n, simplify=None)

    Eq << algebra.any_et.imply.any.limits_absorb.apply(Eq[-1])

    Eq << Eq[-1].this.find(Less).apply(algebra.lt.imply.gt.reverse, simplify=None)
    ...
    Eq << Eq[-1].this.expr.apply(algebra.all.imply.ou.subs, n, a[i], simplify=None)
    return
    ...
    Eq << Eq[-1].this.expr.expr.apply(algebra.ou.imply.all, wrt=a[i], simplify=None)
