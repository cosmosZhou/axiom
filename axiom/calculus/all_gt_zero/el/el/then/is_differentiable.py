...
def prove(Eq):
    ...
    Eq << calculus.all_gt_zero.then.is_differentiable.apply(Eq[0])

    x0_ = Symbol('x0', domain=domain)
    x1_ = Symbol('x1', domain=domain)
    Eq << algebra.all.then.all.limits.restrict.apply(Eq[-1], domain=Interval(x0_, x1_))

    Eq << Eq[-1].subs(x1_, x1)

    Eq << algebra.cond.ou.then.cond.apply(Eq[2], Eq[-1])
