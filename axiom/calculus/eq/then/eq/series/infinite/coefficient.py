...
def prove(Eq):
    ...
    Eq << algebra.eq.then.eq.sum.apply(Eq[-1], (k,))
    ...
    Eq << calculus.eq.then.eq.limit.apply(Eq[-1], (r, 0))
    ...
    Eq << algebra.cond.infer.then.cond.induct.second.apply(Eq.initial, Eq[-1], n=m + 1, k=k, hypothesis=True)
