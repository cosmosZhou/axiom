...
def prove(Eq):
    ...
    Eq << algebra.imply.all.maxima_ge.apply(Maxima[n:N + 1](abs(x[n])))
    ...
    Eq << Eq[-1].this.expr.apply(algebra.eq_max.imply.et.ge, simplify=None)

    Eq << Eq[-1].this.expr.args[0].apply(algebra.le_zero.imply.is_zero, simplify=None)

    Eq << Eq[-1].this.find(Equal[0]).apply(algebra.eq_max.imply.et.ge, simplify=None)

    Eq << Eq[-1].this.expr.apply(algebra.et.imply.et.delete, index=-1)

    Eq << Eq[-1].this.args[0].apply(algebra.abs_le_zero.imply.is_zero)

    Eq << Eq[-1].this.args[1].apply(algebra.abs_le_zero.imply.is_zero)
