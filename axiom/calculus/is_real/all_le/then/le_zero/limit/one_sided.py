...
def prove(Eq):
    ...
    Eq << Eq[-1].this.find(All).apply(algebra.all.then.all.limits.restrict, domain=Interval.open(x0, x0 + Min(δ_quote, delta)))

    Eq << algebra.all.then.all.limits.restrict.apply(Eq[1], domain=Interval.open(x0, x0 + Min(δ_quote, delta)))
