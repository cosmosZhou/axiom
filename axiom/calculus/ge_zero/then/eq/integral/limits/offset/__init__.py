...
def prove(Eq):
    ...
    Eq << algebra.cond.then.et.infer.split.apply(Eq[0], cond=a > b)

    Eq << Eq[-2].this.rhs.apply(calculus.ge_zero.then.eq.integral.limits.offset.interval, Eq[-3].find(Integral), d)

    Eq << Eq[-1].this.rhs.apply(calculus.ge_zero.then.eq.integral.limits.offset.interval, Eq[-4].find(Integral), d)
