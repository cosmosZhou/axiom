...
def apply(is_continuous, is_differentiable, equal):
    from axiom.calculus.lt.is_continuous.is_differentiable.eq.imply.any.eq.Rolle import of_continuous, of_differentiable
...
def prove(Eq):
    ...
    Eq << calculus.lt.is_continuous.is_differentiable.eq.imply.any.eq.Rolle.apply(Eq[-1], Eq[0], Eq[1], Eq[2])
