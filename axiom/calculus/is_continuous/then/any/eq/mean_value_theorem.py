...
def prove(Eq):
    ...
    Eq << calculus.all_eq.then.all.any.eq.intermediate_value_theorem.apply(Eq[0])

    Eq.intermediate_value = Eq[-1].subs(Eq.max.reversed).subs(Eq.min.reversed)

    Eq << algebra.then.all.minima_le.apply(m.definition)

    Eq << calculus.all_le.then.le.integral.apply(Eq[-1])

    Eq << Eq[-1].subs(Eq.min.reversed) / (b - a)

    Eq << algebra.then.all.maxima_ge.apply(M.definition)

    Eq << calculus.all_ge.then.ge.integral.apply(Eq[-1])

    Eq << Eq[-1].subs(Eq.max.reversed) / (b - a)

    Eq <<= sets.le.ge.then.el.interval.apply(Eq[-4], Eq[-1])

    Eq << algebra.all.then.ou.subs.apply(Eq.intermediate_value, Eq.intermediate_value.rhs, Eq[-1].lhs)

    Eq << algebra.ou.then.any.ou.apply(Eq[-1], simplify=None)

    Eq << algebra.cond.any.then.any.et.apply(Eq[-1], Eq[-3], simplify=None)

    Eq << algebra.any_et.then.et.any.apply(Eq[-1])
