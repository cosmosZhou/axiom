...
def prove(Eq):
    ...
    Eq << Calculus.Gt_0.to.is_differentiable.within.apply(Eq[0], x0, x1, x=Eq[1].variable)

    Eq << Calculus.Gt_0.to.is_differentiable.within.apply(Eq[0], x1, x0, x=Eq[1].variable)

    Eq <<= Eq[-1] & Eq[-2]

    Eq << Eq[-1].this.find(Union).apply(Sets.Union.eq.Interval)
