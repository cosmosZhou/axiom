...
def prove(Eq):
    ...
    Eq << algebra.gt_zero.then.gt_zero.pow.apply(Eq.gt_zero, n)

    Eq << algebra.gt_zero.then.gt_zero.abs.apply(Eq[-1])

    Eq << algebra.gt_zero.le.then.le.mul.apply(Eq[-1], Eq[-3])

    Eq <<  calculus.le.then.le.limit.apply(Eq[-1], (n, oo))

    Eq.le = Eq[-1].this.rhs.apply(calculus.limit.to.mul)

    Eq << calculus.el_interval.then.is_zero.limit.geometric_series.positive.apply(Eq[0], n)

    Eq << sets.is_zero.then.is_real.apply(Eq[-1])

    Eq << calculus.is_limited.then.eq.abs.limit.apply(Eq[-1])
    ...
    Eq << algebra.ge.le.then.eq.apply(Eq[-1], Eq[-3])

    Eq << calculus.is_zero.then.is_zero.limit.st.abs.apply(Eq[-1])
