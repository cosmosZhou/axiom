...
def prove(Eq):
    ...
    Eq.gt_zero = sets.el_interval.then.lt.apply(Eq[-1]).this.apply(algebra.lt.then.gt_zero)

    Eq << algebra.cond.infer.then.infer.et.lhs.apply(Eq[-1], Eq[-2])

    Eq << Eq[-1].this.lhs.apply(sets.el.el.of.et.el)
    ...
    Eq << Eq[-1].this.rhs.apply(sets.is_positive.le.then.le.div)

    Eq << algebra.infer.then.all.apply(Eq[-1])

    Eq << sets.gt_zero.then.is_positive.apply(Eq.gt_zero, simplify=None)
    ...
    Eq << Eq[-1].this.expr.apply(calculus.is_extended_real.all_le.then.le_zero.limit.one_sided)
