...
def prove(Eq):
    ...
    Eq.lt_zero = sets.el_interval.imply.gt.apply(Eq[-1]).this.apply(algebra.gt.imply.lt_zero)

    Eq << algebra.cond.infer.imply.infer.et.lhs.apply(Eq[-1], Eq[-2])

    Eq << Eq[-1].this.lhs.apply(sets.el.el.given.et.el, stop=True)
    ...
    Eq << Eq[-1].this.rhs.apply(sets.is_negative.le.imply.ge.div)
