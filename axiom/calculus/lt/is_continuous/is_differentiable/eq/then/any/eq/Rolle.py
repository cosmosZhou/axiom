...
def prove(Eq):
    ...
    Eq << Eq[2].this.expr.apply(sets.el.then.eq.definition)

    Eq.all_et = Eq[-1].this.expr.apply(calculus.eq_grad.then.et.eq_grad)

    Eq << algebra.all_et.then.et.all.apply(Eq.all_et)

    y = Eq[-1].expr.rhs
    Eq << Element(y, Interval(-oo, oo, left_open=False, right_open=False), plausible=True)

    Eq <<= algebra.cond.all.then.all.et.apply(Eq[-1], Eq[-3]), algebra.cond.all.then.all.et.apply(Eq[-1], Eq[-2])

    Eq <<= Eq[-2].this.expr.apply(algebra.eq.cond.then.cond.subs, reverse=True, simplify=None), Eq[-1].this.expr.apply(algebra.eq.cond.then.cond.subs, reverse=True, simplify=None)

    Eq << calculus.lt.is_continuous.is_extended_real.is_extended_real.eq.then.any.le.Rolle.apply(*Eq[:2], *Eq[-2:], Eq[3])

    Eq << Eq.all_et.this.expr.apply(algebra.eq.eq.then.eq.mul)

    Eq << algebra.all.any.then.any.et.apply(*Eq[-2:])

    Eq << Eq[-1].this.expr.apply(algebra.eq.cond.then.cond.subs)

    Eq << Eq[-1].this.expr.apply(algebra.le_zero.then.is_zero)

    Eq << Eq[-1].this.expr.apply(algebra.square_is_zero.then.is_zero)
