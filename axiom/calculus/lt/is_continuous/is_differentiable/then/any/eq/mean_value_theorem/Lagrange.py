...
def prove(Eq):
    ...
    Eq << Eq[-1].this.expr.apply(algebra.eq.cond.of.et.subs, swap=True)
    ...
    Eq << Eq[-1].this.expr.apply(sets.el.of.el.add, f(b) - f(a))

    Eq << sets.all.then.all.et.apply(Eq[2], simplify=None)

    Eq << Eq[-1].this.find(Unequal).apply(sets.interval_ne_empty.then.gt_zero, simplify=None)

    Eq << Eq[-1].this.expr.apply(sets.gt_zero.el.then.el.mul)

    Eq << calculus.lt.is_continuous.is_differentiable.eq.then.any.eq.Rolle.apply(Eq[0], Eq.is_continuous, Eq.is_differentiable, Eq.equal)
