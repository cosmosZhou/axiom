...
def apply(lt, is_continuous, is_differentiable):
    from axiom.calculus.lt.is_continuous.is_differentiable.eq.imply.any.eq.Rolle import of_differentiable, of_continuous
    a, b = lt.of(Less)
...
def prove(Eq):
    ...
    Eq << sets.all.imply.all.et.apply(Eq[2], simplify=None)
    ...
    Eq << calculus.lt.is_continuous.is_differentiable.eq.imply.any.eq.Rolle.apply(Eq[0], Eq.is_continuous, Eq.is_differentiable, Eq.equal)
