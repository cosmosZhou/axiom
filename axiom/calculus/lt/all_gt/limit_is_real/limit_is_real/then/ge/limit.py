...
def prove(Eq):
    ...
    Eq <<= calculus.then.is_continuous.interval.apply(f(x), (x, domain)), calculus.then.is_continuous.interval.apply(g(x), (x, domain))

    Eq <<= Eq[-1] & Eq[-2]

    Eq << Eq[-1].this.expr.apply(calculus.eq_limit.eq_limit.then.eq_limit.sub)

    xi = Eq[-1].variable
    Eq <<= Eq[1].this.expr.apply(algebra.gt.then.gt_zero)

    Eq <<= Eq[-1].limits_subs(x, xi)

    Eq <<= Eq[-1] & Eq[-2]

    Eq <<= Eq[-1].this.expr.apply(algebra.eq.gt.then.gt.subs)
    ...
    Eq <<= Eq[-1].this.lhs.apply(sets.el.of.el.neg)

    Eq <<= Eq[-1].this.lhs.apply(sets.el.of.el.add, b)

    Eq <<= Eq[-1].this.lhs.apply(sets.el_interval.of.et)

    Eq << algebra.infer.then.all.apply(Eq[-1])

    Eq << algebra.lt.then.gt_zero.apply(Eq[0])

    Eq << calculus.gt_zero.all_gt_zero.then.limit_ge_zero.st.limit.apply(Eq[-1], Eq[-2])

    Eq << calculus.is_limited.is_limited.then.eq.sub.algebraic_limit_theorem.apply(Eq[2], Eq[3])

    Eq << Eq[-2].subs(Eq[-1])

    Eq << sets.ge.el.then.ge.add.apply(Eq[-1], Eq[3])
