...
def prove(Eq):
    ...
    Eq << Eq[-2].this.lhs.apply(calculus.is_real.all_ge.imply.ge_zero.limit.one_sided)
    ...
    Eq << calculus.is_infinite.imply.any.all.lt_zero.apply(Eq[-1], delta=δ_quote)

    Eq << Eq[-1].this.expr.apply(algebra.all.imply.all.limits.restrict, domain=Interval.open(x0 - Min(delta, δ_quote), x0))

    Eq << algebra.all.imply.all.limits.restrict.apply(Eq[1], domain=Interval.open(x0 - Min(delta, δ_quote), x0))
    ...
    Eq << Eq[-1].this.lhs.apply(algebra.eq.imply.ge_zero)
