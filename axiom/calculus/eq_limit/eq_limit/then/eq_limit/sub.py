...
def prove(Eq):
    ...
    Eq << sets.eq.then.is_real.apply(Eq[0], simplify=None)

    Eq << sets.eq.then.is_real.apply(Eq[1], simplify=None)

    Eq << calculus.is_limited.is_limited.then.eq.sub.algebraic_limit_theorem.apply(Eq[-2], Eq[-1])
