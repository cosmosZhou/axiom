...
def prove(Eq):
    ...
    Eq.all = algebra.cond.then.all.apply(Eq[1], Eq[1].find(Abs < ~Symbol))

    ε = Eq.all.variable
    Eq << All(Eq.all.expr, (ε, Interval(1, oo)), plausible=True)

    Eq << algebra.all.of.et.all.split.apply(Eq[-1], cond=ε > 1)

    Eq << algebra.all.then.cond.subs.apply(Eq.all, ε, S.One / 2)

    Eq << Eq[-1].this.find(Less).apply(algebra.lt.then.lt.relax, 1)

    χ = Symbol(domain=Interval.open(1, oo))
    Eq << algebra.all.then.cond.subs.apply(Eq.all, ε, 1 / χ)

    Eq << Eq[-1].this.find(Less).apply(algebra.lt.then.lt.relax, χ)

    Eq << algebra.cond.then.all.apply(Eq[-1], χ)
