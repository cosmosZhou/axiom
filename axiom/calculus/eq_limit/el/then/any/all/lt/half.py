...
def prove(Eq):
    ...
    Eq << sets.el.then.any.eq.apply(Eq[1], var='a')

    Eq << algebra.cond.any.then.any.et.apply(Eq[0], Eq[-1], simplify=None)

    Eq << Eq[-1].this.expr.apply(algebra.eq.cond.then.cond.subs, simplify=None, ret=0)

    Eq << algebra.any.then.any.et.limits.single_variable.apply(Eq[-1], simplify=None)

    Eq << Eq[-1].this.find(Element).apply(sets.el.then.ne_zero)

    Eq << algebra.any_et.then.any.limits_absorb.apply(Eq[-1], index=0, simplify=None)

    Eq << Eq[-1].this.expr.apply(calculus.ne_zero.eq_limit.then.any.all.gt)
