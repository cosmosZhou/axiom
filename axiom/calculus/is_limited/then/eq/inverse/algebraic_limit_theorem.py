...
def prove(Eq):
    ...
    Eq << sets.el.then.ne_zero.apply(Eq.is_nonzero_real)

    Eq << algebra.ne_zero.eq.then.eq.inverse.apply(Eq[-1], Eq.is_limited)
    ...
    Eq << calculus.eq_limit.el.then.any.all.lt.half.apply(Eq.is_limited, Eq.is_nonzero_real, delta=δ_1)

    Eq.A_is_positive = sets.is_nonzero.then.gt_zero.abs.apply(Eq.is_nonzero_real)

    Eq << algebra.cond.any_all.then.any.all.et.apply(Eq.A_is_positive / 2, Eq[-1])

    Eq << Eq[-1].this.expr.expr.apply(algebra.gt_zero.gt.then.lt.inverse)

    Eq << algebra.gt_zero.then.gt_zero.div.apply(Eq.A_is_positive)

    Eq << algebra.cond.any_all.then.any.all.et.apply(Eq[-1], Eq[-2])

    Eq << Eq[-1].this.expr.expr.apply(algebra.gt_zero.lt.then.lt.mul)


    Eq << algebra.any_all.any_all.then.any.all.et.limits_intersect.apply(Eq[2], Eq[-1])
    Eq << Eq[-1].this.expr.expr.apply(algebra.lt.lt.then.lt.mul)
    ...
    Eq << algebra.cond.then.ou.subs.apply(Eq[-1], ε_0, abs(A) ** 2 / 2 * ε)
    Eq << algebra.gt_zero.then.gt_zero.square.apply(Eq.A_is_positive) * ε / 2
