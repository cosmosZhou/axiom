...
def prove(Eq):
    ...
    M = Eq[-1].variables[1]
    Eq << calculus.is_limited.then.any.all.limit_definition.symbol_subs.apply(Eq[0], var='A')
    ...
    Eq << Eq[-1].this.expr.expr.apply(algebra.lt.gt.then.lt.abs.max)

    Eq << algebra.cond.then.any.subs.apply(Eq[-1], Eq[-1].expr.expr.rhs, M)
