...
def prove(Eq):
    ...
    Eq << calculus.is_limited.then.any.all.limit_definition.symbol_subs.apply(Eq[0], ε_0, N0, var='A')

    Eq << Eq[-1].subs(ε_0, ε / 2)

    ε_1 = Symbol(real=True, positive=True)
    N1 = Symbol('N_1', real=True, positive=True)
    Eq << calculus.is_limited.then.any.all.limit_definition.symbol_subs.apply(Eq[1], ε_1, N1, var='B')

    Eq << Eq[-1].subs(ε_1, ε / 2)

    Eq << algebra.any_all.any_all.then.any.all.et.limits_intersect.apply(Eq[-1], Eq[-3])

    Eq << Eq[-1].this.expr.expr.apply(algebra.lt.lt.then.lt.abs.sub)

    Eq << algebra.any.then.any.subs.apply(Eq[-1], Max(N0, N1), N)
