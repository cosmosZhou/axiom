...
def prove(Eq):
    ...
    Eq.inverse = calculus.is_limited.then.eq.inverse.algebraic_limit_theorem.apply(Eq[1])

    Eq << sets.el.then.is_real.inverse.apply(Eq[1])

    Eq << algebra.eq.cond.then.cond.subs.apply(Eq.inverse, Eq[-1], reverse=True)

    Eq << calculus.is_limited.is_limited.then.eq.mul.algebraic_limit_theorem.apply(Eq[0], Eq[-1])
