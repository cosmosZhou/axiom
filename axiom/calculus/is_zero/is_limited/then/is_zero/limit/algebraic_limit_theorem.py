...
def prove(Eq):
    ...
    Eq << calculus.eq_limit.then.any.all.limit_definition.apply(Eq[0], ε, δ_0)

    δ_1 = Symbol(real=True, positive=True)
    Eq << calculus.is_limited.then.any.all.le.boundedness.apply(Eq[1], delta=δ_1, var='B')

    B = Eq[-1].variables[1]
    assert B > 0
    Eq << Eq[-2].subs(ε, ε / B)

    Eq << algebra.any_all.any_all.then.any.all.et.limits_intersect.apply(Eq[-1], Eq[-2])

    Eq << Eq[-1].this.expr.expr.apply(algebra.lt.lt.then.lt.abs.mul)
