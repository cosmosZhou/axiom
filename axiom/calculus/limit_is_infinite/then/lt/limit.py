...
def prove(Eq):
    ...
    Eq << calculus.eq_limit.then.any.all.limit_definition.apply(Eq[0], 'chi')

    Eq << Eq[-1].this.expr.apply(algebra.all.then.all.et)

    Eq << Eq[-1].this.find(Element).apply(sets.el_interval.then.lt)

    Eq << Eq[-1].this.expr.expr.apply(algebra.lt_zero.gt.then.lt.mul)
