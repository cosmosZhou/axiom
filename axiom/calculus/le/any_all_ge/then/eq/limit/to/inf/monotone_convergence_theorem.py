from util import *


@apply
def apply(le, any_all_ge):

    ((an, M), (n, S[0], S[oo])), (S[M],) = any_all_ge.of(Any[All[GreaterEqual]])
    S[an._subs(n, n + 1)], S[an] = le.of(LessEqual)
    return Equal(Limit[n:oo](an), Inf[n:oo](an))


@prove
def prove(Eq):
    from axiom import algebra, sets, calculus

    a = Symbol(real=True, shape=(oo,), given=True)
    n = Symbol(integer=True)
    M = Symbol(real=True)
    Eq << apply(a[n + 1] <= a[n], Exists[M](ForAll[n:oo](a[n] >= M)))

    N = Symbol(integer=True, nonnegative=True)
    epsilon = Symbol(real=True, positive=True)
    Eq.any_lt = Exists[N](a[N] < Eq[2].find(Inf) + epsilon, plausible=True)

    Eq << ~Eq.any_lt

    Eq << Eq[-1].this.expr.apply(algebra.all_ge.then.inf_ge)

    Eq.any_ge = Eq[-1].this.find(Inf).limits_subs(N, n)

    Eq << Eq[1].this.expr.apply(algebra.all_ge.then.inf_ge)

    Eq << Eq[-1].this.expr.apply(algebra.ge.then.gt.relax, lower=-oo)

    Eq.le_inf = algebra.then.all.inf_le.apply(Eq[-1].lhs)

    Eq << algebra.le.then.lt.relax.apply(Eq.le_inf, upper=oo)

    Eq.inf_is_real = sets.gt.lt.then.el.interval.apply(Eq[-2], Eq[-1], simplify=None)

    Eq << algebra.cond.any.then.any.et.apply(Eq.inf_is_real, Eq.any_ge, simplify=None)

    Eq << Eq[-1].this.expr.apply(sets.ge.el.then.ge.sub)

    Eq << Eq.any_lt.this.expr - epsilon

    Eq << Eq[-1].this.expr.reversed

    Eq << -Eq[-1].this.expr

    Eq.any_gt = Eq[-1].this.expr + a[N]

    Eq << algebra.le.then.all.le.monotone.apply(Eq[0], n, N)

    Eq << algebra.all.then.all.limits.restrict.apply(Eq[-1], domain=Range(N + 1, oo))

    Eq << algebra.cond.all.then.all.et.apply(Eq.inf_is_real, Eq[-1], simplify=None)

    Eq << Eq[-1].this.expr.apply(sets.le.el.then.le.sub)

    Eq << algebra.all.any.then.any.all.et.apply(Eq[-1], Eq.any_gt)

    Eq << Eq[-1].this.expr.expr.apply(algebra.lt.le.then.lt.trans)

    Eq << algebra.le.then.eq.abs.apply(Eq.le_inf)

    Eq << Eq[-2].subs(Eq[-1].reversed)

    Eq << Eq[-1].this.find(Abs).apply(algebra.abs.neg)

    Eq << calculus.any_all.then.eq.limit_definition.apply(Eq[-1])


if __name__ == '__main__':
    run()
# created on 2020-06-24
