...
def prove(Eq):
    ...
    Eq << Eq[-1].this.find(Sum[Mul[Symbol]]).apply(algebra.sum.to.sub.unshift)

    Eq << Eq[-1].this.find(Sum[Mul[Symbol]]).apply(discrete.sum.binom.to.mul.Newton)
    ...
    Eq << Eq[-1].this.find(Sum).apply(discrete.sum.binom.to.mul.Newton.trois)
