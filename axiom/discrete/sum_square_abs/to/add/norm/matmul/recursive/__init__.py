...
def prove(Eq):
    ...
    Eq << Eq[-1].this.rhs.apply(discrete.square_norm.to.add.matmul)

    Eq << Eq[-1].this.rhs.find(Norm[Indexed * Conjugate]).apply(algebra.norm.to.mul.norm)

    Eq << Eq[-1].this.rhs.args[1:3].apply(discrete.add_square.to.sub.matmul)
    ...
    Eq << Eq[-1].rhs.find(Add[MatMul]).this.apply(discrete.add.to.matmul.block.one.push)
