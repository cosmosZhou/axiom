...
def prove(Eq):
    ...
    w, i, j = Eq[0].lhs.args
    Eq << discrete.then.all_el.permutation.apply(n, w, left=False).subs(i, m).subs(j, d[m])

    Eq << algebra.all.then.ou.subs.apply(Eq[-1], Eq[-1].variable, Eq[2].expr.lhs)

    Eq << Eq[-1].this.args[0].lhs.simplify()

    Eq << Eq[-1].apply(algebra.cond.then.all.restrict, Eq[-3].limits[0])
    ...
    Eq << algebra.infer.then.cond.induct.apply(Eq[-1], n=m)
