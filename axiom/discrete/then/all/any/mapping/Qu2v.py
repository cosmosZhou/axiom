...
def prove(Eq):
    ...
    Eq << algebra.all_et.then.all.apply(Eq[-1], 1)

    Eq.x_j_equality = Eq[-1].this.expr.apply(discrete.eq.then.any.eq.index, v)
    ...
    Eq << Eq[1][k].apply(sets.eq.then.eq.cup.finiteset, (k, 0, n + 1), simplify=False)
    ...
    Eq << algebra.all_any_eq.cond.then.all.any.subs.apply(Eq.x_j_equality, Eq[-1])
