...
def prove(Eq):
    ...
    Eq << Eq[-1].this.find(Element).apply(sets.el.of.subset.cup.finiteset)

    Eq << Eq[-1].this.args[1:].apply(algebra.eq.cond.of.et.subs)

    Eq << sets.any_el.then.ne_empty.apply(Eq.plausible)

    Eq << algebra.cond.then.all.apply(Eq[-1], _t)
