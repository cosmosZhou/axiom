...
def prove(Eq):
    ...
    Eq << algebra.then.ge_zero.square.apply(sqrt(x[0]) - sqrt(x[1]))

    Eq << algebra.ge_zero.then.ge.apply(Eq[-1])

    t = Function(real=True, eval=lambda k: (sigma[k](x[:n]) / binomial(n, k)) ** (1 / k))
    k_ = Symbol("k", domain=Range(2, n))
    Eq << t(k_).this.defun()

    Eq << algebra.eq.then.eq.pow.apply(Eq[-1], exp=k_)
    ...
    Eq << algebra.eq.then.eq.pow.apply(Eq[-1], exp=k_ + 1)
    ...
    Eq << algebra.all.then.cond.subs.apply(Eq[0], k, k_)

    Eq.hypothesis_k = Eq[-1].subs(t(k_).this.defun().reversed).subs(t(k_ + 1).this.defun().reversed)

    Eq << algebra.all.then.cond.subs.apply(Eq[0], k, k_ - 1)
