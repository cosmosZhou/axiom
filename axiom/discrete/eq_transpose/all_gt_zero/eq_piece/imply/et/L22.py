...
def apply(eq, infer, eq_piece):
    ...
    return Element(L[2, 0], S.Complexes), Equal(A[2, 0], L[0, 0] * L[2, 0]), \
        ...
        Element(L[2, 2], Interval(0, oo, left_open=True)), Equal(A[2, 2], L[2, 2] ** 2 + L[2, 0] * ~L[2, 0] + L[2, 1] * ~L[2, 1])
...
def prove(Eq):
    ...
    *Eq[-3:], (Eq.L20_is_complex, Eq.A20_def, Eq.L21_is_complex, Eq.A21_def, Eq.L22_is_positive, Eq.A22_def) = apply(Equal(~A.T, A),
    ...
    Eq << Eq[2].subs(i, 2).subs(j, 1).this.find(MatMul).apply(discrete.matmul.to.sum).reversed
    ...
    Eq << Eq[2].subs(i, 2).subs(j, 2).this.find(MatMul).apply(discrete.matmul.to.sum).this.find(Sum).apply(algebra.sum.to.add.doit).reversed
