...
def prove(Eq):
    ...
    Eq << algebra.any_piece.of.et.infer.apply(Eq[-1])

    Eq << Eq[-1].this.rhs.simplify()

    Eq <<= algebra.infer.of.all.apply(Eq[-3]), algebra.infer.of.infer.subs.apply(Eq[-2])

    Eq <<= algebra.all.of.all.limits.domain_defined.apply(Eq[-2]), Eq[-1].this.rhs.simplify()

    Eq <<= Eq[-2].this(i).find(Min).simplify(), algebra.infer.of.cond.apply(Eq[-1])
    ...
    Eq << algebra.cond.any.then.any.et.apply(Eq[0] & Eq[1], Eq[3], simplify=None)

    Eq << Eq[-1].this.expr.apply(discrete.eq_transpose.infer_gt_zero.eq_piece.then.et.Cholesky, simplify=False, ret=0)
