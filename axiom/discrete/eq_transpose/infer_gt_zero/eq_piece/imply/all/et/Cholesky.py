...
def prove(Eq):
    ...
    *Eq[-3:], Eq.hypothesis = apply(Equal(A.T, A),
    ...
    Eq.gt_zero, Eq.Atj_def_and_Ltj_is_real = discrete.eq_transpose.infer_gt_zero.infer_eq.all_et.imply.et.Cholesky.apply(Eq[0], Eq[1], Eq[-1], Eq.hypothesis)

    Eq << algebra.all_et.imply.all.apply(Eq.Atj_def_and_Ltj_is_real, 1)

    Eq << Eq[-1].this.expr.apply(sets.is_real.imply.is_nonnegative.square)
