...
def prove(Eq):
    ...
    s1_quote = Eq[2].lhs
    Eq << sets.then.all.conditionset.apply(s1_quote)

    i = Eq[1].lhs.indices[0]
    x_slice = Eq[-1].limits[0][0]
    x = x_slice.base
    Eq.x_abs_positive_s1 = algebra.all_et.then.all.apply(Eq[-1])

    Eq.x_abs_sum_s1 = algebra.all_et.then.all.apply(Eq[-1], 1)

    Eq.x_union_s1 = algebra.all_et.then.all.apply(Eq[-1], 2)
    ...
    Eq << sets.subset.of.all_el.apply(Eq.x_quote_set_in_s2)

    Eq << Eq[-1].subs(Eq.s2_definition)

    Eq << Eq[-1].this.expr.apply(sets.el.of.el.split.imageset)

    Eq << Eq[-1].this.expr.rhs.definition

    Eq << Eq[-1].this.expr.args[0].simplify()

    Eq << sets.eq.then.eq.cup.apply(Eq[1], (i, 0, k + 1))
    ...
    u = Eq[-1].lhs.arg
    Eq << sets.then.le.cup.apply(u.expr, *u.limits)

    Eq << Eq[-2].this.expr.apply(algebra.eq.le.then.ge.subs, Eq[-1])
    ...
    Eq << Eq[-2].apply(algebra.cond.then.all.restrict, (i, Unequal(i, j)))
    ...
    Eq << algebra.all.all.then.all.et.apply(Eq.x_abs_positive_s1, Eq[-3].reversed)

    Eq.xi_is_positive = Eq[-1].this.expr.apply(algebra.eq.cond.then.cond.trans)

    Eq.xi_all_is_positive = Eq.xi_is_positive.apply(algebra.all.of.all.limits.delete, index=0)

    Eq << Eq.xi_all_is_positive.this.expr.apply(algebra.cond.of.et.all, cond=Equal(i, j))

    Eq << algebra.all_et.of.et.all.apply(Eq[-1])

    Eq << Eq[-1].apply(algebra.all.of.all.ou.limits.delete, simplify=None)

    Eq << Eq[-1].this.find(NotElement).apply(sets.notin_complement.of.ou, simplify=None)

    Eq << Eq[-1].this.find(Greater).apply(algebra.cond.of.ou.domain_defined, wrt=i, simplify=None)

    Eq << Eq.xi_is_positive.apply(algebra.all.then.all.ou.limits.delete, simplify=None)

    Eq << Eq[-1].this.find(NotElement).apply(sets.notin_complement.then.ou, simplify=None)

    Eq <<= Eq.x_quote_union & Eq.SqueezeTheorem & Eq.xi_all_is_positive

    Eq.x_quote_definition = Eq[1].apply(algebra.eq.then.eq.lamda, (i, 0, k + 1), simplify=False)
    ...
    Eq << sets.supset.of.all_el.apply(Eq[-1])
    ...
    Eq << Eq[-2].reversed.this.expr.apply(sets.eq.eq.then.eq.intersect, Eq[-1])

    Eq << sets.then.eq.principle.inclusion_exclusion.basic.apply(*Eq[-1].lhs.args)

    Eq << algebra.any_eq.cond.then.any.subs.apply(Eq[-2], Eq[-1])

    Eq.set_size_inequality = Eq[-1].this.expr.apply(algebra.eq.lt.then.lt.subs, Less(Eq[-1].expr.rhs, Eq[-1].expr.rhs + 1, plausible=True))

    Eq << Eq.x_quote_union.this.expr.lhs.apply(sets.cup.to.union.split, cond={i, j})

    Eq << sets.then.le.union.apply(*Eq[-1].lhs.args)

    Eq << sets.then.le.cup.apply(*Eq[-2].lhs.args[0].args)

    Eq << algebra.le.le.then.le.subs.apply(Eq[-2], Eq[-1])

    Eq << algebra.cond.any.then.any.et.apply(Eq[-1], Eq.set_size_inequality)
