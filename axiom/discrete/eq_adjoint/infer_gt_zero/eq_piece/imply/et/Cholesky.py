...
def prove(Eq):
    ...
    Eq << discrete.eq_adjoint.infer_gt_zero.eq_piece.imply.all.et.Cholesky.apply(*Eq[:3], t)
    ...
    Eq.eq_conj = sets.is_real.imply.eq.conj.apply(Eq[-2])

    Eq <<= algebra.all_et.imply.et.all.apply(Eq[-1]), sets.el.imply.el.relax.apply(Eq[-2], S.Complexes)
    ...
    Eq << Eq[-1].this.rhs.apply(sets.el.imply.el.relax, S.Complexes)
    ...
    Eq << algebra.eq.imply.eq.conj.apply(Eq[0][i, j])
    ...
    Eq << Eq[-1].this.rhs.apply(algebra.eq.imply.eq.conj)
