...
def prove(Eq):
    ...
    *Eq[-3:], Eq.hypothesis = apply(Equal(~A.T, A),
    ...
    Eq.gt_zero, Eq.Atj_def_and_Ltj_is_complex = discrete.eq_adjoint.infer_gt_zero.infer_eq.all_et.then.et.Cholesky.apply(Eq[0], Eq[1], Eq[-1], Eq.hypothesis)

    Eq << algebra.all_et.then.all.apply(Eq.Atj_def_and_Ltj_is_complex, 1)

    Eq << Eq[-1].this.expr.apply(sets.is_complex.then.is_nonnegative.mul.conj)
