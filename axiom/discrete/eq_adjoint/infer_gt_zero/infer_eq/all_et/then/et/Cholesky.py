...
def prove(Eq):
    ...
    Eq << algebra.all_et.then.et.all.apply(Eq[3])

    Eq.Aii_def, Eq.Lii_is_positive = algebra.all_et.then.et.all.apply(Eq[-2])

    Eq.Aij_def, Eq.Lij_is_complex = algebra.all_et.then.et.all.apply(Eq[-1])

    Eq << algebra.all.all.then.all.et.apply(Eq.Lii_is_positive, Eq.Lij_is_complex)

    Eq << Eq[2].subs(i, t).subs(j, i)

    Eq << algebra.infer.then.all.single_variable.apply(Eq[-1], i)

    Eq << algebra.all.all.then.all.et.apply(Eq[-1], Eq[-3])

    Eq << discrete.all_et.then.all.et.Cholesky.apply(Eq[-1])
    ...
    Eq << algebra.all_et.then.all.apply(Eq[5]).limits_subs(j, i)

    Eq << algebra.all_eq.cond.then.all.subs.apply(Eq[-1], Eq[-2])

    Eq << algebra.all_eq.cond.then.all.subs.apply(Eq.Aii_def, Eq[-1])
    ...
    Eq << Eq[-1].this.lhs.find(Sum[~Add, Tuple, Tuple]).apply(algebra.add.to.mul.re)

    Eq << Eq[-1].this.lhs.find(Sum[Re]).apply(algebra.sum.limits.separate)

    Eq << Eq[-1].this.lhs.find(Sum[Re]).apply(algebra.sum.to.re)

    Eq << Eq[-1].this.lhs.find(Conjugate * ~Sum).apply(discrete.sum.to.matmul, simplify=1)
    ...
    Eq << Eq[-1].this.find(Sum).expr.args[1:].apply(algebra.add.to.re)

    Eq << Eq[-1].this.find(Re[~Add]).apply(algebra.add.to.mul)
    ...
    Eq << discrete.all_is_positive.gt_zero.then.gt_zero.Cholesky.apply(Eq.Lii_is_positive, Eq[-1])
