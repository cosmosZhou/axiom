...
def apply(eq, infer, eq_piece):
    from axiom.discrete.eq_adjoint.infer_gt_zero.eq_piece.imply.all.et.Cholesky import extract
...
def prove(Eq):
    ...
    *Eq[-3:], (Eq.L20_is_complex, Eq.A20_def, Eq.L21_is_complex, Eq.A21_def, Eq.L22_is_positive, Eq.A22_def) = apply(Equal(~A.T, A),
    ...
    Eq.L00_is_positive, Eq.A00_def = discrete.eq_adjoint.all_gt_zero.eq_piece.imply.et.L00.apply(*Eq[:3])

    Eq.L10_is_complex, Eq.A10_def, Eq.L11_is_positive, Eq.A11_def = discrete.eq_adjoint.all_gt_zero.eq_piece.imply.et.L11.apply(*Eq[:3])
    ...
    Eq << sets.is_complex.is_complex.imply.is_complex.apply(Eq[-1], Eq.L20_is_complex)
    ...
    Eq << sets.is_nonzero.imply.eq.conj.square_completing.apply(Eq[-1], Eq[-2].lhs, ξ[0], simplify=None)
    ...
    Eq << sets.is_nonzero.imply.eq.conj.square_completing.apply(Eq[-1], Eq[-2].lhs, ξ[1], simplify=None)
    ...
    Eq << sets.is_complex.imply.is_nonnegative.mul.conj.apply(Eq.L20_is_complex)

    Eq << sets.is_complex.imply.is_nonnegative.mul.conj.apply(Eq.L21_is_complex)
    ...
    Eq << sets.is_real.imply.eq.square.abs.apply(Eq.L22_is_positive)
