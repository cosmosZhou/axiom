...
def prove(Eq):
    ...
    Eq.hypothesis = Eq[0].subs(n, m + i).this.rhs.apply(algebra.kroneckerDelta.offset, -i)

    Eq.initial = Eq.hypothesis.subs(i, 0)

    Eq << Eq.initial.this.lhs.apply(discrete.sum_binom.to.kroneckerDelta.zero)
    ...
    Eq << Eq[-1].this.find(Sum[Pow * Binomial]).apply(discrete.sum_binom.to.add)
    ...
    Eq << Eq[-1].this.find(Sum[Binomial]).apply(discrete.sum_binom.to.binom)
    ...
    Eq << Eq[-1].this.rhs.apply(algebra.kroneckerDelta.offset, i)
