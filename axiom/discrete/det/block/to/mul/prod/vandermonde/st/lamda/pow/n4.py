...
def prove(Eq):
    ...
    #reference:
    #http://localhost/axiom/index.php?module=discrete.det_block.to.mul.prod.vandermonde.st.lamda.pow.n3
    ...
    Eq << Eq[-1].this.rhs.expr.apply(discrete.sum.binom.to.mul.stirling, simplify=None)
    ...
    Eq.eq_block = Eq[-1].this.find(Sum).apply(discrete.sum.binom.to.pow.Newton)
    ...
    Eq << Eq[-1].this.rhs.find(Sum).apply(discrete.sum.binom.to.mul.Newton)
    ...
    Eq << Eq[-1].this.rhs.find(Sum).apply(discrete.sum.binom.to.mul.Newton.deux)
    ...
    Eq << Eq[-1].this.rhs.find(Sum).apply(discrete.sum.binom.to.mul.Newton.trois)
    ...
    Eq << Eq[-1].this.rhs.apply(discrete.det.block.to.mul)
