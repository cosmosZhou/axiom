...
def prove(Eq):
    ...
    Eq << Eq[-1].this.rhs.args[0].args[1].apply(discrete.matmul.lamda.to.zeroMatrix.vandermonde.col_transformation)
    ...
    Eq << Eq[-1].this.find(Det[2]).apply(discrete.det.matmul.to.mul.prod.vandermonde.col_transform.st.one)

    Eq << Eq[-1].this.find(Det[MatMul]).apply(discrete.det.matmul.lamda.to.mul.prod.vandermonde)
