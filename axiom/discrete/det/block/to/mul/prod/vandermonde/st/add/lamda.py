...
def prove(Eq):
    ...
    Eq << discrete.det.block.to.mul.prod.vandermonde.st.lamda.pow.apply(Det(BlockMatrix([Lamda[j:n + 1](r ** j), Lamda[j:n + 1, i:n](j ** i)])))
    ...
    Eq << Eq[-1].this.rhs.apply(algebra.add.block.to.block)
    ...
    Eq << ZeroMatrix(n).this.apply(algebra.expr.to.block, 1)

    Eq << Eq[-2].subs(Eq[-1].reversed)

    Eq << Eq[-1].this.lhs.apply(discrete.det.block.to.mul)
