...
def prove(Eq):
    ...
    Eq << discrete.eq.then.eq.swap2.general.apply(Eq[0])

    Eq.permutation = discrete.all_el.then.all_el.swapn.permutation.apply(Eq[-1])
    ...
    Eq << Eq[-2].apply(sets.eq.then.eq.card)
    ...
    Eq << sets.subset.all.then.all.apply(Eq[-1], Eq.permutation)
    ...
    Eq << Eq[-1].this.expr.apply(sets.eq.then.eq.permutation, x)

    Eq.equality_e = Eq[-3] & Eq[-1]

    return
    Eq << sets.then.all.conditionset.apply(F(e)).reversed
