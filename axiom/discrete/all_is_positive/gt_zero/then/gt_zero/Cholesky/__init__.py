...
def prove(Eq):
    ...
    Eq.Lkk_is_positive = algebra.all.then.cond.subs.apply(Eq[0], i, k)

    Eq.eq_square_Lkk = sets.is_real.then.eq.square.abs.apply(Eq.Lkk_is_positive)

    Eq << Eq.gt_zero.subs(Eq[-4], Eq[-1], Eq.eq_square_Lkk)

    # now start the square completing process:
    Eq << sets.is_positive.then.is_positive.square.apply(Eq.Lkk_is_positive)

    Eq << sets.is_nonzero.then.eq.re.square_completing.apply(Eq[-1], Add(*[arg for arg in Eq[-2].lhs.args if arg._has(ξ[k])]), ξ[k], simplify=None)
    ...
    Eq << sets.is_positive.then.ne_zero.apply(Eq.Lkk_is_positive)

    Eq << algebra.et.then.et.apply(Eq[-2])

    Eq << Infer(Eq.hypothesis, Eq.induct, plausible=True)

    Eq << algebra.cond.infer.then.cond.induct.apply(Eq[1], Eq[-1], k, 0)
