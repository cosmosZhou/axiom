...
def prove(Eq):
    ...
    Eq << algebra.cond.of.et.infer.split.apply(Eq[0], cond=n > 0)

    Eq << Eq[2].this.lhs.apply(algebra.le_zero.then.is_zero)

    Eq << algebra.infer.of.infer.subs.apply(Eq[-1])

    Eq << algebra.infer.of.all.apply(Eq[1])

    n_ = Symbol('n', integer=True, positive=True)
    Eq << algebra.all.of.cond.subs.apply(Eq[-1], Eq[-1].variable, n_)

    Eq << Eq[-1].this.lhs.apply(discrete.matProd.to.matmul.pop)
    Eq << Eq[-1].this.rhs.args[1].apply(discrete.matProd.to.matmul.pop)
