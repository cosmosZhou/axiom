...
def prove(Eq):
    ...
    Eq << discrete.eq.imply.eq.matmul.apply(Eq[-1], Eq[-1].lhs.find(BlockMatrix)).reversed
    ...
    Eq <<= discrete.eq_matmul.imply.eq.inverse.apply(Eq[-2]), discrete.eq_matmul.imply.eq.inverse.apply(Eq[-1])
