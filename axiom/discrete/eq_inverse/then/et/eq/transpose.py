...
def prove(Eq):
    ...
    Eq << discrete.eq.then.eq.rmatmul.apply(Eq[0], Eq[0].rhs.find(BlockMatrix))
    ...
    Eq <<= discrete.eq_matmul.then.eq.inverse.apply(Eq[-2], left=True), discrete.eq_matmul.then.eq.inverse.apply(Eq[-1], left=True)
