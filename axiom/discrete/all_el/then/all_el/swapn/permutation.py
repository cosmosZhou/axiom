...
def prove(Eq):
    ...
    Eq << discrete.then.all.any.factorization.apply(n)
    ...
    * _, b_i = Eq[-1].rhs.args[1].expr.args
    ...
    Eq << algebra.all_any_eq.cond.then.all.any.subs.apply(Eq[-2].reversed, Eq[-1])

    Eq << algebra.all.then.all.limits.restrict.apply(Eq[-1], (x[:n], S))

    Eq <<= Eq[-1] & Eq.axiom

    Eq << Eq[-1].this.expr.apply(algebra.cond.any.of.any.et, simplify=None)

    Eq << Eq[-1].this.expr.expr.apply(algebra.eq.cond.of.et.subs)

    Eq << Eq[-1].this.expr.apply(algebra.any_et.of.et, index=-1)

    Eq << algebra.all_et.of.et.all.apply(Eq[-1])

    Eq << discrete.all_el.then.all_el.swapn.matProd.apply(Eq.swap.T, n, b)
