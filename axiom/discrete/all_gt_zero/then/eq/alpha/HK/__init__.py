from util import *

from axiom.discrete.H.to.add.definition import H
from axiom.discrete.K.to.add.definition import K
from axiom.discrete.then.gt_zero.alpha import alpha

@apply
def apply(given):
    (x, j), (S[j], n) = given.of(All[Indexed > 0, Tuple[1, Expr]])
    return Equal(alpha(x[:n]), H(x[:n]) / K(x[:n]))


@prove
def prove(Eq):
    from axiom import algebra, discrete

    x = Symbol(integer=True, shape=(oo,))
    n = Symbol(integer=True, positive=True)
    j = Symbol(integer=True)
    Eq << apply(All[j:1:n](x[j] > 0))

    Eq << algebra.cond.of.et.infer.split.apply(Eq[1], cond=n >= 3)

    Eq.case1, Eq.case2 = algebra.infer.of.et.infer.split.apply(Eq[-1], cond=n < 2)

    Eq << Eq.case1.this.lhs.apply(algebra.lt.to.eq.squeeze)

    Eq << Eq[-1].this.apply(algebra.infer.subs)

    Eq << Eq[-1].this.find(alpha).defun()

    Eq << Eq[-1].this.find(H).defun()

    Eq << Eq[-1].this.find(K).defun()

    Eq << Eq.case2.this.apply(algebra.infer.subs)

    Eq << Eq[-1].this.find(alpha).defun()

    Eq << Eq[-1].this.find(alpha).defun()

    Eq << Eq[-1].this.find(H).defun()

    Eq << Eq[-1].this.find(K).defun()

    Eq << Eq[-1].this.rhs.rhs.expand()

    n_ = Symbol('n', domain=Range(3, oo))
    Eq << discrete.all_gt_zero.then.eq.alpha.HK.induct.apply(Eq[0].subs(n, n_))

    Eq << algebra.cond.then.all.apply(Eq[-1], n_)

    _n = Eq[-1].variable
    Eq << algebra.all.then.infer.apply(Eq[-1])

    Eq << Eq[-1].subs(_n, n)





if __name__ == '__main__':
    run()

from . import offset0
# created on 2020-09-24
from . import induct
# updated on 2023-05-21
