...
def prove(Eq):
    ...
    Eq << Eq[-1].this.lhs.apply(algebra.gt.then.ne_zero)
    ...
    Eq.alpha_recurrence = Eq.induct.lhs.this.apply(discrete.all_gt_zero.then.eq.alpha.recurrence)

    # try to avoid using reciprocal expression, instead, using find(1 / Expr) to ensure logical correctness!
    Eq << algebra.cond.then.cond.subs.apply(Eq.hypothesis, x[:n + 1], BlockMatrix(x[:n], Eq.alpha_recurrence.find(Indexed + 1 / Indexed)))

    Eq << Eq[-1].this.rhs.lhs.apply(discrete.alpha.block)

    Eq << Eq[-1].this.lhs.apply(algebra.all.of.et.all, cond={n})

    Eq << Eq[-1].this.find(All)().expr.simplify()

    Eq << Eq[-1].this.find(Greater).apply(algebra.add_gt_zero.of.et.gt_zero)

    Eq << Eq[-1].this.find(Greater[2]).apply(algebra.gt_zero.of.gt_zero.inverse)

    Eq << Eq[-1].this.lhs.apply(algebra.et.to.all.limits.push)

    Eq <<= Eq.alpha_recurrence & Eq[-1]

    Eq << Eq[-1].this.rhs.apply(algebra.eq.eq.then.eq.trans, reverse=True)
    ...
    Eq << algebra.cond.infer.then.cond.induct.apply(Eq.initial, Eq[-1], n, 2)
