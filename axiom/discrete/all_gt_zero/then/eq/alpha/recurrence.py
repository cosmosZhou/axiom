...
def prove(Eq):
    ...
    Eq << algebra.cond.then.cond.subs.apply(Eq.hypothesis, x[:n + 2], x[1:n + 3])
    ...
    Eq << Eq[-1].this.lhs.apply(discrete.all_gt_zero.then.ne_zero.alpha.offset)

    Eq <<= Eq[-1] & Eq[-2]

    Eq << Eq[-1].this.rhs.apply(algebra.ne_zero.eq.then.eq.inverse)
