...
def prove(Eq):
    ...
    Eq << Eq[-1].this.find(And, All).apply(algebra.all.then.cond.subs, Eq[-1].lhs.variable, n + 1)

    Eq << Eq[-1].this.find(And).apply(algebra.gt_zero.gt_zero.then.gt_zero)

    Eq << Eq.hypothesis.this.lhs.apply(algebra.all.of.all.limits.relax, Range(1, n + 2))

    Eq <<= Eq[-1] & Eq[-2]

    Eq << Eq[-1].this.find(And).apply(algebra.gt_zero.gt_zero.then.gt_zero.add)
