...
def prove(Eq):
    ...
    Eq << discrete.eq.then.et.index.apply(Eq[0], j=j)
    ...
    Eq << discrete.eq.then.et.index.apply(Eq[0], j=i)

    Eq.di_domain, Eq.x_di_eqaulity = Eq[-2].subs(Eq.di_definition.reversed), Eq[-1].subs(Eq.di_definition.reversed)

    Eq << algebra.cond.cond.then.cond.subs.apply(Eq.di_domain, Eq.expand)

    Eq.expand = algebra.cond.cond.then.cond.subs.apply(Eq.dj_domain, Eq[-1])

    Eq << sets.el.el.then.subset.finiteset.apply(Eq.dj_domain, Eq.di_domain, simplify=False)

    Eq.eq_intersection = sets.subset.then.eq.intersect.apply(Eq[-1])
    ...
    Eq.dj_eq = sets.el.then.eq.piece.expr_swap.apply(Eq.dj_domain, Eq.piecewise_equality.lhs.args[2])

    Eq << sets.el.then.eq.piece.expr_swap.apply(Eq.di_domain, Eq.piecewise_equality.lhs.args[-1])

    Eq << sets.el.then.eq.intersect.apply(Eq.dj_domain)
    ...
    Eq << discrete.eq.then.eq.index.delta.indexOf.apply(Eq[0], i, j)
