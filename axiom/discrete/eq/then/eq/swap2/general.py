...
def prove(Eq):
    ...
    Eq << algebra.eq.then.eq.lamda.apply(Eq[-1], (i, 0, n), simplify=False)
    ...
    Eq << algebra.all.then.all.limits.swap.apply(Eq.given)
    ...
    Eq << algebra.all.all.then.all.et.apply(Eq[-2], Eq[-3])

    Eq << discrete.eq.all_el.then.all_el.swap2.apply(Eq[1], Eq[-1])
