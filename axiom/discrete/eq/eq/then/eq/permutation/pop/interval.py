...
def prove(Eq):
    ...
    Eq << sets.eq.then.eq.complement.apply(Eq[-1], {n})
    ...
    Eq << Eq[-1].apply(sets.el_cup.then.any_el)
    ...
    Eq << algebra.any_eq.cond.then.any.subs.apply(Eq[-3].reversed, Eq[-1])
    ...
    Eq << Eq.paradox.this.expr.apply(sets.eq.then.eq.card)

    Eq << Eq[-1].subs(Eq[0])

    Eq << algebra.any_eq.cond.then.any.subs.apply(Eq[-1].reversed, Eq[-3])

    Eq << sets.notin.then.eq.complement.apply(Eq.plausible)
