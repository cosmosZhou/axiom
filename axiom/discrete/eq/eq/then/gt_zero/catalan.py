...
def prove(Eq):
    ...
    Eq.initial = algebra.eq.then.gt.apply(Eq[0], 0)
    ...
    Eq.hypothsis_nk = algebra.cond.then.cond.subs.apply(Eq.hypothsis_k, k, n - k)

    Eq << Eq.hypothsis_nk * Eq.hypothsis_k

    Eq << algebra.gt.then.gt.sum.apply(Eq[-1], (k,))
    ...
    Eq << Eq[-1].this.lhs.apply(algebra.cond.of.all, k)
