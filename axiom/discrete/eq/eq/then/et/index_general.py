...
def prove(Eq):
    ...
    Eq << algebra.eq.eq.then.eq.subs.rhs.apply(Eq[-1], Eq[-2])
    ...
    Eq.s_j_greater_than_1 = sets.ne_empty.then.ge.apply(Eq[-1])

    Eq.distribute = Eq.distribute.subs(Eq.s_j_definition_reversed)

    Eq.ou = sets.then.ou.ne.apply(Eq.s_j_greater_than_1.lhs.arg)
    ...
    Eq << sets.eq.then.all.ne.complement.apply(Eq[-1], exclude=a)

    Eq << Eq[-1].subs(k_, a)

    Eq << algebra.all.then.ou.subs.apply(Eq[-1], Eq[-1].variable, b)

    Eq << algebra.cond.any.then.any.et.apply(Eq[-1], Eq.inequality_ab)

    Eq.distribute_ab = Eq[-1].this.expr.apply(algebra.et.then.ou)

    Eq.j_equality = sets.then.all.conditionset.apply(s_j)

    Eq << Eq.j_equality.limits_subs(k, a)

    Eq << algebra.all.any.then.any.et.apply(Eq.j_equality.reversed, Eq.distribute_ab)

    Eq << Eq[-1].this.expr.args[::2].apply(algebra.eq.cond.then.cond.subs, ret=0)

    Eq << Eq[-1].this.expr.apply(algebra.et.then.et.delete)

    Eq << Eq.j_equality.limits_subs(a, b)

    Eq << algebra.all.any.then.any.et.apply(Eq.j_equality, Eq[-1])
    ...
    Eq << sets.eq.then.el.sum.apply(Eq[-1], var=k)

    Eq.index_domain = Eq[-1].subs(Eq.crossproduct.reversed)

    Eq.ou = algebra.all.then.ou.subs.apply(Eq.j_equality, Eq.j_equality.variable, Eq.index_domain.lhs)
    ...
    Eq << sets.el.subset.then.el.apply(Eq.index_domain, Eq[-1])
