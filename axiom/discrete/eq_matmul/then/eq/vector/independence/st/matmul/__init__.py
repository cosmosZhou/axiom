...
def prove(Eq):
    ...
    Eq << algebra.cond.then.all.apply(Eq[-1], i)
    ...
    Eq << discrete.det.lamda.to.prod.vandermonde.st.linear.apply(Det(Lamda[i:n, k:n]((i + 1) ** k)))
    ...
    Eq << discrete.ne_zero.eq.then.eq.matmul.apply(Eq[-1], Eq.statement)
