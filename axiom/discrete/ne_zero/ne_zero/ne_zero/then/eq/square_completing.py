...
def prove(Eq):
    ...
    Eq << discrete.ne_zero.then.eq.square_completing.apply(Eq[2], Eq[-1].rhs)
    ...
    Eq.eq = algebra.eq.then.eq.transport.apply(Eq[-1], rhs=2)
    ...
    Eq << discrete.ne_zero.ne_zero.ne_zero.then.eq.inverse.apply(*Eq[:3])

    Eq << Eq[-2].subs(Eq[-1])

    Eq << algebra.cond.then.cond.domain_defined.apply(Eq[-1])

    Eq << discrete.ne_zero.then.eq.square_completing.apply(Eq[-1], Eq[-2].rhs)
