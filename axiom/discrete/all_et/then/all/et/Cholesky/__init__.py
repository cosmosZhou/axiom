...
def prove(Eq):
    ...
    Eq[-1], Eq.Lij_is_complex = algebra.all_et.then.et.all.apply(Eq[0])

    Eq << Eq[-1].this.expr.apply(sets.is_positive.eq.then.eq.mul, ret=1)

    Eq.Ati_def, Eq.Lii_is_positive = algebra.all_et.then.et.all.apply(Eq[-1])
    ...
    Eq << algebra.all.of.et.all.split.apply(Eq.induct, cond=Equal(j, k))

    Eq.Lij_conj_is_complex = Eq.Lij_is_complex.this.expr.apply(sets.is_complex.then.is_complex.conj)

    Eq << algebra.all.then.cond.subs.apply(Eq.Lij_conj_is_complex, i, k)

    Eq <<= Eq.hypothesis & Eq[-1]

    Eq << Eq[-1].this.expr.apply(sets.is_complex.is_complex.then.is_complex)

    Eq << sets.all_is_complex.then.is_complex.sum.apply(Eq[-1])
    ...
    Eq.Lkk_is_positive = algebra.all.then.cond.subs.apply(Eq.Lii_is_positive, i, k)

    Eq << sets.is_complex.is_positive.then.is_complex.apply(Eq[-1], Eq.Lkk_is_positive)

    Eq << algebra.all.then.cond.subs.apply(Eq.Ati_def, i, k)

    Eq <<= Eq[-2].subs(Eq[-1].reversed)

    Eq << algebra.et.then.cond.apply(Eq[-1], -1)

    Eq << Infer(Eq.hypothesis, Eq.induct, plausible=True)

    Eq << algebra.infer.then.cond.induct.apply(Eq[-1], k, 0)

    Eq << algebra.cond.infer.then.cond.trans.apply(Eq.hypothesis, Eq[-1])

    Eq << Eq.induct.subs(k, t - 1)

    Eq << Eq.Ati_def.this.find(MatMul).apply(discrete.matmul.to.sum)

    Eq << algebra.all.all.then.all.et.apply(Eq.Lij_conj_is_complex, Eq[3])
    ...
    Eq << Eq[-1].this.expr.apply(sets.all_is_complex.then.is_complex.sum)

    Eq << Eq[-1].this.find(Sum).apply(discrete.sum.to.matmul)

    Eq << algebra.all.all.then.all.et.apply(Eq.Ati_def, Eq[-1])

    Eq << Eq[-1].this.expr.apply(sets.is_complex.eq.then.eq.add)

    Eq << Eq[-1].this.expr.reversed

    Eq << Eq.Lii_is_positive.this.expr.apply(sets.is_real.then.eq.conj, reverse=True)

    Eq << algebra.all.all.then.all.et.apply(Eq[-1], Eq[-2])

    Eq << Eq[-1].this.expr.apply(algebra.eq.eq.then.eq.subs, swap=True)
