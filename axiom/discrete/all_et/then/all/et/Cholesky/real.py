...
def prove(Eq):
    ...
    Eq[-1], Eq.Lij_is_real = algebra.all_et.then.et.all.apply(Eq[0])
    ...
    Eq << algebra.all.then.cond.subs.apply(Eq.Lij_is_real, i, k)

    Eq <<= Eq.hypothesis & Eq[-1]

    Eq << Eq[-1].this.expr.apply(sets.is_real.is_real.then.is_real)

    Eq << sets.all_is_real.then.is_real.sum.apply(Eq[-1])

    Eq << Eq[-1].this.lhs.apply(discrete.sum.to.matmul)

    Eq << Element(A[t, k], Reals, plausible=True)

    Eq << sets.is_real.is_real.then.is_real.sub.apply(Eq[-1], Eq[-2])

    Eq.Lkk_is_positive = algebra.all.then.cond.subs.apply(Eq.Lii_is_positive, i, k)

    Eq << sets.is_real.is_positive.then.is_real.apply(Eq[-1], Eq.Lkk_is_positive)

    Eq << algebra.all.then.cond.subs.apply(Eq.Ati_def, i, k).this.find(MatMul).T
