...
def prove(Eq):
    ...
    Eq << Eq[-1].this.find(Equal[-2] & Equal[0]).apply(algebra.poly_is_zero.eq_ceiling.imply.ou.eq.cubic, x, ret=0)

    Eq << Eq[-1].this.find(Equal[-1] & Equal[0]).apply(algebra.poly_is_zero.eq_ceiling.imply.ou.eq.cubic, x, ret=0)

    Eq << Eq[-1].this.find(Equal[0] & Equal[0]).apply(algebra.poly_is_zero.eq_ceiling.imply.ou.eq.cubic, x, ret=0)

    Eq << Eq[-1].this.find(Equal[S(1)] & Equal[0]).apply(algebra.poly_is_zero.eq_ceiling.imply.ou.eq.cubic, x, ret=0)

    Eq << Eq[-1].this.find(Equal[S(2)] & Equal[0]).apply(algebra.poly_is_zero.eq_ceiling.imply.ou.eq.cubic, x, ret=0)
