...
def prove(Eq):
    ...
    Eq <<= algebra.infer_et.imply.infer.et.apply(Eq[-2]), Eq[-1].this.rhs.apply(algebra.eq.transport, lhs=0)

    Eq <<= Eq[-2].this.rhs.apply(algebra.eq.cond.imply.cond.subs), algebra.infer_et.imply.infer.et.apply(Eq[-1])
