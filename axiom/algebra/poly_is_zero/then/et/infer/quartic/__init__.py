from util import *
from axiom.algebra.poly_is_zero.then.et.infer.cubic import cubic_root
...
def apply(given, x=None):
    ...
    from axiom.algebra.poly_is_zero.ne_zero.then.et.infer.quartic.depressed import solver_set
...
def prove(Eq):
    ...
    Eq << algebra.cond.then.et.infer.split.apply(Eq[0], cond=Equal(a, 0))

    Eq <<= algebra.infer.then.infer.subs.apply(Eq[-2]), algebra.infer_et.then.infer.et.apply(Eq[-1])

    Eq <<= algebra.infer.then.et.infer.apply(Eq[-2].this.rhs.apply(algebra.poly_is_zero.then.et.infer.cubic), None), Eq[-1].this.rhs.apply(algebra.ne_zero.eq.then.eq.div)

    Eq << algebra.infer.then.et.infer.apply(Eq[-1].this.rhs.apply(algebra.poly_is_zero.then.et.infer.quartic.one_leaded).subs(Eq[1].reversed, Eq[2].reversed, Eq[3].reversed), None)
