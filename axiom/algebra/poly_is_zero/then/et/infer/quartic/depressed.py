...
def prove(Eq):
    ...
    Eq << algebra.cond.then.et.infer.split.apply(Eq[0], cond=Equal(beta, 0))

    Eq <<= algebra.infer.then.infer.subs.apply(Eq[-2]), algebra.infer_et.then.infer.et.apply(Eq[-1])

    Eq << Eq[-2].this.rhs.apply(algebra.poly_is_zero.then.ou_eq.biquadratic, x)

    Eq << algebra.infer.then.et.infer.apply(Eq[-1].this.rhs.apply(algebra.poly_is_zero.ne_zero.then.et.infer.quartic.depressed, x), None)
