...
def prove(Eq):
    ...
    Eq <<= Eq[-2].this.rhs.apply(algebra.poly_is_zero.then.et.infer.quadratic, x=x), Eq[-1].this.rhs.apply(algebra.ne_zero.eq.then.eq.div)

    Eq <<= algebra.infer.then.et.infer.apply(Eq[-2], None), algebra.infer.then.et.infer.apply(Eq[-1].this.rhs.apply(algebra.poly_is_zero.then.et.infer.ou.eq.cubic.one_leaded, x), None)
