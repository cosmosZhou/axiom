...
def prove(Eq):
    ...
    Eq <<= Eq[-2].this.rhs.apply(algebra.eq.cond.then.cond.subs), Eq[-1].this.rhs.apply(algebra.ne_zero.eq.then.ou.quadratic, x=x, simplify=False)

    Eq << Eq[-1].this.rhs.apply(algebra.poly_is_zero.then.et.infer.simple_equation, x=x)

    Eq << algebra.infer.then.et.infer.apply(Eq[-1])
