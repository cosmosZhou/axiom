from util import *


@apply
def apply(is_zero, x=None):
    from axiom.algebra.poly_is_zero.then.et.infer.cubic import cubic_coefficient
    fx = is_zero.of(Equal[0])
    S[1], S[0], p, q = cubic_coefficient(fx, x=x)

    delta = 4 * p ** 3 / 27 + q ** 2
    U = sqrt(delta) - q
    V = -sqrt(delta) - q

    A = (sqrt(delta) / 2 - q / 2) ** (S.One / 3)
    B = (-sqrt(delta) / 2 - q / 2) ** (S.One / 3)

    w = -S.One / 2 + S.ImaginaryUnit * sqrt(3) / 2
    arg_p = Ceiling(3 * Arg(-p / 3) / (S.Pi * 2) - S.One / 2)
    # arg_AB = Ceiling(3 * Arg(A * B) / (S.Pi * 2) - S.One / 2)

    arg_AB = Piecewise((0, Equal(p * Ceiling((Arg(U) + Arg(V)) / (2 * S.Pi) - S.One / 2), 0)), (1, Arg(U) + Arg(V) > S.Pi), (-1, True))

    d = arg_p - arg_AB
    return Infer(Equal(d, 0), Equal(x, A + B) | Equal(x, A * w + B * ~w) | Equal(x, A * ~w + B * w)), Infer(Equal(d % 3, 1), Equal(x, A * w + B) | Equal(x, A * ~w + B * ~w) | Equal(x, A + B * w)), Infer(Equal(d % 3, 2), Equal(x, A * ~w + B) | Equal(x, A + B * ~w) | Equal(x, A * w + B * w))



@prove
def prove(Eq):
    from axiom import sets, algebra

    x, p, q = Symbol(complex=True, given=True)
    Eq << apply(Equal(x ** 3 + p * x + q, 0), x=x)

    Eq << sets.then.el.arg.apply(-p)

    Eq << sets.el.then.el.mul.interval.apply(Eq[-1], 3, simplify=None)

    Eq << sets.el.then.el.sub.apply(Eq[-1], S.Pi, simplify=None)

    Eq << sets.el.then.el.div.interval.apply(Eq[-1], S.Pi * 2, simplify=None)

    Eq << sets.el_interval.then.el_range.ceiling.apply(Eq[-1])

    Eq << Eq[-1].this.find(Mul).apply(algebra.mul.to.add)

    Eq << Eq[-1].this.rhs.apply(sets.range.to.finiteset)

    Eq << sets.then.el.piece.apply(Eq[1].find(Piecewise))

    Eq << sets.el.then.el.neg.apply(Eq[-1])

    Eq << sets.el.el.then.el.add.finiteset.apply(Eq[-1], Eq[-3])

    V = Eq[-1].find(Arg[Add]).arg
    U = Eq[-1].find(Arg[2]).arg
    Eq.eq_peicewise = algebra.ceiling.arg.to.piece.apply(Eq[-1].find(Ceiling)._subs(-p, U ** (S.One / 3) * V ** (S.One / 3)))

    Eq << Eq[-1].subs(Eq.eq_peicewise.reversed)

    Eq.ou = sets.el_finiteset.then.ou.eq.apply(Eq[-1])

    Eq <<= Eq.ou & Eq[0]

    Eq << algebra.et.then.ou.apply(Eq[-1], simplify=None)

    Eq << Eq[-1].this.find(Equal[-2] & Equal[0]).apply(algebra.poly_is_zero.eq_ceiling.then.ou.eq.cubic, x, ret=0)

    Eq << Eq[-1].this.find(Equal[-1] & Equal[0]).apply(algebra.poly_is_zero.eq_ceiling.then.ou.eq.cubic, x, ret=0)

    Eq << Eq[-1].this.find(Equal[0] & Equal[0]).apply(algebra.poly_is_zero.eq_ceiling.then.ou.eq.cubic, x, ret=0)

    Eq << Eq[-1].this.find(Equal[S(1)] & Equal[0]).apply(algebra.poly_is_zero.eq_ceiling.then.ou.eq.cubic, x, ret=0)

    Eq << Eq[-1].this.find(Equal[S(2)] & Equal[0]).apply(algebra.poly_is_zero.eq_ceiling.then.ou.eq.cubic, x, ret=0)

    # find Equal[S(1)] & Equal[S(-2)]
    Eq << Eq[-1].this.args[:3:2].apply(algebra.ou.then.et.collect)

    # find Equal[S(-1)] & Equal[S(2)]
    Eq << Eq[-1].this.args[:2].apply(algebra.ou.then.et.collect)

    Eq << Eq[-1].this.find(Equal[Integer] | Equal[Integer]).apply(algebra.ou_eq.then.eq.mod)

    Eq << Eq[-1].this.find(Equal[Integer] | Equal[Integer]).apply(algebra.ou_eq.then.eq.mod)

    Eq << Eq[-1].this.find(Equal[Ceiling, Ceiling]).apply(algebra.eq.then.is_zero.mod, 3, swap=True)

    Eq << Eq[-1].subs(Eq.eq_peicewise)

    Eq << algebra.ou.then.et.infer.apply(Eq[-1])

    Eq << Eq[1].this.lhs.apply(algebra.eq.then.is_zero.mod, 3)


if __name__ == '__main__':
    run()
# created on 2018-11-24
# updated on 2023-05-15
