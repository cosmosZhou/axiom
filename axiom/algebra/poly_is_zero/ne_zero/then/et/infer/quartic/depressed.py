...
def prove(Eq):
    ...
    Eq << algebra.cond.then.infer.apply(Eq[0] & Eq[1], cond=Eq[2].lhs)

    Eq << algebra.infer_et.then.infer.et.apply(Eq[-1])

    Eq << Eq[-1].this.rhs.apply(algebra.poly_is_zero.poly_is_zero.ne_zero.then.ou_eq.quartic.depressed, x)

    Eq << algebra.cond.then.infer.apply(Eq[0] & Eq[1], cond=Eq[3].lhs)

    Eq << algebra.infer_et.then.infer.et.apply(Eq[-1])

    Eq << Eq[-1].this.rhs.apply(algebra.poly_is_zero.mod_is_zero.ne_zero.then.ou_eq.quartic.depressed, x)

    Eq << algebra.cond.then.infer.apply(Eq[0] & Eq[1], cond=Eq[4].lhs)
