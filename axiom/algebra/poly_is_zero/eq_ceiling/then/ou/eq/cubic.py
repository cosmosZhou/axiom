...
def prove(Eq):
    ...
    Eq.w, Eq.w_conj, Eq.add_ww, Eq.mul_ww, Eq.w_square, Eq.w_conj_square, Eq.w3 = algebra.then.et.eq.omega.apply('omega')

    w = Eq.w.lhs
    Eq.w_sub = Eq.add_ww.this.apply(algebra.eq.transport)

    Eq.w3_conj = algebra.eq.then.eq.conj.apply(Eq.w3)
    ...
    Eq.A3_def = algebra.eq.then.eq.pow.apply(Eq.A_def, exp=3)

    Eq.B3_def = algebra.eq.then.eq.pow.apply(Eq.B_def, exp=3)
    ...
    Eq << algebra.eq_pow.then.eq.cubic_root.omega.apply(Eq.eq_pow)
