...
from axiom.algebra.poly_is_zero.of.et.eq.cubic.one_leaded import cubic_solve
from axiom.algebra.ne_zero.poly_is_zero.then.ne import cubic_delta
...
def prove(Eq):
    ...
    Eq << Equal(Eq[-1].rhs, 0).this.apply(algebra.poly_is_zero.then.et.infer.quadratic, x)

    Eq << Equal(cubic_delta(y, alpha, beta, gamma), 0).this.apply(algebra.poly_is_zero.of.et.eq.mod.cubic.one_leaded, y, d=1)

    Eq << Eq[-1].subs(Eq[1])

    Eq << algebra.cond.assuming.then.cond.trans.apply(Eq[4], Eq[-1], simplify=None)

    Eq << algebra.ne_zero.poly_is_zero.then.ne.apply(Eq[2], Eq[-1], y)

    Eq << algebra.ne.then.ne_zero.apply(Eq[-1]) * 2

    Eq << algebra.ne_zero.poly_is_zero.then.eq.square.apply(Eq[-1], Eq[-3] * -8, Eq.eq.rhs)

    Eq << Eq.eq.subs(Eq[-1])

    Eq << algebra.eq_square.then.ou_is_zero.apply(Eq[-1])

    Eq << Eq[-1].this.args[0].apply(algebra.poly_is_zero.then.et.infer.quadratic)

    Eq.root = Eq[-1].this.args[-1].apply(algebra.poly_is_zero.then.et.infer.quadratic)
    ...
    Eq << algebra.eq.then.eq.div.apply(Eq[-1], 3)
    ...
    Eq << algebra.eq.then.eq.div.apply(Eq[-1], 27)
