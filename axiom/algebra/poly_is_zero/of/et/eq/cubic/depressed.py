...
def prove(Eq):
    ...
    Eq <<= algebra.eq.then.eq.pow.apply(Eq.A, exp=3), algebra.eq.then.eq.pow.apply(Eq.B, exp=3)
    ...
    Eq << algebra.eq.then.eq.pow.apply(Eq[-1], exp=3)

    Eq << Eq[-1].this.rhs.apply(algebra.mul.to.add, deep=True)

    Eq << algebra.eq_pow.then.eq.cubic_root.omega.apply(Eq[-1]) * 3
