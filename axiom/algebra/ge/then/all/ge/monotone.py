...
def prove(Eq):
    ...
    Eq << algebra.ge.ge.then.ge.trans.apply(Eq[-1], Eq[-2])

    Eq << Infer(Eq[2], Eq.induct, plausible=True)

    Eq << algebra.infer.then.cond.induct.apply(Eq[-1], n=m, start=0)

    Eq << algebra.cond.then.all.apply(Eq[2], m)

    Eq << Eq[-1].subs(n, N)

    Eq << Eq[-1].limits_subs(Eq[-1].variable, n)

    Eq << algebra.all.then.all.limits.subs.offset.apply(Eq[-1], -N)
