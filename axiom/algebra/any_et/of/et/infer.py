...
def prove(Eq):
    ...
    Eq <<= algebra.infer.then.infer.et.apply(Eq[-2]), algebra.infer.then.infer.et.apply(Eq[-1])

    Eq <<= Eq[-2].this.rhs.apply(algebra.cond.any.then.any.et, simplify=None), Eq[-1].this.rhs.apply(algebra.cond.any.then.any.et, simplify=None)

    Eq << algebra.infer.infer.then.infer.ou.apply(Eq[-2], Eq[-1])
    ...
    Eq << algebra.any_ou.of.ou.any.apply(Eq[-1])

    Eq << Eq[-1].this.find(And[Or]).apply(algebra.et.to.ou)

    Eq << Eq[-1].this.find(Any[Or]).apply(algebra.any_ou.of.ou.any)

    Eq << Eq[-1].this.args[:2].apply(algebra.ou_any.of.any.ou)
