...
def prove(Eq):
    ...
    Eq << Eq[-1].this.expr.apply(algebra.cond.then.any.et, wrt=j)

    Eq << Eq[-1].this.expr.apply(algebra.eq.cond.then.cond.delta, delta=False, simplify=None, ret=0)

    Eq << algebra.any.then.any.limits.swap.apply(Eq[-1], simplify=None)

    Eq << Eq[0].this.expr.apply(algebra.eq.cond.of.et.subs, simplify=None)
