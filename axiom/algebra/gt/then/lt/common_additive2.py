...
def prove(Eq):
    ...
    Eq.less_than = algebra.then.abs_le.add.substract.apply(Eq[-1].lhs.arg, x - y)
    ...
    Eq << algebra.le.lt.then.lt.add.apply(Eq[-2], Eq[-1])

    Eq << algebra.gt_zero.lt.then.lt.mul.apply(Eq[0], Eq.lt)

    Eq << algebra.le.lt.then.lt.trans.apply(Eq.less_than, Eq[-1])
