...
def prove(Eq):
    ...
    Eq << Equal(Eq[-1].rhs, 0).this.apply(Algebra.Add.eq.Zero.to.And_Imply_Or_EqS_Div.quadratic, x)

    Eq << Equal(cubic_delta(y, alpha, beta, gamma), 0).this.apply(Algebra.Eq_.Add.Zero.of.And.Eq.Mod.cubic.one_leaded, y, d=1)

    Eq << Eq[-1].subs(Eq[1])

    Eq << Algebra.Cond.Given.to.Cond.trans.apply(Eq[4], Eq[-1], simplify=None)

    Eq << Algebra.Ne_0.Eq_.Add.Zero.to.Ne.apply(Eq[2], Eq[-1], y)

    Eq << Algebra.Ne.to.Ne_0.apply(Eq[-1]) * 2

    Eq << Algebra.Ne_0.Eq_.Add.Zero.to.Eq.Square.apply(Eq[-1], Eq[-3] * -8, Eq.eq.rhs)

    Eq << Eq.eq.subs(Eq[-1])

    Eq << Algebra.Eq_Square.to.Or_Eq_0.apply(Eq[-1])

    Eq << Eq[-1].this.args[0].apply(Algebra.Add.eq.Zero.to.And_Imply_Or_EqS_Div.quadratic)

    Eq.root = Eq[-1].this.args[-1].apply(Algebra.Add.eq.Zero.to.And_Imply_Or_EqS_Div.quadratic)
    ...
    Eq << Algebra.Eq.to.Eq.Div.apply(Eq[-1], 3)
    ...
    Eq << Algebra.Eq.to.Eq.Div.apply(Eq[-1], 27)
