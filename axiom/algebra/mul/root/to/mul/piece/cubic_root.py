...
def prove(Eq):
    ...
    Eq << algebra.cond.of.et.infer.split.apply(Eq[0], cond=Equal(A, 0) | Equal(B, 0))
    ...
    Eq << algebra.cond.of.et.infer.split.apply(Eq[-1], cond=Eq[-1].find(ExprCondPair[~Equal]))

    Eq <<= algebra.infer.of.infer.subs.apply(Eq[-2]), algebra.infer.of.infer.subs.bool.apply(Eq[-1], invert=True)

    Eq <<= Eq[-2].this.apply(algebra.infer.flatten), Eq[-1].this.lhs.apply(algebra.ceiling_ne_zero.then.ou_eq.arg)

    Eq << Eq[-2].this.lhs.apply(algebra.ne_zero.ne_zero.eq.then.eq.cubic_root)

    Eq << Eq[-1].this.find(Greater).apply(algebra.gt_arg.to.eq_ceiling, simplify=None)

    Eq << algebra.infer_ou.of.et.infer.apply(Eq[-1])

    Eq <<= algebra.infer.of.infer.subs.apply(Eq[-2]), algebra.infer.of.infer.subs.apply(Eq[-1])

    Eq <<= Eq[-2].this.apply(algebra.infer.flatten), Eq[-1].this.apply(algebra.infer.flatten)
    Eq <<= Eq[-2].this.lhs.apply(algebra.ne_zero.ne_zero.eq.then.eq.cubic_root)
    Eq <<= Eq[-1].this.lhs.apply(algebra.ne_zero.ne_zero.eq.then.eq.cubic_root)
