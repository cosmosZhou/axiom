...
def prove(Eq):
    ...
    Eq <<= algebra.cond.infer.given.et.infer.et.apply(Eq[0], Eq[-2]), algebra.cond.given.et.infer.split.apply(Eq[-1], cond=a < 0)
    ...
    Eq <<= algebra.cond.infer.given.et.infer.et.apply(Eq[0], Eq[-2]), algebra.infer.given.infer.subs.apply(Eq[-1])
