...
def prove(Eq):
    ...
    Eq << algebra.gt.then.gt.relax.apply(Eq[-1], 0)

    Eq << Eq[1] - x

    Eq.is_positive = algebra.gt_zero.gt_zero.then.gt_zero.apply(Eq[-1], Eq[-2])
    ...
    Eq << algebra.lt_zero.ge_zero.then.le_zero.apply(Eq[-2], Eq[-1])

    Eq << algebra.gt.le.then.lt.trans.apply(Eq.is_positive, Eq[-1])
