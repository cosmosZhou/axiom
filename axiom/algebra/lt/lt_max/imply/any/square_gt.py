...
def prove(Eq):
    ...
    Eq <<= algebra.infer.imply.et.infer.apply(Eq[-4], simplify=None), \
        Eq[-3].this.rhs.args[:4:2].apply(algebra.eq.cond.imply.cond.subs, simplify=None), \
    ...
    Eq <<= Eq[-5].this.rhs.rhs.apply(algebra.et.imply.cond, index=slice(2, None, -2), simplify=None), \
        ...
        Eq[-3].this.rhs.args[:3].apply(algebra.le_zero.lt.lt.imply.any.square_gt), \
    ...
    Eq <<= Eq[-3].this.rhs.rhs.args[1].apply(algebra.gt.transport, lhs=0), \
        ...
        Eq[-1].this.rhs.apply(algebra.ge_zero.lt.lt.imply.any.square_gt)

    Eq <<= Eq[-2].this.rhs.rhs.args[1:].apply(algebra.le_zero.gt.imply.eq.max, ret=1), \
        Eq[-1].this.rhs.rhs.args[1:].apply(algebra.gt_zero.le.imply.eq.max, ret=0)

    Eq <<= Eq[-2].this.rhs.rhs.args[:2].apply(algebra.eq.cond.imply.cond.subs), \
        Eq[-1].this.rhs.rhs.args[:2].apply(algebra.eq.cond.imply.cond.subs)

    Eq <<= Eq[-2].this.rhs.apply(algebra.infer_et.imply.infer.et), \
        Eq[-1].this.rhs.rhs.args[1].apply(algebra.gt.imply.ge.relax)

    Eq.is_positive, Eq.is_nonpositive = Eq[-2].this.rhs.rhs.apply(algebra.le_zero.gt_zero.lt.imply.any.square_gt),\
        Eq[-1].this.rhs.apply(algebra.infer_et.imply.infer.et)
    ...
    Eq <<= Eq[-1].this.rhs.rhs.apply(algebra.et.imply.cond, index=slice(None, None, 2)), Eq[-2].this.rhs.rhs.apply(algebra.et.imply.cond, index=0)

    Eq <<= Eq[-2].this.rhs.apply(algebra.infer_et.imply.infer.et), Eq[-1].this.rhs.apply(algebra.infer_et.imply.infer.et)

    Eq.is_negative = Eq[-2].this.rhs.rhs.apply(algebra.ge_zero.lt_zero.lt.imply.any.square_gt)

    Eq << Eq[-1].this.rhs.rhs.args[0].apply(algebra.eq.transport, lhs=0)

    Eq << Eq[-1].this.rhs.rhs.apply(algebra.eq.cond.imply.cond.subs, ret=0)

    Eq << algebra.infer_et.imply.infer.et.apply(Eq[-1], index=0)

    Eq << Eq[-1].this.rhs.apply(algebra.cond.infer.imply.infer.et.rhs)

    Eq << Eq[-1].this.rhs.rhs.args[1:].apply(algebra.gt_zero.lt.imply.any.square_gt)
