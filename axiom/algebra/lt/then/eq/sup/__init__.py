...
def prove(Eq):
    ...
    Eq.eq_min = algebra.lt.then.eq.min.apply(Eq[0])

    Eq << algebra.eq.of.et.squeeze.apply(Eq[-1])

    y = Symbol(real=True)
    Eq <<= algebra.sup_ge.of.all_any_gt.apply(Eq[-1], y), algebra.sup_le.of.all.le.apply(Eq[-2])

    Eq <<= algebra.all.of.et.all.split.apply(Eq[-2], cond=y < m), algebra.all.of.ou.apply(Eq[-1])

    Eq <<= Eq[-2].subs(Eq.eq_min), Eq[-3].this.expr.apply(algebra.any.of.cond.subs, x, (M + y) / 2), Eq[-1].this.find(NotElement).apply(sets.notin_interval.of.ou)
    ...
    Eq <<= Eq[-3].this.lhs.apply(algebra.lt.lt.then.lt.trans, ret=1), Eq[-1].this.rhs.apply(sets.el.of.et.strengthen, upper=m, strict=True)

    Eq <<= Eq[-2].this.lhs.apply(algebra.lt.lt.then.lt.add), algebra.infer.of.cond.apply(Eq[-1])
