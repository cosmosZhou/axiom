...
def prove(Eq):
    ...
    Eq << algebra.is_zero.infer.then.is_zero.induct.apply(Eq.is_zero, Eq.suffice, n=m)
    ...
    Eq << algebra.is_zero.infer.then.is_zero.induct.apply(Eq.is_zero_even, Eq.suffice_even, n=m)

    Eq << Eq[-1].this.lhs.definition

    Eq.even = Eq[-1].reversed

    Eq << algebra.eq.then.all.eq.limit_is_odd.apply(Eq.odd, m)

    Eq << algebra.eq.then.all.eq.limit_is_even.apply(Eq.even, m)
