...
def prove(Eq):
    ...
    Eq << Eq[-1].this.lhs.apply(algebra.cond.then.cond.subs, x, x + 1)

    Eq << algebra.infer_et.then.infer.et.apply(Eq[-1])

    Eq << algebra.infer.infer.then.infer.trans.apply(Eq[-1], Eq[1])

    Eq << algebra.cond.infer.then.cond.induct.apply(Eq[0], Eq[-1], n=n)
