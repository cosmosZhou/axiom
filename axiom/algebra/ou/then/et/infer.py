...
def prove(Eq):
    ...
    Eq << algebra.cond.then.infer.apply(Eq[0], cond=Equal(f(x), 0))

    Eq << algebra.infer.then.infer.subs.apply(Eq[-1])

    Eq << algebra.cond.then.infer.apply(Eq[0], cond=Equal(f(x), 1))

    Eq << algebra.infer.then.infer.subs.apply(Eq[-1])

    Eq << algebra.cond.then.infer.apply(Eq[0], cond=Equal(f(x), 2))

    Eq << algebra.infer.then.infer.subs.apply(Eq[-1])

    Eq << algebra.cond.then.infer.apply(Eq[0], cond=Equal(f(x), 3))
