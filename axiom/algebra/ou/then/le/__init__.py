...
def prove(Eq):
    ...
    Eq << Eq[0].this.args[1].args[1].apply(sets.el_complement.then.et, simplify=None)
    ...
    Eq << Eq[-1].this.find(Or).apply(algebra.ou.then.le.two, wrt=p)

    Eq << Eq[-1].apply(algebra.ou.then.le.two, wrt=p)
