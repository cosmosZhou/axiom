...
def prove(Eq):
    ...
    Eq << Eq[-1].this.expr.apply(algebra.cond.then.eq.bool, split=False)

    Eq << algebra.cond.then.eq.bool.invert.apply(Eq[1])

    Eq << Eq[-2].this.expr.apply(algebra.eq.eq.then.eq.trans, Eq[-1])
