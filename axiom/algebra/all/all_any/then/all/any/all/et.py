...
def prove(Eq):
    ...
    Eq << Eq[-1].this.expr.expr.apply(algebra.all.of.ou)

    Eq << Eq[-1].this.expr.apply(algebra.any_ou.of.ou.any, simplify=None)

    Eq << Eq[-1].this.find(Any).apply(algebra.any.of.cond, simplify=None)

    Eq << Eq[-1].this.expr.apply(algebra.ou.of.all)

    Eq << Eq[-1].this.expr.apply(algebra.any_et.of.et, index=0)
