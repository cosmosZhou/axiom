...
def prove(Eq):
    ...
    Eq << Eq.induct.this.find(Minima).apply(algebra.minima.to.min.pop)

    Eq << Eq[-1].this.find(GreaterEqual[~Minima]).apply(algebra.minima.to.min.pop)

    Eq << algebra.infer.then.infer.et.both_sided.apply(Eq[2], cond=f(n) >= g(n))

    Eq << Eq[-1].this.lhs.apply(algebra.cond.all.of.all.push)

    Eq << Eq[-1].this.rhs.apply(algebra.ge.ge.then.ge.min)
