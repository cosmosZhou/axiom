...
def prove(Eq):
    ...
    Eq.delta_is_nonnegative = algebra.ge_zero.then.ge_zero.sqrt.apply(Eq[1])

    Eq << Eq.delta_is_nonnegative - b

    Eq << algebra.lt_zero.ge.then.le.div.apply(Eq[0], Eq[-1])

    Eq << Eq[-1] / 2

    Eq << sets.le.then.el.interval.apply(Eq[-1])

    Eq << sets.el.then.el.relax.apply(Eq[-1], Reals, simplify=None)

    epsilon = Symbol(negative=True)
    Eq << sets.el.then.el.add.apply(Eq[-1], epsilon, simplify=None)

    Eq << algebra.any.of.cond.subs.apply(Eq[2], x, Eq[-1].lhs)

    Eq << algebra.et.of.et.apply(Eq[-1], simplify=None)
    ...
    Eq << algebra.gt_zero.ge_zero.then.gt_zero.add.apply(Eq[-1], Eq.delta_is_nonnegative)
