...
def prove(Eq):
    ...
    Eq << algebra.lt_zero.lt.then.gt.mul.apply(Eq[0], Eq[1])

    Eq << algebra.gt.then.eq.max.apply(Eq[-1]) + b

    Eq << Eq[-1].this.lhs.apply(algebra.add.to.max)

    Eq << algebra.lt_zero.lt.then.eq.sup.st.simple.apply(Eq[0], Eq[1], a * x + b, x)
