...
def prove(Eq):
    ...
    Eq << sets.lt_zero.then.is_negative.apply(Eq[0], simplify=None)

    Eq.a_reciprocal_is_negative = sets.is_negative.then.is_negative.div.apply(Eq[-1])

    t = Symbol(x + Eq.a_reciprocal_is_negative.lhs * b / 2)
    Eq << t.this.definition

    Eq << algebra.eq.then.eq.transport.apply(Eq[-1], rhs=1)
    ...
    Eq << algebra.lt_zero.lt_zero.then.gt_zero.apply(Eq.a_reciprocal_is_negative, Eq[1])

    Eq << -Eq[-1].this.lhs.apply(algebra.mul.to.add) / 4

    Eq << GreaterEqual(t ** 2, 0, plausible=True)

    Eq << algebra.lt_zero.ge_zero.then.le_zero.apply(Eq[0], Eq[-1])

    Eq << algebra.le_zero.lt_zero.then.lt_zero.add.apply(Eq[-1], Eq[-3])
