...
def prove(Eq):
    ...
    Eq << algebra.cond.then.et.infer.split.apply(Eq[0], cond=b ** 2 - 4 * a * c >= 0)

    Eq <<= algebra.infer_et.then.infer.et.apply(Eq[-2]), algebra.infer_et.then.infer.et.apply(Eq[-1])

    Eq <<= Eq[-2].this.rhs.apply(algebra.lt_zero.add_ge_zero.then.any.lt_zero, x=x), Eq[-1].this.rhs.apply(algebra.lt_zero.add_lt_zero.then.lt_zero, x=x)

    Eq << Eq[-1].this.rhs.apply(algebra.cond.then.any, wrt=x)
