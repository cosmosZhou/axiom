...
def prove(Eq):
    ...
    Eq << Eq[-1].lhs.this.apply(algebra.square.abs.to.mul.conj)

    Eq << Eq[-1].this.rhs.apply(algebra.mul.to.add, deep=True)

    Eq << Eq[-1].this.rhs.args[0].apply(algebra.mul.conj.to.square.abs)

    Eq << Eq[-1].this.rhs.args[:4:2].apply(algebra.add.to.mul.re)

    Eq << Eq[-1].this.rhs.args[1].apply(algebra.mul.conj.to.square.abs)
