...
def prove(Eq):
    ...
    Eq << algebra.ge.then.ge.relax.apply(Eq[-1], 0)

    Eq << Eq[1] - x

    Eq.is_nonnegative = algebra.ge_zero.ge_zero.then.ge_zero.apply(Eq[-1], Eq[-2])
    ...
    Eq << algebra.le_zero.ge_zero.then.le_zero.apply(Eq[-2], Eq[-1])

    Eq << algebra.ge.le.then.le.trans.apply(Eq.is_nonnegative, Eq[-1])
