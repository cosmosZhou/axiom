...
def prove(Eq):
    ...
    Eq << algebra.le.then.le.square.apply(Eq[0])
    ...
    Eq << Eq[-1].this.lhs.find(Sum).expr.apply(algebra.square.negate, simplify=None)

    Eq << Eq[-1] - Eq[-1].lhs.args[0]

    Eq << Eq[-1].this.rhs.collect(Eq[-2].lhs.find(Sum))

    Eq.is_nonnegative = algebra.le.of.ge_zero.apply(Eq[-1])
    ...
    Eq << algebra.le.le.then.le.trans.apply(Eq.le_plausible, Eq.le_given)
