...
def prove(Eq):
    ...
    Eq << discrete.eq_matmul.then.eq.sum.apply(Eq[2], j)
    ...
    Eq << algebra.le.then.le.st.square.pop.apply(Eq[-1])
    ...
    Eq << discrete.eq_matmul.then.eq.sum.square.double_limits.apply(Eq[2])

    Eq << Eq[-2].subs(Eq[-1].reversed)

    Eq << discrete.eq_matmul.then.eq.sum.square.offset.apply(Eq[2], y[t], j)
