...
def prove(Eq):
    ...
    Eq << Eq[-1].rhs.args[0].this.apply(algebra.sum.square.to.div.sum.square)
    ...
    Eq << Eq[-1].this.lhs.find(Sum).apply(algebra.sum.square.to.div.sum.square)

    Eq << Eq[-1].this.rhs.find(Sum).apply(algebra.sum.square.to.div.sum.square)

    Eq << Eq[-1].this.rhs.args[1].apply(algebra.sum.square.to.div.sum.square)

    Eq << algebra.le.imply.le.st.square.pop.apply(Eq[0])

    Eq << Eq[-1].this.lhs.find(Sum).limits_subs(i, j)

    Eq << Eq[-1].this.lhs.args[1].find(Sum).limits_subs(i, j)

    Eq << Eq[-1].this.lhs.args[0].find(Sum).expr.apply(algebra.square.negate)
