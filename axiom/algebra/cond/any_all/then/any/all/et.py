...
def prove(Eq):
    ...
    Eq << Eq[-1].this.expr.apply(algebra.all_et.of.et.all, simplify=None)

    Eq << Eq[-1].this.find(Or).apply(algebra.ou.of.cond, 1, simplify=None)

    Eq << ~Eq[-1]

    Eq << algebra.cond.all.then.all.et.apply(Eq[0], Eq[-1], simplify=None)
