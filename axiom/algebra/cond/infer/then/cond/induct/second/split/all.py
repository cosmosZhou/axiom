...
def prove(Eq):
    ...
    Eq << Eq[-1].this.rhs.apply(algebra.cond.all.then.all.push)

    Eq << algebra.cond.infer.then.cond.induct.apply(Eq[0], Eq[-1], n=n, start=1)

    Eq << algebra.all.then.ou.subs.apply(Eq[-1], k, n - 1)
