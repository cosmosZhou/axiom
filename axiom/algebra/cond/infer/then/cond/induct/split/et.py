...
def prove(Eq):
    ...
    Eq << Eq[2].cond.this.apply(algebra.cond.then.cond.subs, x, t(x))

    Eq << algebra.infer_et.then.infer.et.apply(Eq[-1])
    Eq.induct = algebra.infer.infer.then.infer.trans.apply(Eq[-1], Eq[1])
    Eq << algebra.cond.infer.then.cond.induct.apply(Eq[0], Eq.induct, n=n)
