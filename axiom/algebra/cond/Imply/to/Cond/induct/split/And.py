...
def prove(Eq):
    ...
    Eq << Eq[2].cond.this.apply(Algebra.Cond.to.Cond.subs, x, t(x))

    Eq << Algebra.Imply_And.to.Imply.And.apply(Eq[-1])
    Eq.induct = Algebra.Imply.Imply.to.Imply.trans.apply(Eq[-1], Eq[1])
    Eq << Algebra.Cond.Imply.to.Cond.induct.apply(Eq[0], Eq.induct, n=n)
