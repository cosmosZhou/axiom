...
def prove(Eq):
    ...
    Eq << Algebra.Cond.to.Imply.apply(Eq[0], cond=Equal(f(x), 0))

    Eq << Algebra.Imply.to.Imply.subs.apply(Eq[-1])

    Eq << Algebra.Cond.to.Imply.apply(Eq[0], cond=Equal(f(x), 1))

    Eq << Algebra.Imply.to.Imply.subs.apply(Eq[-1])

    Eq << Algebra.Cond.to.Imply.apply(Eq[0], cond=Equal(f(x), 2))

    Eq << Algebra.Imply.to.Imply.subs.apply(Eq[-1])

    Eq << Algebra.Cond.to.Imply.apply(Eq[0], cond=Equal(f(x), 3))
