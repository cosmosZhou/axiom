...
def prove(Eq):
    ...
    Eq.all_is_nonnegative = algebra.all.then.all.limits.restrict.apply(Eq[2], domain=Range(n))

    Eq << algebra.all_ge_zero.then.ge_zero.sum.apply(Eq.all_is_nonnegative)

    Eq << algebra.eq.ge.then.le.sub.apply(Eq.eq, Eq[-1])

    Eq << algebra.ge.le.then.eq.apply(Eq[1], Eq[-1])
    ...
    Eq << algebra.all.any.then.any.et.apply(Eq.all_is_nonnegative, Eq[-1])
    ...
    Eq << algebra.all.then.all.limits.restrict.apply(Eq.all_is_nonnegative, Range(n) - {i})
