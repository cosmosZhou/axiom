...
def prove(Eq):
    ...
    Eq.square_is_zero = algebra.eq.then.eq.pow.apply(Eq[-1], exp=2)

    Eq.Im_is_positive = Greater(Im(x) ** 2, 0, plausible=True)

    Eq << GreaterEqual(Re(x) ** 2, 0, plausible=True)

    Eq << algebra.gt_zero.ge_zero.then.gt_zero.add.apply(Eq.Im_is_positive, Eq[-1])

    Eq << Eq[-1].subs(Eq.square_is_zero)

    Eq << ~Eq.Im_is_positive

    Eq << algebra.le_zero.then.is_zero.apply(Eq[-1])

    Eq.Im_is_zero = algebra.square_is_zero.then.is_zero.real.apply(Eq[-1])

    Eq.Re_is_positive = Greater(Re(x) ** 2, 0, plausible=True)

    Eq << GreaterEqual(Im(x) ** 2, 0, plausible=True)

    Eq << algebra.gt_zero.ge_zero.then.gt_zero.add.apply(Eq.Re_is_positive, Eq[-1])
    ...
    Eq.Re_is_zero = algebra.square_is_zero.then.is_zero.real.apply(Eq[-1])
