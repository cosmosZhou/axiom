...
def prove(Eq):
    ...
    Eq << Eq[-1].this.expr.apply(Algebra.Cond.to.Any.And, wrt=j)

    Eq << Eq[-1].this.expr.apply(Algebra.Eq.Cond.to.Cond.Delta, delta=False, simplify=None, ret=0)

    Eq << Algebra.Any.to.Any.limits.swap.apply(Eq[-1], simplify=None)

    Eq << Eq[0].this.expr.apply(Algebra.Eq.Cond.of.And.subs, simplify=None)
