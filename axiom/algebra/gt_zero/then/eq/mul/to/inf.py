...
def prove(Eq):
    ...
    y_ = Eq[-4].variable
    Eq <<= algebra.all.then.infer.apply(Eq[-4]), algebra.inf_ge.of.all.ge.apply(Eq[-1]), algebra.inf_le.of.all_any_lt.apply(Eq[-2])

    Eq <<= Eq[-3].subs(y_, Eq[2].lhs * y_), Eq[-2].this.expr.apply(algebra.ge.of.et.scale.positive, a, div=True), algebra.all.of.infer.apply(Eq[-1])

    Eq << algebra.et.of.et.apply(Eq[-2])

    Eq << Eq[-3].this.rhs.apply(algebra.cond.any.then.any.et, Eq[0], simplify=None)

    Eq << Eq[-1].this.find(And).apply(algebra.gt_zero.lt.then.lt.mul)

    Eq << Eq[-1].this.lhs.apply(algebra.gt.of.et.scale.positive, a)
