...
def prove(Eq):
    ...
    Eq << Eq[-1].this.find((Expr < 0) & (Expr >= 0)).apply(algebra.ge_zero.lt_zero.then.le_zero)

    Eq << Eq[-1].this.find((Expr < 0) & (Expr >= 0)).apply(algebra.ge_zero.lt_zero.then.le_zero)

    Eq << Eq[-1].this.find((Expr > 0) & (Expr <= 0)).args[:2].apply(algebra.le_zero.gt_zero.then.le_zero)
