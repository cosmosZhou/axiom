...
def prove(Eq):
    ...
    Eq.a_reciprocal = algebra.gt_zero.then.gt_zero.div.apply(Eq[0])
    ...
    Eq << algebra.lt.then.eq.sup_square.to.max.apply(Eq[-1])

    Eq << algebra.gt_zero.then.eq.mul.to.sup.apply(Eq[0], Eq[-1].lhs)
    ...
    Eq << algebra.gt_zero.then.eq.mul.to.max.apply(Eq[0], Eq[-1].lhs.find(Max))
