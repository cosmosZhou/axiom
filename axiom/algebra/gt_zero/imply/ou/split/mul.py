...
def prove(Eq):
    ...
    Eq << Eq[-1].this.find(And).apply(algebra.et.imply.ou)

    Eq << Eq[-1].this.find((Expr < 0) & (Expr >= 0)).apply(algebra.ge_zero.lt_zero.imply.le_zero)

    Eq << Eq[-1].this.find((Expr < 0) & (Expr >= 0)).apply(algebra.ge_zero.lt_zero.imply.le_zero)

    Eq << Eq[-1].this.find((Expr > 0) & (Expr <= 0)).args[:2].apply(algebra.le_zero.gt_zero.imply.le_zero)
