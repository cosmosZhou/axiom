...
def prove(Eq):
    ...
    Eq << algebra.gt_zero.then.gt_zero.sqrt.apply(Eq[0] * Eq[1])

    Eq << algebra.gt_zero.gt.then.gt.mul.apply(Eq[-1], Eq[-2])

    Eq << algebra.gt_zero.gt.then.gt.square.apply(Eq[-2], Eq[-1])

    Eq << algebra.then.square_le.Cauchy.apply(a, b)
