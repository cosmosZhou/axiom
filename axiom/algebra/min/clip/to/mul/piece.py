...
def prove(Eq):
    ...
    Eq << algebra.cond.of.et.infer.split.apply(Eq[0], cond=Eq[-1].find(GreaterEqual))

    Eq <<= algebra.infer.of.infer.subs.bool.apply(Eq[-2]), algebra.infer.of.infer.subs.bool.apply(Eq[-1], invert=True)

    Eq << algebra.infer.of.et.infer.split.apply(Eq[-2], cond=A_t > 0)

    Eq << algebra.infer.of.infer.subs.apply(Eq[-1])

    Eq <<= algebra.infer.of.infer.et.apply(Eq[-3]), algebra.infer.of.infer.et.apply(Eq[-2])

    Eq <<= Eq[-2].this.rhs.apply(algebra.lt_zero.eq.of.et.div), Eq[-1].this.rhs.apply(algebra.gt_zero.eq.of.et.div)

    Eq <<= Eq[-2].this.lhs.apply(algebra.lt_zero.then.eq.abs), Eq[-1].this.lhs.apply(algebra.gt_zero.then.eq.abs)
