...
def prove(Eq):
    ...
    Eq << algebra.is_zero.imply.eq.sum.to.neg.sum.apply(Eq[0], Eq[1].find(Sum), i=i)

    Eq << algebra.eq.imply.eq.abs.apply(Eq[-1])

    Eq << algebra.imply.abs_le.sum.abs.apply(Eq[-1].rhs.arg)

    Eq << algebra.eq.le.imply.le.transit.apply(Eq[-2], Eq[-1])

    Eq << Eq[-1].this.find(Abs[Mul]).apply(algebra.abs.to.mul)

    Eq << algebra.imply.all.le_maxima.apply(Eq[1].find(Maxima))

    Eq << Eq[-1].this.expr.apply(algebra.le.imply.le.mul, Eq[-2].rhs.find(Abs[Sum]))

    Eq << algebra.all_le.imply.le.sum.apply(Eq[-1])

    Eq << algebra.le.le.imply.le.transit.apply(Eq[-4], Eq[-1])
