...
def prove(Eq):
    ...
    Eq << algebra.ge_zero.lt.lt.then.any.square_gt.apply(Eq[-2], Eq[-1], Eq[2])

    Eq << sets.le_zero.then.subset.interval.upper.apply(Eq[0], M, left_open=True, right_open=True)

    Eq << sets.subset.any.then.any.apply(Eq[-1], Eq[-2])
