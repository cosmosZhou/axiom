...
def prove(Eq):
    ...
    Eq.is_multiplication_zero = Algebra.Or.to.Eq_0.apply(Eq.or_statement)
    ...
    Eq << (D[:m] @ E).this.apply(Discrete.Dot.eq.Lamda)
    ...
    Eq << Eq[-1].this.rhs.expr.apply(Algebra.Add.eq.Piece)
    ...
    Eq << Eq[-1].this.args[1].apply(Sets.NotIn.to.NotIn.Add, 1)

    Eq << Algebra.Or.to.All.apply(Eq[-1], 1)

    Eq << Algebra.Eq.Eq.to.Eq.subs.apply(Eq[-1].reversed, Eq[-5])

    k = Symbol(integer=True)
    E_quote = Symbol("E'", Lamda[j:m, i:m](Piecewise((Product[k:i + 1:j + 1](D[k]), j >= i), (0, True))))
    Eq.D_is_zero = Eq[-1] @ E_quote

    Eq << (E @ E_quote).this.apply(Discrete.Dot.eq.Lamda)
    ...
    Eq << (-Eq[-1].rhs.expr.args[0].args[0].expr).this.apply(Algebra.Mul.eq.Prod.limits.unshift)
