...
def prove(Eq):
    ...
    Eq << algebra.ne_zero.eq.then.eq.scalar.apply(Eq[0], Eq[-1])
    ...
    Eq << algebra.eq.eq.then.eq.trans.apply(Eq[-2], Eq.t_squared)
    ...
    Eq << algebra.ou.of.et.collect.apply(Eq[-1])

    Eq << algebra.ou.of.eq.abs.apply(Eq[-1])

    Eq << Eq[-1].this.rhs.apply(algebra.mul.to.abs)

    Eq << algebra.ou.then.eq.abs.apply(Eq.ou)
