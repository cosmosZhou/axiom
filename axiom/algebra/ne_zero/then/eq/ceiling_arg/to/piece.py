...
def prove(Eq):
    ...
    Eq << Or(*Eq[-1].find(Or).args[:2]).this.apply(algebra.ou.then.is_zero)

    Eq << Eq[-1].subs(Eq.UV)

    Eq << Eq[-1].this.rhs * (-Integer(27) / 4)

    Eq.suffice = Eq[-1].this.rhs.apply(algebra.pow_is_zero.then.is_zero)

    Eq << Equal(U * V, 0).this.apply(algebra.mul_is_zero.then.ou.is_zero)

    Eq << Eq[-1].subs(Eq.UV)

    Eq << Eq[-1].this.lhs * (-Integer(27) / 4)

    Eq << Eq[-1].this.lhs.apply(algebra.pow_is_zero.of.is_zero)
    ...
    Eq.p_is_positive = algebra.ne_zero.then.gt_zero.abs.apply(Eq[0])

    Eq << algebra.gt_zero.then.eq.arg.apply(Eq.p_is_positive, Eq.p_cubic.find(Exp))

    Eq << algebra.eq.then.eq.arg.apply(Eq.p_cubic)
    ...
    Eq << algebra.gt_zero.then.eq.arg.apply(Eq.p_is_positive, Mul(*Eq[-1].rhs.args[1:]))

    Eq << algebra.eq.then.eq.arg.apply(Eq[-2])
    ...
    Eq.p3_contains = sets.then.el.arg.apply(-p ** 3)

    Eq << sets.el.then.el.add.apply(Eq.p3_contains, S.Pi * 2, simplify=None)

    Eq << sets.el.then.el.div.interval.apply(Eq[-1], 3, simplify=None)

    Eq << sets.el.then.eq.arg.expi.apply(Eq[-1])
    ...
    Eq << sets.el.then.el.sub.apply(Eq.p3_contains, S.Pi * 2, simplify=None)
