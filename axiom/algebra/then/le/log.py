...
def prove(Eq):
    ...
    Eq << algebra.all.of.et.all.apply(Eq[-1], cond=x >= 1)
    ...
    Eq << calculus.gt_zero.then.lt.monotony.apply(Eq.is_positive)

    Eq << algebra.lt.then.le.relax.apply(Eq[-1])

    Eq << calculus.le_zero.then.le.monotony.apply(Eq.is_nonpositive)
