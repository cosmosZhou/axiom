...
def prove(Eq):
    ...
    Eq << Eq[-1].this.find(All).apply(algebra.all_ne.then.eq.sum, Eq.hypothesis.rhs.lhs)

    Eq << Eq[-1].this.lhs.find(Equal).apply(algebra.eq.transport, rhs=0)

    Eq << Eq[-1].this.lhs.find(Equal).reversed

    Eq << algebra.infer.then.infer.et.both_sided.apply(Eq.hypothesis, cond=Eq[-1].lhs.find(Equal))

    Eq << Eq[-1].this.rhs.apply(algebra.eq.eq.then.eq.add)
