...
def prove(Eq):
    ...
    Eq << algebra.all.all.then.all.limits_union.apply(Eq[-1], Eq[0])

    Eq << algebra.all.then.ou.apply(Eq[-1])

    Eq << algebra.ou.then.infer.apply(Eq[-1], 1)

    Eq << Element(i, Range(-1, n + 1)).this.apply(sets.el_range.then.eq.union.to.complement)

    Eq << Eq[-1].this.lhs.apply(sets.el.of.el.restrict, Range(0, n))
    ...
    Eq << Eq[-1].this.expr.apply(algebra.ou.then.all, 1)
