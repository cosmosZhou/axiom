...
def prove(Eq):
    ...
    Eq << Eq[-1].this.find(~Mul + Mul).apply(algebra.mul.to.add)

    Eq << Eq[-1].this.rhs.find(Mul[Add]).apply(algebra.mul.to.add)

    Eq << algebra.eq_reducedSum.eq_reducedSum.eq_div.then.eq.sum.square.parallel_variance.apply(*Eq[:2], Eq[-1], k)
