...
def prove(Eq):
    ...
    Eq << algebra.et.then.cond.apply(Eq[-1], 1).reversed + 1

    Eq << algebra.gt.then.gt.relax.apply(Eq[-1], lower=0)

    Eq << algebra.gt_zero.eq.of.et.mul.apply(Eq[-1], Eq[1], simplify=None)
