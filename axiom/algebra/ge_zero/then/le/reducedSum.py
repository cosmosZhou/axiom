...
def prove(Eq):
    ...
    Eq << algebra.ge.then.all.ge.apply(Eq[0], i)

    Eq << algebra.ge_zero.then.ge_zero.sum.apply(Eq[-1], (i, Range(n) - {i}))

    Eq << algebra.eq.ge.then.ge.add.apply(Eq[-3], Eq[-1])

    Eq << algebra.cond.then.all.restrict.apply(Eq[-1], (i, 0, n))
