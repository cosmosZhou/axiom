...
def prove(Eq):
    ...
    Eq <<= Eq[-2].this.rhs.apply(algebra.et.given.et.div.lt_zero.eq), Eq[-1].this.rhs.apply(algebra.et.given.et.div.gt_zero.eq)
    ...
    Eq << Eq[-1].this.find(Max).apply(algebra.max.to.piece.le)

    Eq << Eq[-2].this.find(Piecewise).apply(algebra.piece.swap)
