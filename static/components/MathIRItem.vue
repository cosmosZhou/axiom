<template>
	<div v-if=show tabindex=12 @keydown=keydown>
		...
		<a :href=href target="_blank">MathML</a>:
		<math display="block" xmlns="http://www.w3.org/1998/Math/MathML" v-html="mathml"></math><br>
		LaTex:
		<p v-render v-text=`\\[${latex}\\]`></p>
		<input type=hidden name=training[] :value=training>
		<input type=hidden name=latex[] :value=latex>
		<input type=hidden name=mathml[] :value=mathml>
		<br>
		<br>
	</div>
</template>
...
<script>
console.log('import MathIRItem.vue');
...
export default {
	...
	props : [ 'id', 'latex', 'mathml', 'offset', 'training', 'index'],
	...
	created(){
		this.$parent.MathIRItem[this.index] = this;
